...
AbaqusCAE_ADAMS.py, version 1.4.0
...
def readAndDefineMarkers(key, MAIN, myAsm, lodFilePath):                                                                 # Read marker data from the LOD files and define them in the Abaqus assembly
    logger = logging.getLogger()                                                                                # Open the corresponding LOD file for the part 
    i=1
...
def AbaqusCAE_ADAMS(instRef, includeAnchoredPart, anchoredPart, myModel,
    ...
    for (key,entry) in instRef.iteritems():                                                             # Iterate through all parts
        if key in parts_to_be_merged:
            ...
            ADAMS_setup_INP(myAsm,key,jointsMBDXML,instRef, instAssemblyRef,
                            myModel,parallelCores,ramAllocated)
    ...
    for j in merging_asms:
        ...
        ADAMS_setup_INP(myAsm,j,jointsMBDXML,instRef, instAssemblyRef,
                        myModel,parallelCores,ramAllocated)
