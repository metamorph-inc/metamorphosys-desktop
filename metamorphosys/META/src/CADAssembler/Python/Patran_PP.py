...
# Don't stop run if args.ResultsJson cannot be found.  Log this later
#if (not os.path.exists(args.ResultsJson)):
#	print("Error: %s file not found" % ResultsJson)

if (not os.path.exists(os.path.expandvars(META_PATH + '\\bin\\CAD\\' + LIB_FILE_NAME))):
	print("Error: %s file not found in Meta-Tools installation" % LIB_FILE_NAME)

shutil.copy2(os.path.expandvars(META_PATH + '\\bin\\CAD\\' + LIB_FILE_NAME), os.getcwd())
...
print "\nStarting Patran \n"
command = "patran -b -graphics -sfp " +  filename + "_PP.ses -stdout " + filename + "_PP_log.txt"
print command
...
if(retcode == 0):
    ...
    print "Patran process returned with an error: " + str(retcode)
    exit(-1)
