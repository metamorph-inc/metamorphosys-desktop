...
package Incompressible
 ...
 package Examples
  ...
  // locally defined classes in TestGlycol
      package Medium = Glycol47 "Medium model (Glycol47)";

  // components of TestGlycol
  ...
  // algorithms and equations of TestGlycol
  equation
   medium.p = 1e5;
...
<li><b>Essotherm650</b>, a medium model for thermal oil, also based on tables.</li>
 ...
 package TableBased
  ...
  // algorithms and equations of invertTemp
  algorithm
  ...
  redeclare model
   extends BaseProperties(final standardOrderComponents=true,p_bar=Cv.to_bar(p),T_degC(start=T_start-273.15)=Cv.to_degC(T),T(start=T_start,stateSelect=(if preferredMediumStates then StateSelect.prefer else StateSelect.default)))
...
which is only exactly true for a fluid with constant density d=d0.
  ...
  // algorithms and equations of s_T
  algorithm
 ...
 as a function of the thermodynamic state record"
  ...
  // components of h_pT
   ...
   input SI.Temperature T "Temperature";
   input Boolean densityOfT=false "include or neglect density derivative dependence of enthalpy" annotation(Evaluate);
   output SI.SpecificEnthalpy h "Specific enthalpy at p, T";
  ...
  // components of density_T
   input Temperature T "temperature";
   output Density d "density";

  // algorithms and equations of density_T
  algorithm
  ...
  // locally defined classes in T_ph
  ...
  package Polynomials_Temp
   ...
   // algorithms and equations of derivativeValue
   algorithm
      y:=p[1]*(n-1);
   ...
   // components of secondDerivativeValue
    input Real p[:] "Polynomial coefficients (p[1] is coefficient of highest power)";
    input Real u "Abszissa value";
   ...
   // algorithms and equations of secondDerivativeValue
   algorithm
      y:=p[1]*(n-1)*(n-2);
   ...
   // algorithms and equations of integral
   algorithm
      for j in 1:n-1 loop
        p2[j]:=p1[j]/(n-j);
      end for;
   ...
   // algorithms and equations of integralValue
   algorithm
      for j in 1:n loop
   ...
   // algorithms and equations of fitting
   algorithm
      V[:,n+1]:=ones(size(u,1));
      for j in n:-1:1 loop
...
<pre>
  p(u) = p[1]*u^n + p[2]*u^(n-1) + ... + p[n]*u + p[n+1];
   ...
   // algorithms and equations of evaluate_der
   algorithm
      dy:=p[1]*(n-1);
      for j in 2:size(p,1)-1 loop
        dy:=p[j]*(n-j)+u*dy;
   ...
   // components of integralValue_der
    input Real p[:] "Polynomial coefficients";
    input Real u_high "High integrand value";
    input Real u_low=0 "Low integrand value, default 0";
   ...
   // algorithms and equations of integralValue_der
   algorithm
   ...
   // components of derivativeValue_der
    input Real p[:] "Polynomial coefficients (p[1] is coefficient of highest power)";
    input Real u "Abszissa value";
   ...
   // algorithms and equations of derivativeValue_der
   algorithm
...
<li><i>March 1, 2004</i> by Martin Otter (DLR):<br>
 ...
 // components of TableBased
  ...
  final constant Real poly_rho[:]=(if hasDensity then Poly.fitting(tableDensity[:,1],tableDensity[:,2],npol) else zeros(npol+1)) annotation(__Dymola_keepConstant=true);
  final constant Real poly_Cp[:]=(if hasHeatCapacity then Poly.fitting(tableHeatCapacity[:,1],tableHeatCapacity[:,2],npol) else zeros(npol+1)) annotation(__Dymola_keepConstant=true);
  final constant Real poly_eta[:]=(if hasViscosity then Poly.fitting(invTK,Math.log(tableViscosity[:,2]),npol) else zeros(npol+1)) annotation(__Dymola_keepConstant=true);
  final constant Real poly_pVap[:]=(if hasVaporPressure then Poly.fitting(tableVaporPressure[:,1],tableVaporPressure[:,2],npol) else zeros(npol+1)) annotation(__Dymola_keepConstant=true);
  final constant Real poly_lam[:]=(if size(tableConductivity,1)>0 then Poly.fitting(tableConductivity[:,1],tableConductivity[:,2],npol) else zeros(npol+1)) annotation(__Dymola_keepConstant=true);
