...
<blockquote><pre>
 ...
 // algorithms and equations of repeat
 algorithm
    repeatedString:="";
    for i in 1:n loop
...
optionally ignoring case.
 ...
 // algorithms and equations of count
 algorithm
    result:=0;
    while i<>0 loop
...
"));
 ...
 // algorithms and equations of findLast
 algorithm
...
"));
 ...
 // algorithms and equations of replace
 algorithm
    result:=(if startIndex==1 then "" else substring(string,1,startIndex-1));
    while i>0 loop
      i_found:=find(string,searchString,i,caseSensitive);
      if i_found>0 then
        result:=(if i_found==1 then replaceString else result+(if (i_found-1)<i then "" else substring(string,i,i_found-1))+replaceString);
...
<blockquote><pre>
 ...
 // algorithms and equations of scanToken
 algorithm
    token.real:=0.0;
    token.integer:=0;
    token.boolean:=false;
...
<blockquote><pre>
 ...
 // algorithms and equations of scanReal
 algorithm
...
is not present in \"string\",  an assert is triggered.
 ...
 // algorithms and equations of scanInteger
 algorithm
...
leading white space.
 ...
 // algorithms and equations of scanBoolean
 algorithm
...
leading white space.
 ...
 // algorithms and equations of scanString
 algorithm
...
leading white space.
 ...
 // algorithms and equations of scanIdentifier
 algorithm
    (nextIndex,identifier):=Advanced.scanIdentifier(string,startIndex);
    if nextIndex==startIndex then
      nextIndex:=Advanced.skipWhiteSpace(string,startIndex);
...
leading white space.
 ...
 // locally defined classes in scanDelimiter

 // components of scanDelimiter
  input String string "String to be scanned";
 ...
 protected
  ...
  encapsulated function concatenate
   "Concatenate strings together"

  // components of concatenate
   input String strings[:];
   output String string;

  // algorithms and equations of concatenate
  algorithm
     string:="{\"";
 ...
 // algorithms and equations of scanDelimiter
 algorithm
    ...
    if found then
      ...
      if size(requiredDelimiters,1)==1 then
        ...
        syntaxError(string,nextIndex,"Expected a delimiter out of "+concatenate(requiredDelimiters)+"\n"+message);
      end if;
    end if;
...
is the index of the first non white space character.
 ...
 // algorithms and equations of scanNoToken
 algorithm
    nextIndex:=Advanced.skipLineComments(string,startIndex);
...
the error text.
 ...
 // algorithms and equations of syntaxError
 algorithm
...
"));
  ...
  // components of skipLineComments
   input String string;
   input Integer startIndex(min=1)=1;
   output Integer nextIndex;
  protected
  ...
  // algorithms and equations of skipLineComments
  algorithm
     nextIndex:=startIndex;
     scanning:=true;
     while scanning loop
