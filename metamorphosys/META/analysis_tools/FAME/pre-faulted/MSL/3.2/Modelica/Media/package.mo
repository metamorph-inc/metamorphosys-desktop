...
"));
   ...
   class SpecificEnthalpyAsFunction
...
<li><a href=\"modelica://Modelica.Media.UsersGuide.Contact\">Contact</a>
 ...
 package Examples
  ...
  // locally defined classes in SimpleLiquidWater
      package Medium = Water.ConstantPropertyLiquidWater(SpecificEnthalpy(max=1e6)) "Medium model";

  // components of SimpleLiquidWater
   parameter SI.Volume V=1 "Volume";
   parameter SI.EnthalpyFlowRate H_flow_ext=1.e6 "Constant enthalpy flow rate into the volume";
  ...
  // algorithms and equations of SimpleLiquidWater
  equation
   medium.p = 1.0e5;
   m = medium.d*V;
   U = m*medium.u;
  ...
  // locally defined classes in IdealGasH2O
      package Medium = IdealGases.SingleGases.H2O "Medium model";

  // components of IdealGasH2O
  ...
  // algorithms and equations of IdealGasH2O
  equation
   state.p = 100000.0;
...
<p>An example for using ideal gas properties and how to compute
  ...
  // locally defined classes in WaterIF97
      package Medium = Water.StandardWater "Medium model";

  // components of WaterIF97
   Medium.BaseProperties medium(p(start=1.e5,stateSelect=StateSelect.prefer),h(start=1.0e5,stateSelect=StateSelect.prefer),T(start=275.0),d(start=999.0));
   Modelica.SIunits.Volume V(start=0.1);
   parameter Modelica.SIunits.VolumeFlowRate dV=0.0 "Fixed time derivative of volume";
   parameter Medium.MassFlowRate m_flow_ext=0 "Fixed mass flow rate into volume";
   parameter Medium.EnthalpyFlowRate H_flow_ext=10000 "Fixed enthalpy flow rate into volume";
  ...
  // algorithms and equations of WaterIF97
  equation
   der(V) = dV;
   m = medium.d*V;
   U = m*medium.u;
  ...
  // locally defined classes in MixtureGases
      package Medium1 = Modelica.Media.IdealGases.MixtureGases.CombustionAir "Medium model";
      package Medium2 = Modelica.Media.IdealGases.MixtureGases.SimpleNaturalGas "Medium model";

  // components of MixtureGases
   parameter Modelica.SIunits.Volume V=1 "Fixed size of volume 1 and volume 2";
   parameter Modelica.SIunits.MassFlowRate m_flow_ext=0.01 "Fixed mass flow rate in to volume 1 and in to volume 2";
   parameter Modelica.SIunits.EnthalpyFlowRate H_flow_ext=5000 "Fixed enthalpy flow rate in to volume and in to volume 2";
   Medium1.BaseProperties medium1(p(start=1.e5,stateSelect=StateSelect.prefer),T(start=300,stateSelect=StateSelect.prefer),X(start={0.8,0.2}));
   ...
   Medium2.BaseProperties medium2(p(start=1.e5,stateSelect=StateSelect.prefer),T(start=300,stateSelect=StateSelect.prefer),X(start={0.1,0.1,0.1,0.2,0.2,0.3}));
  ...
  // algorithms and equations of MixtureGases
  equation
   medium1.X = {0.8,0.2};
   m1 = medium1.d*V;
   U1 = m1*medium1.u;
  ...
  // locally defined classes in MoistAir
      package Medium = Air.MoistAir "Medium model";

  // components of MoistAir
   Medium.BaseProperties medium(T(start=274.0),X(start={0.95,0.05}),p(start=1.0e5));
   parameter Medium.MolarMass MMx[2]={Medium.dryair.MM,Medium.steam.MM} "Vector of molar masses (consisting of dry air and of steam)";
  ...
  // algorithms and equations of MoistAir
  equation
   der(medium.p) = 0.0;
   der(medium.T) = 90;
...
is given to compare the approximation.
  ...
  package TwoPhaseWater
   ...
   // components of ExtendedProperties
   ...
   // algorithms and equations of ExtendedProperties
   equation
    eta = (if phase==1 then dynamicViscosity(state) else 1/(x/(eta_d)+(1-x)/(eta_b)));
    dew = setDewState(sat);
   ...
   model TestTwoPhaseStates
    "Test the above model"
    extends Modelica.Icons.Example;

   // components of TestTwoPhaseStates
    ExtendedProperties medium(p(start=700.0),h(start=8.0e5));
    parameter Real dh=80000.0 "80 kJ/second";
    parameter Real dp=1.0e6 "10 bars per second";

   // algorithms and equations of TestTwoPhaseStates
   equation
    der(medium.p) = dp;
    der(medium.h) = dh;

   annotation(experiment(StopTime=22,NumberOfIntervals=2500),Documentation(info="<html>
...
"));
  ...
  package TestOnly
   ...
   // locally defined classes in MixIdealGasAir
        package Medium = IdealGases.MixtureGases.CombustionAir "Medium model";

   // components of MixIdealGasAir
    Medium.BaseProperties medium(T(start=200.0),X(start={0.2,0.8}),p(start=1.0e5));
    Medium.BaseProperties medium2(T(start=300.0),X(start={0.2,0.8}),p(start=2.0e5));
   ...
   // algorithms and equations of MixIdealGasAir
   equation
    der(medium.p) = 1000.0;
    der(medium.T) = 1000;
    medium.X = {0.2,0.8};
...
is given to compare the approximation.
   ...
   // locally defined classes in FlueGas
        package Medium = IdealGases.MixtureGases.FlueGasLambdaOnePlus "Medium model";

   // components of FlueGas
    Medium.ThermodynamicState state(T(start=200.0),X(start={0.2,0.3,0.4,0.1}),p(start=1.0e5));
    Medium.BaseProperties medium2(T(start=300.0),X(start={0.2,0.1,0.3,0.4}),p(start=2.0e5));
    Medium.SpecificHeatCapacity cp=Medium.specificHeatCapacityCp(state);
    Medium.SpecificHeatCapacity cv=Medium.specificHeatCapacityCv(state);
    ...
    parameter Medium.MolarMass MMx[4]=Medium.data.MM "Molar masses of flue gas";
    Medium.MolarMass MM=1/(sum(state.X[j]/(MMx[j]) for j in 1:4)) "molar mass";
    Real dddX[4]=Medium.density_derX(medium2.state);

   // algorithms and equations of FlueGas
   equation
    der(state.p) = 1000.0;
    der(state.T) = 1000;
...
is given to compare the approximation.
   ...
   package N2AsMix
    ...
    extends IdealGases.Common.MixtureGasNasa(mediumName="Nitrogen",data={IdealGases.Common.SingleGasesData.N2},fluidConstants={IdealGases.Common.FluidData.N2},substanceNames={"Nitrogen"},reference_X={1.0});
   ...
   // locally defined classes in IdealGasN2
        package Medium = IdealGases.SingleGases.N2 "Medium model";

   // components of IdealGasN2
    ...
    parameter Medium.EnthalpyFlowRate H_flow_ext=5000 "Enthalpy flow rate into volume";
    Medium.BaseProperties medium(preferredMediumStates=true,p(start=1.e5),T(start=300));
   ...
   // algorithms and equations of IdealGasN2
   equation
    m = medium.d*V;
    U = m*medium.u;
    der(m) = m_flow_ext;
   ...
   package TestMedia
    ...
    model TemplateMedium
     "Test Interfaces.TemplateMedium"
     extends Modelica.Icons.Example;

    // locally defined classes in TemplateMedium
          package Medium = Interfaces.TemplateMedium "Medium model";

    // components of TemplateMedium
    ...
    // algorithms and equations of TemplateMedium
    equation
     medium.p = 1.0e5;
   ...
   // locally defined classes in IdealGasN2Mix
        package Medium = N2AsMix "Medium model";

   // components of IdealGasN2Mix
    parameter Modelica.SIunits.Volume V=1 "Size of volume";
    parameter Medium.MassFlowRate m_flow_ext=0.01 "Mass flow rate flowing into volume";
    parameter Medium.EnthalpyFlowRate H_flow_ext=5000 "Enthalpy flow rate flowing into volume";
    Medium.BaseProperties medium(preferredMediumStates=true,p(start=1.e5),T(start=300));
    Real m(quantity=Medium.mediumName,start=1.0);
    SI.InternalEnergy U;
    Medium.SpecificHeatCapacity cp=Medium.specificHeatCapacityCp(medium);
   ...
   // algorithms and equations of IdealGasN2Mix
   equation
    m = medium.d*V;
    U = m*medium.u;
    medium.X = {1.0};
  ...
  package Tests
   ...
   package Components
    ...
    // locally defined classes in FluidPort
     replaceable      package Medium = Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(__Dymola_choicesAllMatching=true);
    ...
    annotation(Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Ellipse(extent={{-100,100},{100,-100}},lineColor={0,127,255},fillColor={0,127,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-100,100},{100,-100}},lineColor={0,0,0},fillColor={0,127,255},fillPattern=FillPattern.Solid),Text(extent={{-88,206},{112,112}},textString="%name",lineColor={0,0,255})}),Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Ellipse(extent={{-100,100},{100,-100}},lineColor={0,127,255},fillColor={0,127,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-100,100},{100,-100}},lineColor={0,0,0},fillColor={0,127,255},fillPattern=FillPattern.Solid)}),Documentation(info="<html>Modelica.Media.Examples.Tests.Components.FluidPort_a
</html>"));
    end FluidPort_a;
    ...
    annotation(Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Ellipse(extent={{-100,100},{100,-100}},lineColor={0,127,255},fillColor={0,127,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-100,100},{100,-100}},lineColor={0,0,0},fillColor={0,127,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-80,80},{80,-80}},lineColor={0,127,255},fillColor={255,255,255},fillPattern=FillPattern.Solid),Text(extent={{-88,192},{112,98}},textString="%name",lineColor={0,0,255})}),Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Ellipse(extent={{-100,100},{100,-100}},lineColor={0,127,255},fillColor={0,127,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-100,100},{100,-100}},lineColor={0,0,0},fillColor={0,127,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-80,80},{80,-80}},lineColor={0,127,255},fillColor={255,255,255},fillPattern=FillPattern.Solid)}),Documentation(info="<html>
    ...
    // locally defined classes in PortVolume
     replaceable      package Medium = Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(__Dymola_choicesAllMatching=true);

    // components of PortVolume
     parameter SI.Volume V=1e-6 "Fixed size of junction volume";
     parameter Boolean use_p_start=true "select p_start or d_start" annotation(Evaluate=true,Dialog(group="Initial pressure or initial density"));
     parameter Medium.AbsolutePressure p_start=101325 "Initial pressure" annotation(Dialog(group="Initial pressure or initial density",enable=use_p_start));
     parameter Medium.Density d_start=1 "Initial density" annotation(Dialog(group="Initial pressure or initial density",enable=not use_p_start));
     parameter Boolean use_T_start=true "select T_start or h_start" annotation(Evaluate=true,Dialog(group="Initial temperature or initial specific enthalpy"));
     parameter Medium.Temperature T_start=Modelica.SIunits.Conversions.from_degC(20) "Initial temperature" annotation(Dialog(group="Initial temperature or initial specific enthalpy",enable=use_T_start));
     parameter Medium.SpecificEnthalpy h_start=1.e4 "Initial specific enthalpy" annotation(Dialog(group="Initial temperature or initial specific enthalpy",enable=not use_T_start));
     parameter Medium.MassFraction X_start[Medium.nX] "Initial mass fractions m_i/m" annotation(Dialog(group="Only for multi-substance flow",enable=Medium.nX>0));
     FluidPort_a port(redeclare package Medium = Medium) annotation(Placement(transformation(extent={{-10,-10},{10,10}},rotation=0)));
    ...
    // algorithms and equations of PortVolume
    initial equation
    ...
    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Ellipse(extent={{-100,100},{100,-100}},lineColor={0,0,0},fillPattern=FillPattern.Sphere,fillColor={170,213,255}),Text(extent={{-144,178},{146,116}},textString="%name",lineColor={0,0,255}),Text(extent={{-130,-108},{144,-150}},lineColor={0,0,0},textString="V=%V")}),Documentation(info="<html>
<p>
...
transport.
    ...
    // locally defined classes in FixedMassFlowRate
     replaceable      package Medium = Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(__Dymola_choicesAllMatching=true);

    // components of FixedMassFlowRate
     parameter Medium.MassFlowRate m_flow "Fixed mass flow rate from an infinite reservoir to the fluid port";
     parameter Boolean use_T_ambient=true "select T_ambient or h_ambient" annotation(Evaluate=true,Dialog(group="Ambient temperature or ambient specific enthalpy"));
     parameter Medium.Temperature T_ambient=Modelica.SIunits.Conversions.from_degC(20) "Ambient temperature" annotation(Dialog(group="Ambient temperature or ambient specific enthalpy",enable=use_T_ambient));
     parameter Medium.SpecificEnthalpy h_ambient=1.e4 "Ambient specific enthalpy" annotation(Dialog(group="Ambient temperature or ambient specific enthalpy",enable=not use_T_ambient));
     parameter Medium.MassFraction X_ambient[Medium.nX] "Ambient mass fractions m_i/m of reservoir";
     Medium.BaseProperties medium "Medium in the source";
     FluidPort_b port(redeclare package Medium = Medium) annotation(Placement(transformation(extent={{100,-10},{120,10}},rotation=0)));

    // algorithms and equations of FixedMassFlowRate
    equation
    ...
    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}},grid={2,2}),graphics={Rectangle(extent={{20,60},{100,-60}},lineColor={0,0,0},fillPattern=FillPattern.HorizontalCylinder,fillColor={192,192,192}),Rectangle(extent={{38,40},{100,-40}},lineColor={0,0,0},fillPattern=FillPattern.HorizontalCylinder,fillColor={0,127,255}),Ellipse(extent={{-100,80},{60,-80}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineColor={0,0,255}),Polygon(points={{-60,70},{60,0},{-60,-68},{-60,70}},lineColor={0,0,255},fillColor={0,0,255},fillPattern=FillPattern.Solid),Text(extent={{-54,32},{16,-30}},lineColor={255,0,0},textString="m"),Text(extent={{-142,142},{156,88}},textString="%name",lineColor={0,0,255}),Text(extent={{-154,-88},{150,-132}},lineColor={0,0,0},textString="%m_flow"),Ellipse(extent={{-26,30},{-18,22}},lineColor={255,0,0},fillColor={255,0,0},fillPattern=FillPattern.Solid)}),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}},grid={2,2}),graphics),Documentation(info="<html>
    ...
    // locally defined classes in FixedAmbient
     replaceable      package Medium = Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(__Dymola_choicesAllMatching=true);

    // components of FixedAmbient
     parameter Boolean use_p_ambient=true "select p_ambient or d_ambient" annotation(Evaluate=true,Dialog(group="Ambient pressure or ambient density"));
     parameter Medium.AbsolutePressure p_ambient=101325 "Ambient pressure" annotation(Dialog(group="Ambient pressure or ambient density",enable=use_p_ambient));
     parameter Medium.Density d_ambient=1 "Ambient density" annotation(Dialog(group="Ambient pressure or ambient density",enable=not use_p_ambient));
     parameter Boolean use_T_ambient=true "select T_ambient or h_ambient" annotation(Evaluate=true,Dialog(group="Ambient temperature or ambient specific enthalpy"));
     parameter Medium.Temperature T_ambient=Modelica.SIunits.Conversions.from_degC(20) "Ambient temperature" annotation(Dialog(group="Ambient temperature or ambient specific enthalpy",enable=use_T_ambient));
     parameter Medium.SpecificEnthalpy h_ambient=1.e4 "Ambient specific enthalpy" annotation(Dialog(group="Ambient temperature or ambient specific enthalpy",enable=not use_T_ambient));
     parameter Medium.MassFraction X_ambient[Medium.nX] "Ambient mass fractions m_i/m" annotation(Dialog(group="Only for multi-substance flow",enable=Medium.nX>0));
     Medium.BaseProperties medium "Medium in the source";
     FluidPort_b port(redeclare package Medium = Medium) annotation(Placement(transformation(extent={{100,-10},{120,10}},rotation=0)));

    // algorithms and equations of FixedAmbient
    equation
    ...
    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}},grid={2,2}),graphics={Ellipse(extent={{-100,80},{100,-80}},lineColor={0,0,0},fillPattern=FillPattern.Sphere,fillColor={0,127,255}),Text(extent={{-136,144},{132,82}},textString="%name",lineColor={0,0,255})}),Documentation(info="<html>
<p>
...
with exception of ambient pressure, do not have an effect.
    ...
    // locally defined classes in ShortPipe
     replaceable      package Medium = Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(__Dymola_choicesAllMatching=true);
          model _famefaults_port_b = FAME.Dampers.FluidHeatWithoutConnectEquations;
          model _famefaults_port_a = FAME.Dampers.FluidHeatWithoutConnectEquations;

    // components of ShortPipe
     ...
     Modelica.Media.Examples.Tests.Components.FluidPort_b port_b(redeclare package Medium = Medium) annotation(Placement(transformation(extent={{120,-10},{100,10}},rotation=0)));
     FAME.Dampers.FluidHeatWithoutConnectEquations _famefault_port_b(amount=0.0,port_b(redeclare package Medium = Medium),port_a(redeclare package Medium = Medium),medium=port_b.medium);
     Modelica.SIunits.Pressure dp "Pressure drop from port_a to port_b";
     Modelica.Media.Examples.Tests.Components.FluidPort_a port_a(redeclare package Medium = Medium) annotation(Placement(transformation(extent={{-120,-10},{-100,10}},rotation=0)));
     FAME.Dampers.FluidHeatWithoutConnectEquations _famefault_port_a(amount=0.0,port_b(redeclare package Medium = Medium),port_a(redeclare package Medium = Medium),medium=port_a.medium);

    // algorithms and equations of ShortPipe
    equation
     _famefault_port_a.port_b.H_flow = semiLinear(_famefault_port_a.port_b.m_flow,_famefault_port_a.port_b.h,_famefault_port_b.port_b.h);
     _famefault_port_a.port_b.mXi_flow = semiLinear(_famefault_port_a.port_b.m_flow,_famefault_port_a.port_b.Xi,_famefault_port_b.port_b.Xi);
     _famefault_port_a.port_b.H_flow+_famefault_port_b.port_b.H_flow = 0;
     _famefault_port_a.port_b.m_flow+_famefault_port_b.port_b.m_flow = 0;
     _famefault_port_a.port_b.mXi_flow+_famefault_port_b.port_b.mXi_flow = zeros(Medium.nXi);
     m_flow = _famefault_port_a.port_b.m_flow;
     dp = _famefault_port_a.port_b.p-_famefault_port_b.port_b.p;
     m_flow = m_flow_nominal/(dp_nominal)*dp;
     connect(port_b,_famefault_port_b.port_a);
     connect(port_a,_famefault_port_a.port_a);

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-100,60},{100,-60}},lineColor={0,0,0},fillPattern=FillPattern.HorizontalCylinder,fillColor={192,192,192}),Rectangle(extent={{-100,34},{100,-36}},lineColor={0,0,0},fillPattern=FillPattern.HorizontalCylinder,fillColor={0,127,255}),Text(extent={{-150,140},{150,80}},lineColor={0,0,0},textString="%name"),Text(extent={{-136,-62},{122,-108}},lineColor={0,0,0},textString="k=%m_flow_nominal/%dp_nominal")}),Documentation(info="<html>
<p>
...
no mass or energy is stored in the pipe.
    ...
    // locally defined classes in PartialTestModel
     replaceable      package Medium = Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(__Dymola_choicesAllMatching=true);

    // components of PartialTestModel
     ...
     PortVolume volume(redeclare package Medium = Medium,p_start=p_start,T_start=T_start,h_start=h_start,X_start=X_start,V=0.1) annotation(Placement(transformation(extent={{-40,0},{-20,20}},rotation=0)));
     FixedMassFlowRate fixedMassFlowRate(redeclare package Medium = Medium,T_ambient=1.2*T_start,h_ambient=1.2*h_start,m_flow=1,X_ambient=0.5*X_start) annotation(Placement(transformation(extent={{-80,0},{-60,20}},rotation=0)));
     FixedAmbient ambient(redeclare package Medium = Medium,T_ambient=T_start,h_ambient=h_start,X_ambient=X_start,p_ambient=p_start) annotation(Placement(transformation(extent={{60,0},{40,20}},rotation=0)));
     ShortPipe shortPipe(redeclare package Medium = Medium,m_flow_nominal=1,dp_nominal=0.1e5) annotation(Placement(transformation(extent={{0,0},{20,20}},rotation=0)));

    // algorithms and equations of PartialTestModel
    equation
    ...
    // locally defined classes in PartialTestModel2
     replaceable      package Medium = Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(__Dymola_choicesAllMatching=true);

    // components of PartialTestModel2
     ...
     parameter SI.SpecificEnthalpy h_start=1 "Initial value of specific enthalpy";
     parameter Real X_start[Medium.nX]=Medium.reference_X "Initial value of mass fractions";
     PortVolume volume(redeclare package Medium = Medium,p_start=p_start,T_start=T_start,h_start=h_start,X_start=X_start,V=0.1) annotation(Placement(transformation(extent={{-60,0},{-40,20}},rotation=0)));
     FixedMassFlowRate fixedMassFlowRate(redeclare package Medium = Medium,T_ambient=1.2*T_start,h_ambient=1.2*h_start,m_flow=1,X_ambient=0.5*X_start) annotation(Placement(transformation(extent={{-100,0},{-80,20}},rotation=0)));
     FixedAmbient ambient(redeclare package Medium = Medium,T_ambient=T_start,h_ambient=h_start,X_ambient=X_start,p_ambient=p_start) annotation(Placement(transformation(extent={{92,0},{72,20}},rotation=0)));
     ShortPipe shortPipe(redeclare package Medium = Medium,m_flow_nominal=1,dp_nominal=0.1e5) annotation(Placement(transformation(extent={{-30,0},{-10,20}},rotation=0)));
     PortVolume volume1(redeclare package Medium = Medium,p_start=p_start,T_start=T_start,h_start=h_start,X_start=X_start,V=0.1) annotation(Placement(transformation(extent={{0,0},{20,20}},rotation=0)));
     ShortPipe shortPipe1(redeclare package Medium = Medium,m_flow_nominal=1,dp_nominal=0.1e5) annotation(Placement(transformation(extent={{36,0},{56,20}},rotation=0)));

    // algorithms and equations of PartialTestModel2
    equation
     connect(fixedMassFlowRate.port,volume.port) annotation(Line(points={{-79,10},{-50,10}},color={0,127,255}));
     connect(volume.port,shortPipe.port_a) annotation(Line(points={{-50,10},{-31,10}},color={0,127,255}));
     connect(volume1.port,shortPipe1.port_a) annotation(Line(points={{10,10},{35,10}},color={0,127,255}));
     connect(shortPipe.port_b,volume1.port) annotation(Line(points={{-9,10},{10,10}},color={0,127,255}));
     connect(shortPipe1.port_b,ambient.port) annotation(Line(points={{57,10},{71,10}},color={0,127,255}));
   ...
   package MediaTestModels
    ...
    package Air
     ...
     model SimpleAir
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Air.SimpleAir);
     ...
     model DryAirNasa
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Air.DryAirNasa);
     ...
     model MoistAir
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Air.MoistAir);
    ...
    package IdealGases
     ...
     model Nitrogen
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.IdealGases.SingleGases.N2);
     ...
     model SimpleNaturalGas
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.IdealGases.MixtureGases.SimpleNaturalGas);
     ...
     model SimpleNaturalGasFixedComposition
      "Test mixture gas Modelica.Media.IdealGases.MixtureGases.SimpleNaturalGas"
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.IdealGases.MixtureGases.SimpleNaturalGasFixedComposition);
    ...
    package Incompressible
     ...
     model Glycol47
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Incompressible.Examples.Glycol47(final singleState=true,final enthalpyOfT=true));
     ...
     model Essotherm650
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Incompressible.Examples.Essotherm650);
    ...
    package Water
     ...
     model ConstantPropertyLiquidWater
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Water.ConstantPropertyLiquidWater);
     ...
     model IdealSteam
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Water.IdealSteam);
     ...
     model WaterIF97OnePhase_ph
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Water.WaterIF97OnePhase_ph,fixedMassFlowRate(use_T_ambient=false,h_ambient=363755),ambient(use_T_ambient=false,h_ambient=112570));
     ...
     model WaterIF97_pT
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Water.WaterIF97_pT);
     ...
     model WaterIF97_ph
      ...
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.Water.WaterIF97_ph,ambient(use_T_ambient=false,h_ambient=112570),fixedMassFlowRate(use_T_ambient=false,h_ambient=363755));
    ...
    package LinearFluid
     ...
     model LinearColdWater
      "Test Modelica.Media.Incompressible.Examples.Glycol47"
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.CompressibleLiquids.LinearColdWater);
     ...
     model LinearWater_pT
      "Test Modelica.Media.Incompressible.Examples.Essotherm65"
      extends Modelica.Icons.Example;
      extends Modelica.Media.Examples.Tests.Components.PartialTestModel(redeclare package Medium = Modelica.Media.CompressibleLiquids.LinearWater_pT_Ambient);
  ...
  package SolveOneNonlinearEquation
   ...
   // locally defined classes in Inverse_sine
    ...
    encapsulated package Inverse_sine_definition
     ...
     // components of f_nonlinear_Data
      Real A;
      Real w;
     ...
     // algorithms and equations of f_nonlinear
     algorithm
   ...
   // components of Inverse_sine
    ...
    parameter Real A=1;
    parameter Real w=1;
    parameter Inverse_sine_definition.f_nonlinear_Data data=Inverse_sine_definition.f_nonlinear_Data(A=A,w=w);
    Real x_zero "y_zero = A*sin(w*x_zero)";

   // algorithms and equations of Inverse_sine
   equation
...
output window.
   ...
   // locally defined classes in Inverse_sh_T
    replaceable     package Medium = Modelica.Media.Air.DryAirNasa constrainedby Modelica.Media.IdealGases.Common.SingleGasNasa;

   // components of Inverse_sh_T
    parameter SI.Temperature T_min=300 "Vary temperature linearly from T_min (time=0) upto T_max (time=1)";
    parameter SI.Temperature T_max=500 "Vary temperature linearly from T_min (time=0) upto T_max (time=1)";
    parameter SI.Pressure p=1.0e5 "Fixed pressure in model";
    final parameter SI.SpecificEnthalpy h_min=Medium.h_T(Medium.data,T_min) "Specific enthalpy at T_min";
    final parameter SI.SpecificEnthalpy h_max=Medium.h_T(Medium.data,T_max) "Specific enthalpy at T_max";
    final parameter SI.SpecificEntropy s_min=Medium.specificEntropy(Medium.setState_pT(p,T_min)) "Specific entropy at T_min";
    final parameter SI.SpecificEntropy s_max=Medium.specificEntropy(Medium.setState_pT(p,T_max)) "Specific entropy at T_max";
   ...
   // algorithms and equations of Inverse_sh_T
   equation
   ...
   // locally defined classes in InverseIncompressible_sh_T
    replaceable     package Medium = Modelica.Media.Incompressible.Examples.Glycol47 "Medium model" annotation(__Dymola_choicesAllMatching=true);

   // components of InverseIncompressible_sh_T
    parameter SI.Temperature T_min=Medium.T_min "Vary temperature linearly from T_min (time=0) upto T_max (time=1)";
    parameter SI.Temperature T_max=Medium.T_max "Vary temperature linearly from T_min (time=0) upto T_max (time=1)";
    parameter SI.Pressure p=1.0e5 "Fixed pressure in model";
    final parameter SI.SpecificEnthalpy h_min=Medium.h_T(Medium.T_min) "Specific enthalpy at T_min";
    final parameter SI.SpecificEnthalpy h_max=Medium.h_T(Medium.T_max) "Specific enthalpy at T_max";
    final parameter SI.SpecificEntropy s_min=Medium.specificEntropy(Medium.setState_pT(p,T_min)) "Specific entropy at T_min";
    final parameter SI.SpecificEntropy s_max=Medium.specificEntropy(Medium.setState_pT(p,T_max)) "Specific entropy at T_max";
    SI.SpecificEnthalpy h1 "Pre-defined specific enthalpy";
    SI.SpecificEnthalpy h2 "Specific enthalpy computed from T (= h1 required)";
   ...
   // algorithms and equations of InverseIncompressible_sh_T
   equation
    h1 = (if time<0 then h_min else (if time>1 then h_max else h_min+time/(timeUnit)*(h_max-h_min)));
    s1 = (if time<0 then s_min else (if time>1 then s_max else s_min+time/(timeUnit)*(s_max-s_min)));
    Th = Medium.temperature_phX(p,h1,fill(0.0,0));
   ...
   // locally defined classes in Inverse_sh_TX
    replaceable     package Medium = Modelica.Media.IdealGases.MixtureGases.FlueGasLambdaOnePlus constrainedby Modelica.Media.IdealGases.Common.MixtureGasNasa;

   // components of Inverse_sh_TX
    parameter SI.Temperature T_min=300 "Vary temperature linearly from T_min (time=0) upto T_max (time=1)";
    parameter SI.Temperature T_max=500 "Vary temperature linearly from T_min (time=0) upto T_max (time=1)";
    parameter SI.Pressure p=1.0e5 "Fixed pressure in model";
    ...
    SI.SpecificEntropy s_min=Medium.specificEntropy(Medium.setState_pTX(p,T_min,Medium.reference_X)) "Specific entropy at T_min";
    SI.SpecificEntropy s_max=Medium.specificEntropy(Medium.setState_pTX(p,T_max,Medium.reference_X)) "Specific entropy at T_max";
    SI.SpecificEnthalpy h1 "Pre-defined specific enthalpy";
    SI.SpecificEnthalpy h2 "Specific enthalpy computed from T (= h1 required)";
    SI.SpecificEntropy s1 "Pre-defined specific entropy";
    SI.SpecificEntropy s2 "Specific entropy computed from T (= h1 required)";
    SI.Temperature Th "Temperature computed from h1";
    SI.Temperature Ts "Temperature computed from s1";
    SI.MassFraction X[4] "mass fraction vector";
   protected
    constant SI.Time timeUnit=1.0;

   // algorithms and equations of Inverse_sh_TX
   equation
...
"));
 ...
 package Interfaces
  ...
  partial package TemplateMedium
   "Template for media models"
   extends Modelica.Media.Interfaces.PartialMedium(final mediumName="NameOfMedium",final substanceNames={mediumName},final singleState=false,final reducedX=true,final fixedX=true,Temperature(min=273,max=450,start=323));

   redeclare model
    extends BaseProperties(final standardOrderComponents=true)
    "Base properties of medium"

   // algorithms and equations of BaseProperties
   equation
    d = 1;
    h = cp_const*T;
    u = h-p/(d);
   ...
   // components of ThermodynamicState
    AbsolutePressure p "Absolute pressure of medium";
    Temperature T "Temperature of medium";
   ...
   // algorithms and equations of dynamicViscosity
   algorithm
      eta:=10-state.T*0.3+state.p*0.2;
   ...
   // algorithms and equations of thermalConductivity
   algorithm
      lambda:=0;
   ...
   // algorithms and equations of specificEntropy
   algorithm
      s:=0;
   ...
   // algorithms and equations of specificHeatCapacityCp
   algorithm
      cp:=0;
   ...
   // algorithms and equations of specificHeatCapacityCv
   algorithm
      cv:=0;
   ...
   // algorithms and equations of isentropicExponent
   algorithm
      gamma:=1;
   ...
   // algorithms and equations of velocityOfSound
   algorithm
      a:=0;
  ...
  // components of TemplateMedium
   constant SpecificHeatCapacity cp_const=123456 "Constant specific heat capacity at constant pressure";
...
Modelica source.
  ...
  partial package PartialMedium
   ...
   // locally defined classes in BaseProperties
...
<pre>
   ...
   // components of dynamicViscosity
   ...
   // components of thermalConductivity
    input ThermodynamicState state "thermodynamic state record";
    output ThermalConductivity lambda "Thermal conductivity";
   end thermalConductivity;

   replaceable function prandtlNumber
   ...
   // components of specificEnthalpy
    input ThermodynamicState state "thermodynamic state record";
   ...
   // components of specificInternalEnergy
    input ThermodynamicState state "thermodynamic state record";
   ...
   // components of specificGibbsEnergy
    input ThermodynamicState state "thermodynamic state record";
   ...
   // components of specificHelmholtzEnergy
    input ThermodynamicState state "thermodynamic state record";
   ...
   // components of specificHeatCapacityCp
    input ThermodynamicState state "thermodynamic state record";
   ...
   // components of specificHeatCapacityCv
    input ThermodynamicState state "thermodynamic state record";
   ...
   // components of isentropicExponent
    input ThermodynamicState state "thermodynamic state record";
   ...
   // components of isentropicEnthalpy
...
<li> The enhalpy at another state (h_is) shall be computed
   ...
   // components of isobaricExpansionCoefficient
    input ThermodynamicState state "thermodynamic state record";
...
beta is defined as  1/v * der(v,T), with v = 1/d, at constant pressure p.
   ...
   // components of isothermalCompressibility
    input ThermodynamicState state "thermodynamic state record";
...
kappa is defined as - 1/v * der(v,p), with v = 1/d at constant temperature T.
   ...
   // components of specificEnthalpy_pTX
    input AbsolutePressure p "Pressure";
    input Temperature T "Temperature";
    input MassFraction X[:]=reference_X "Mass fractions";
    output SpecificEnthalpy h "Specific enthalpy";

   // algorithms and equations of specificEnthalpy_pTX
   algorithm
   ...
   // components of specificEntropy_pTX
    input AbsolutePressure p "Pressure";
    input Temperature T "Temperature";
    input MassFraction X[:]=reference_X "Mass fractions";
    output SpecificEntropy s "Specific entropy";

   // algorithms and equations of specificEntropy_pTX
   algorithm
   ...
   // components of density_pTX
    input AbsolutePressure p "Pressure";
    input Temperature T "Temperature";
    input MassFraction X[:] "Mass fractions";
    output Density d "Density";

   // algorithms and equations of density_pTX
   algorithm
   ...
   // components of temperature_phX
    input AbsolutePressure p "Pressure";
    input SpecificEnthalpy h "Specific enthalpy";
    input MassFraction X[:]=reference_X "Mass fractions";
    output Temperature T "Temperature";

   // algorithms and equations of temperature_phX
   algorithm
   ...
   // components of density_phX
    input AbsolutePressure p "Pressure";
    input SpecificEnthalpy h "Specific enthalpy";
    input MassFraction X[:]=reference_X "Mass fractions";
    output Density d "Density";

   // algorithms and equations of density_phX
   algorithm
   ...
   // components of temperature_psX
    input AbsolutePressure p "Pressure";
    input SpecificEntropy s "Specific entropy";
    input MassFraction X[:]=reference_X "Mass fractions";
    output Temperature T "Temperature";

   // algorithms and equations of temperature_psX
   algorithm
   ...
   // components of density_psX
    input AbsolutePressure p "Pressure";
    input SpecificEntropy s "Specific entropy";
    input MassFraction X[:]=reference_X "Mass fractions";
    output Density d "Density";

   // algorithms and equations of density_psX
   algorithm
   ...
   // components of specificEnthalpy_psX
    input AbsolutePressure p "Pressure";
    input SpecificEntropy s "Specific entropy";
    input MassFraction X[:]=reference_X "Mass fractions";
    output SpecificEnthalpy h "Specific enthalpy";

   // algorithms and equations of specificEnthalpy_psX
   algorithm
      ...
      type Temperature = SI.Temperature(min=1,max=1.e4,nominal=300,start=300) "Type for temperature with medium specific attributes";
      ...
      type PrandtlNumber = SI.PrandtlNumber(min=1e-3,max=1e5,nominal=1.0) "Type for Prandtl number with medium specific attributes";
      ...
      type CumulativeExtraProperty = Real(min=0.0,start=1.0) "Type for conserved integral of unspecified, mass specific property";
      ...
      type DerDensityByPressure = SI.DerDensityByPressure "Type for partial derivative of density with resect to pressure with medium specific attributes";
      type DerDensityByEnthalpy = SI.DerDensityByEnthalpy "Type for partial derivative of density with resect to enthalpy with medium specific attributes";
      type DerEnthalpyByPressure = SI.DerEnthalpyByPressure "Type for partial derivative of enthalpy with resect to pressure with medium specific attributes";
      type DerDensityByTemperature = SI.DerDensityByTemperature "Type for partial derivative of density with resect to temperature with medium specific attributes";

   package Choices
    "Types, constants to define menu choices"

    type IndependentVariables = enumeration(T, pT, ph, phX, pTX, dTX)     "Enumeration defining the independent variables of a medium"
;

    type Init = enumeration(NoInit, InitialStates, SteadyState, SteadyMass)     "Enumeration defining initialization for fluid flow"
 annotation(Evaluate=true);

    type ReferenceEnthalpy = enumeration(ZeroAt0K, ZeroAt25C, UserDefined)     "Enumeration defining the reference enthalpy of a medium"
 annotation(Evaluate=true);

    type ReferenceEntropy = enumeration(ZeroAt0K, ZeroAt0C, UserDefined)     "Enumeration defining the reference entropy of a medium"
 annotation(Evaluate=true);

    type pd = enumeration(default, p_known, d_known)     "Enumeration defining whether p or d are known for the boundary condition"
 annotation(Evaluate=true);

    type Th = enumeration(default, T_known, h_known)     "Enumeration defining whether T or h are known as boundary condition"
 annotation(Evaluate=true);
...
Note: Reference enthalpy might have to be extended with enthalpy of formation.
  ...
  // components of PartialMedium
   constant Modelica.Media.Interfaces.PartialMedium.Choices.IndependentVariables ThermoStates "Enumeration type for independent variables";
...
",revisions="<html>
  ...
  partial package PartialPureSubstance
   ...
   // components of setState_pT
    input AbsolutePressure p "Pressure";
    input Temperature T "Temperature";
    output ThermodynamicState state "thermodynamic state record";

   // algorithms and equations of setState_pT
   algorithm
   ...
   // components of setState_ph
    input AbsolutePressure p "Pressure";
    input SpecificEnthalpy h "Specific enthalpy";
    output ThermodynamicState state "thermodynamic state record";

   // algorithms and equations of setState_ph
   algorithm
   ...
   // components of setState_ps
    input AbsolutePressure p "Pressure";
    input SpecificEntropy s "Specific entropy";
    output ThermodynamicState state "thermodynamic state record";

   // algorithms and equations of setState_ps
   algorithm
   ...
   // components of setState_dT
    input Density d "density";
    input Temperature T "Temperature";
    output ThermodynamicState state "thermodynamic state record";

   // algorithms and equations of setState_dT
   algorithm
   ...
   // components of density_ph
    input AbsolutePressure p "Pressure";
    input SpecificEnthalpy h "Specific enthalpy";
    output Density d "Density";

   // algorithms and equations of density_ph
   algorithm
   ...
   // components of temperature_ph
    input AbsolutePressure p "Pressure";
    input SpecificEnthalpy h "Specific enthalpy";
    output Temperature T "Temperature";

   // algorithms and equations of temperature_ph
   algorithm
   ...
   // components of pressure_dT
    input Density d "Density";
    input Temperature T "Temperature";
    output AbsolutePressure p "Pressure";

   // algorithms and equations of pressure_dT
   algorithm
   ...
   // components of specificEnthalpy_dT
    input Density d "Density";
    input Temperature T "Temperature";
    output SpecificEnthalpy h "specific enthalpy";

   // algorithms and equations of specificEnthalpy_dT
   algorithm
   ...
   // components of specificEnthalpy_ps
    input AbsolutePressure p "Pressure";
    input SpecificEntropy s "Specific entropy";
    output SpecificEnthalpy h "specific enthalpy";

   // algorithms and equations of specificEnthalpy_ps
   algorithm
   ...
   // components of temperature_ps
    input AbsolutePressure p "Pressure";
    input SpecificEntropy s "Specific entropy";
    output Temperature T "Temperature";

   // algorithms and equations of temperature_ps
   algorithm
   ...
   // components of density_ps
    input AbsolutePressure p "Pressure";
    input SpecificEntropy s "Specific entropy";
    output Density d "Density";

   // algorithms and equations of density_ps
   algorithm
   ...
   // components of specificEnthalpy_pT
    input AbsolutePressure p "Pressure";
    input Temperature T "Temperature";
    output SpecificEnthalpy h "specific enthalpy";

   // algorithms and equations of specificEnthalpy_pT
   algorithm
   ...
   // components of density_pT
    input AbsolutePressure p "Pressure";
    input Temperature T "Temperature";
    output Density d "Density";

   // algorithms and equations of density_pT
   algorithm
      d:=density(setState_pTX(p,T,fill(0,0)));
   end density_pT;

   redeclare replaceable partial model
    extends BaseProperties(final standardOrderComponents=true)
   end BaseProperties;
  ...
  partial package PartialLinearFluid
   ...
   extends Interfaces.PartialPureSubstance(ThermoStates=Choices.IndependentVariables.pTX,singleState=false);
   ...
   redeclare model
    extends BaseProperties(T(stateSelect=(if preferredMediumStates then StateSelect.prefer else StateSelect.default)),p(stateSelect=(if preferredMediumStates then StateSelect.prefer else StateSelect.default)))
    "Base properties of medium"
   ...
   // algorithms and equations of setState_pTX
   algorithm
   ...
   // algorithms and equations of setState_phX
   algorithm
   ...
   // algorithms and equations of setState_psX
   algorithm
      state:=ThermodynamicState(p=p,T=reference_T*cp_const/(cp_const-s+reference_s+(p-reference_p)*(-beta_const)/(reference_d)));
   end setState_psX;
   ...
   // algorithms and equations of setState_dTX
   algorithm
      state:=ThermodynamicState(p=(d-reference_d+(state.T-reference_T)*beta_const*reference_d)/(reference_d*kappa_const)+reference_p,T=T);
   end setState_dTX;
   ...
   // algorithms and equations of setSmoothState
   algorithm
   ...
   // algorithms and equations of pressure
   algorithm
      p:=state.p;
   end pressure;
   ...
   // algorithms and equations of temperature
   algorithm
      T:=state.T;
   end temperature;
   ...
   // algorithms and equations of density
   algorithm
   ...
   // algorithms and equations of specificEnthalpy
   algorithm
   ...
   // algorithms and equations of specificInternalEnergy
   algorithm
   ...
   // algorithms and equations of specificGibbsEnergy
   algorithm
   ...
   // algorithms and equations of specificHelmholtzEnergy
   algorithm
   ...
   // algorithms and equations of isentropicEnthalpy
   algorithm
      h_is:=specificEnthalpy(setState_pTX(p_downstream,reference_T/(1-((refState.T-reference_T)/(refState.T)+(refState.p-p_downstream)*(-beta_const)/(reference_d*cp_const))),reference_X));
...
one, which would require a numeric solution.
   ...
   // algorithms and equations of specificHeatCapacityCv
   algorithm
      cv:=(if constantJacobian then cp_const-reference_T*beta_const*beta_const/(kappa_const*reference_d) else state.T*beta_const*beta_const/(kappa_const*reference_d));
   end specificHeatCapacityCv;
   ...
   // algorithms and equations of isothermalCompressibility
   algorithm
      kappa:=kappa_const;
   end isothermalCompressibility;
   ...
   // algorithms and equations of isobaricExpansionCoefficient
   algorithm
      beta:=beta_const;
   end isobaricExpansionCoefficient;
   ...
   // algorithms and equations of density_derp_h
   algorithm
      ddph:=(if constantJacobian then kappa_const*reference_d+beta_const*(1-reference_T*beta_const)/(cp_const) else kappa_const*density(state)+beta_const*(1-temperature(state)*beta_const)/(cp_const));
   end density_derp_h;
   ...
   // algorithms and equations of density_derh_p
   algorithm
      ddhp:=(if constantJacobian then (-beta_const)*reference_d/(cp_const) else (-beta_const)*density(state)/(cp_const));
   end density_derh_p;
   ...
   // algorithms and equations of density_derp_T
   algorithm
      ddpT:=(if constantJacobian then kappa_const*reference_d else kappa_const*density(state));
   end density_derp_T;
   ...
   // algorithms and equations of density_derT_p
   algorithm
      ddTp:=(if constantJacobian then (-beta_const)*reference_d else (-beta_const)*density(state));
   end density_derT_p;
   ...
   // algorithms and equations of density_derX
   algorithm
      dddX:=fill(0,nX);
   end density_derX;
   ...
   // algorithms and equations of molarMass
   algorithm
      MM:=MM_const;
   end molarMass;
   ...
   // algorithms and equations of T_ph
   algorithm
      T:=(h-reference_h-(p-reference_p)*(1-beta_const*reference_T)/(reference_d))/(cp_const)+reference_T;
   end T_ph;
...
<dd>Francesco Casella<br>
  ...
  partial package PartialMixtureMedium
   ...
   // components of moleToMassFractions
   ...
   // algorithms and equations of moleToMassFractions
   algorithm
   ...
   // components of massToMoleFractions
   ...
   // algorithms and equations of massToMoleFractions
   algorithm
  ...
  // components of PartialMixtureMedium
  ...
  partial package PartialCondensingGases
   ...
   // components of saturationPressure
   ...
   // components of enthalpyOfVaporization
    input Temperature T "temperature";
   ...
   // components of enthalpyOfLiquid
    input Temperature T "temperature";
   ...
   // components of enthalpyOfCondensingGas
    input Temperature T "temperature";
    output SpecificEnthalpy h "specific enthalpy";
   ...
   // components of enthalpyOfNonCondensingGas
    input Temperature T "temperature";
    output SpecificEnthalpy h "specific enthalpy";
  ...
  partial package PartialTwoPhaseMedium
   ...
   // components of SaturationProperties
   ...
   // components of setBubbleState
    input SaturationProperties sat "saturation point";
    input FixedPhase phase(min=1,max=2)=1 "phase: default is one phase";
    output ThermodynamicState state "complete thermodynamic state info";
   end setBubbleState;

   redeclare replaceable partial function
    extends setState_dTX
    "Return thermodynamic state as function of d, T and composition X or Xi"
   ...
   // components of setSat_T
    input Temperature T "temperature";
    output SaturationProperties sat "saturation property record";

   // algorithms and equations of setSat_T
   algorithm
      sat.Tsat:=T;
   ...
   // components of setSat_p
    input AbsolutePressure p "pressure";
    output SaturationProperties sat "saturation property record";

   // algorithms and equations of setSat_p
   algorithm
      sat.psat:=p;
   ...
   // components of saturationTemperature
    input AbsolutePressure p "pressure";
   ...
   // components of saturationPressure_sat
    input SaturationProperties sat "saturation property record";
    output AbsolutePressure p "saturation pressure";

   // algorithms and equations of saturationPressure_sat
   algorithm
      p:=sat.psat;
   ...
   // components of saturationTemperature_sat
    input SaturationProperties sat "saturation property record";
    output Temperature T "saturation temperature";

   // algorithms and equations of saturationTemperature_sat
   algorithm
      T:=sat.Tsat;
   ...
   // components of saturationTemperature_derp
    input AbsolutePressure p "pressure";
   ...
   // components of saturationTemperature_derp_sat
    input SaturationProperties sat "saturation property record";
    output Real dTp "derivative of saturation temperature w.r.t. pressure";

   // algorithms and equations of saturationTemperature_derp_sat
   algorithm
   ...
   // components of dBubbleDensity_dPressure
    input SaturationProperties sat "saturation property record";
   ...
   // components of dDewDensity_dPressure
    input SaturationProperties sat "saturation property record";
   ...
   // components of dBubbleEnthalpy_dPressure
    input SaturationProperties sat "saturation property record";
   ...
   // components of dDewEnthalpy_dPressure
    input SaturationProperties sat "saturation property record";
   ...
   // components of setState_px
    input AbsolutePressure p "Pressure";
    input MassFraction x "Vapour quality";
    output ThermodynamicState state "Thermodynamic state record";

   // algorithms and equations of setState_px
   algorithm
   ...
   // components of setState_Tx
    input Temperature T "Temperature";
    input MassFraction x "Vapour quality";
    output ThermodynamicState state "thermodynamic state record";

   // algorithms and equations of setState_Tx
   algorithm
  ...
  // components of PartialTwoPhaseMedium
...
This function computes the specific internal energy of the fluid, but neglects the (small) influence of the pressure term p/d.
  ...
  // components of PartialSimpleMedium
  ...
  partial package PartialSimpleIdealGasMedium
   ...
   extends Interfaces.PartialPureSubstance(ThermoStates=Choices.IndependentVariables.pT,final singleState=false);
...
quantities are assumed to be constant.
  ...
  // components of PartialSimpleIdealGasMedium
   constant SpecificHeatCapacity cp_const "Constant specific heat capacity at constant pressure";
...
kind of media.
 ...
 package Common
  ...
  // components of SaturationBoundaryProperties
  ...
  // components of IF97BaseTwoPhase
  ...
  // components of IF97PhaseBoundaryProperties
  ...
  // components of TwoPhaseTransportProps
  ...
  // components of PhaseBoundaryProperties
   SI.Density d "density";
   SI.SpecificEnthalpy h "specific enthalpy";
  ...
  // components of NewtonDerivatives_ph
   SI.Pressure p "pressure";
   SI.SpecificEnthalpy h "specific enthalpy";
   DerPressureByDensity pd "derivative of pressure w.r.t. density";
   DerPressureByTemperature pt "derivative of pressure w.r.t. temperature";
  ...
  // components of NewtonDerivatives_ps
   SI.Pressure p "pressure";
   SI.SpecificEntropy s "specific entropy";
   DerPressureByDensity pd "derivative of pressure w.r.t. density";
   DerPressureByTemperature pt "derivative of pressure w.r.t. temperature";
  ...
  // components of NewtonDerivatives_pT
   SI.Pressure p "pressure";
   DerPressureByDensity pd "derivative of pressure w.r.t. density";
  ...
  // components of ExtraDerivatives
...
derOfsByTAtConstantv =  bt.dsv/bt.dTv;
  ...
  // components of gibbsToBridgmansTables
   ...
   output IsobaricVolumeExpansionCoefficient alpha "isobaric volume expansion coefficient";
  ...
  // algorithms and equations of gibbsToBridgmansTables
  algorithm
  ...
  // components of helmholtzToBridgmansTables
  ...
  protected
   DerPressureByTemperature pt "derivative of pressure w.r.t. temperature";
   DerPressureBySpecificVolume pv "derivative of pressure w.r.t. specific volume ";
   SI.SpecificHeatCapacity cv "isochoric specific heat capacity";

  // algorithms and equations of helmholtzToBridgmansTables
  algorithm
  ...
  // components of gibbsToBoundaryProps
   input GibbsDerivs g "dimensionless derivatives of Gibbs function";
  ...
  // algorithms and equations of gibbsToBoundaryProps
  algorithm
  ...
  // components of helmholtzToBoundaryProps
   input HelmholtzDerivs f "dimensionless derivatives of Helmholtz function";
   output PhaseBoundaryProperties sat "phase boundary property record";
  protected
   SI.Pressure p "pressure";

  // algorithms and equations of helmholtzToBoundaryProps
  algorithm
     p:=f.R*f.d*f.T*f.delta*f.fdelta;
     sat.d:=f.d;
  ...
  // algorithms and equations of cv2Phase
  algorithm
  ...
  // algorithms and equations of cvdpT2Phase
  algorithm
     dxv:=(if liq.d<>vap.d then liq.d*vap.d/(liq.d-vap.d) else 0.0);
     dpT:=(vap.s-liq.s)*dxv;
     dvTl:=(liq.pt-dpT)/(liq.pd)/(liq.d)/(liq.d);
  ...
  // components of gibbsToExtraDerivs
   input GibbsDerivs g "dimensionless derivatives of Gibbs function";
  ...
  // algorithms and equations of gibbsToExtraDerivs
  algorithm
  ...
  // components of helmholtzToExtraDerivs
   input HelmholtzDerivs f "dimensionless derivatives of Helmholtz function";
   output ExtraDerivatives dpro "additional property derivatives";
  protected
   SI.Pressure p "pressure";
  ...
  // algorithms and equations of helmholtzToExtraDerivs
  algorithm
     v:=1/(f.d);
     p:=f.R*f.d*f.T*f.delta*f.fdelta;
     pv:=(-f.d*f.d)*f.R*f.T*f.delta*(2.0*f.fdelta+f.delta*f.fdeltadelta);
  ...
  // algorithms and equations of smoothStep
  algorithm
...
<pre>
  ...
  package OneNonLinearEquation
   ...
   // algorithms and equations of solve
   algorithm
...
<pre>
 ...
 protected
  ...
  package ThermoFluidSpecial
   ...
   record FixedIGProperties
    "constant properties for ideal gases"
    extends Modelica.Icons.Record;

   // components of FixedIGProperties
    parameter Integer nspecies(min=1) "number of components";
    SI.MolarMass MM[nspecies] "molar mass of components";
    Real invMM[nspecies] "inverse of molar mass of components";
    SI.SpecificHeatCapacity R[nspecies] "gas constant";
    SI.SpecificEnthalpy Hf[nspecies] "enthalpy of formation at 298.15K";
    SI.SpecificEnthalpy H0[nspecies] "H0(298.15K) - H0(0K)";
   end FixedIGProperties;

   record ThermoBaseVars
    extends Modelica.Icons.Record;

   // components of ThermoBaseVars
    parameter Integer n(min=1) "discretization number";
    parameter Integer nspecies(min=1) "number of species";
    SI.Pressure p[n](min=PMIN,max=PMAX,nominal=PNOM,start=fill(1.0e5,n)) "Pressure";
    SI.Temperature T[n](min=TMIN,max=TMAX,nominal=TNOM) "temperature";
    SI.Density d[n](min=DMIN,max=DMAX,nominal=DNOM) "density";
    SI.SpecificEnthalpy h[n](min=SHMIN,max=SHMAX,nominal=SHNOM) "specific enthalpy";
    SI.SpecificEntropy s[n](min=SSMIN,max=SSMAX,nominal=SSNOM) "specific entropy";
    SI.RatioOfSpecificHeatCapacities kappa[n] "ratio of cp/cv";
    SI.Mass M[n](min=MMIN,max=MMAX,nominal=MNOM) "Total mass";
    SI.Energy U[n](min=EMIN,max=EMAX,nominal=ENOM) "Inner energy";
    SI.MassFlowRate dM[n](min=MDOTMIN,max=MDOTMAX,nominal=MDOTNOM) "Change in total mass";
    SI.Power dU[n](min=POWMIN,max=POWMAX,nominal=POWNOM) "Change in inner energy";
    SI.Volume V[n](min=VMIN,max=VMAX,nominal=VNOM) "Volume";
    SI.MassFraction mass_x[n,nspecies](min=MASSXMIN,max=MASSXMAX,nominal=MASSXNOM) "mass fraction";
    SI.MoleFraction mole_y[n,nspecies](min=MOLEYMIN,max=MOLEYMAX,nominal=MOLEYNOM) "mole fraction";
    SI.Mass M_x[n,nspecies](min=MMIN,max=MMAX,nominal=MNOM) "component mass";
    SI.MassFlowRate dM_x[n,nspecies](min=MDOTMIN,max=MDOTMAX,nominal=MDOTNOM) "rate of change in component mass";
    MolarFlowRate dZ[n,nspecies](min=-1.0e6,max=1.0e6,nominal=0.0) "rate of change in component moles";
    MolarFlowRate rZ[n,nspecies](min=-1.0e6,max=1.0e6,nominal=0.0) "Reaction(source) mole rates";
    SI.MolarMass MM[n](min=MMMIN,max=MMMAX,nominal=MMNOM) "molar mass of mixture";
    SI.AmountOfSubstance Moles[n](min=MOLMIN,max=MOLMAX,nominal=MOLNOM) "total moles";
    SI.AmountOfSubstance Moles_z[n,nspecies](min=MOLMIN,max=MOLMAX,nominal=MOLNOM) "mole vector";
   ...
   // components of ThermoProperties
    parameter Integer nspecies(min=1) "number of species";
    SI.Temperature T(min=TMIN,max=TMAX,nominal=TNOM) "temperature";
    SI.Density d(min=DMIN,max=DMAX,nominal=DNOM) "density";
    SI.Pressure p(min=PMIN,max=PMAX,nominal=PNOM) "pressure";
    SI.Volume V(min=VMIN,max=VMAX,nominal=VNOM) "Volume";
    SI.SpecificEnthalpy h(min=SHMIN,max=SHMAX,nominal=SHNOM) "specific enthalpy";
    SI.SpecificEnergy u(min=SEMIN,max=SEMAX,nominal=SENOM) "specific inner energy";
    SI.SpecificEntropy s(min=SSMIN,max=SSMAX,nominal=SSNOM) "specific entropy";
    SI.SpecificGibbsFreeEnergy g(min=SHMIN,max=SHMAX,nominal=SHNOM) "specific Gibbs free energy";
    SI.SpecificHeatCapacity cp(min=CPMIN,max=CPMAX,nominal=CPNOM) "heat capacity at constant pressure";
    SI.SpecificHeatCapacity cv(min=CPMIN,max=CPMAX,nominal=CPNOM) "heat capacity at constant volume";
    SI.SpecificHeatCapacity R(min=CPMIN,max=CPMAX,nominal=CPNOM) "gas constant";
    SI.MolarMass MM(min=MMMIN,max=MMMAX,nominal=MMNOM) "molar mass of mixture";
    SI.MassFraction mass_x[nspecies](min=MASSXMIN,max=MASSXMAX,nominal=MASSXNOM) "mass fraction";
    SI.MoleFraction mole_y[nspecies](min=MOLEYMIN,max=MOLEYMAX,nominal=MOLEYNOM) "mole fraction";
    ...
    Real dupT(unit="m3.kg-1") "derivative of inner energy by pressure at constant T";
    Real dudT(unit="(J.m3)/(kg2)") "derivative of inner energy by density at constant T";
    SI.SpecificHeatCapacity duTp "derivative of inner energy by temperature at constant p";
    SI.SpecificEnergy ddx[nspecies] "derivative vector of density by change in mass composition";
    SI.SpecificEnergy compu[nspecies](min=SEMIN,max=SEMAX,nominal=SENOM) "inner energy of the components";
    SI.Pressure compp[nspecies](min=COMPPMIN,max=COMPPMAX,nominal=COMPPNOM) "partial pressures of the components";
    SI.Velocity a(min=VELMIN,max=VELMAX,nominal=VELNOM) "speed of sound";
    SI.HeatCapacity dUTZ "derivative of inner energy by temperature at constant moles";
    SI.MolarInternalEnergy dUZT[nspecies] "derivative of inner energy by moles at constant temperature";
    SI.SpecificEnthalpy dHMxT[nspecies](min=SEMIN,max=SEMAX,nominal=SENOM) "derivative of total enthalpy w.r.t. component mass at constant T";
    Real dpT "derivative of pressure w.r.t. temperature";
   ...
   // components of ThermoProperties_ph
    SI.Temperature T(min=1.0e-9,max=10000.0,nominal=298.15) "temperature";
    SI.Density d(min=1.0e-9,max=10000.0,nominal=10.0) "density";
    SI.SpecificEnergy u(min=-1.0e8,max=1.0e8,nominal=1.0e6) "specific inner energy";
    SI.SpecificEntropy s(min=-1.0e6,max=1.0e6,nominal=1.0e3) "specific entropy";
    SI.SpecificHeatCapacity cp(min=1.0,max=1.0e6,nominal=1000.0) "heat capacity at constant pressure";
    SI.SpecificHeatCapacity cv(min=1.0,max=1.0e6,nominal=1000.0) "heat capacity at constant volume";
    SI.SpecificHeatCapacity R(min=1.0,max=1.0e6,nominal=1000.0) "gas constant";
    SI.RatioOfSpecificHeatCapacities kappa "ratio of cp/cv";
    SI.Velocity a(min=1.0,max=10000.0,nominal=300.0) "speed of sound";
    ...
    Real duph(unit="m3/kg") "derivative of inner energy by pressure at constant enthalpy";
    Real duhp(unit="1") "derivative of inner energy by enthalpy at constant pressure";
...
"));
   ...
   // components of ThermoProperties_pT
    SI.Density d(min=1.0e-9,max=10000.0,nominal=10.0) "density";
    SI.SpecificEnthalpy h(min=-1.0e8,max=1.0e8,nominal=1.0e6) "specific enthalpy";
    SI.SpecificEnergy u(min=-1.0e8,max=1.0e8,nominal=1.0e6) "specific inner energy";
    SI.SpecificEntropy s(min=-1.0e6,max=1.0e6,nominal=1.0e3) "specific entropy";
    SI.SpecificHeatCapacity cp(min=1.0,max=1.0e6,nominal=1000.0) "heat capacity at constant pressure";
    SI.SpecificHeatCapacity cv(min=1.0,max=1.0e6,nominal=1000.0) "heat capacity at constant volume";
...
"));
   ...
   // components of ThermoProperties_dT
    SI.Pressure p(min=1.0,max=1.0e9,nominal=1.0e5) "pressure";
    SI.SpecificEnthalpy h(min=-1.0e8,max=1.0e8,nominal=1.0e6) "specific enthalpy";
    SI.SpecificEnergy u(min=-1.0e8,max=1.0e8,nominal=1.0e6) "specific inner energy";
    SI.SpecificEntropy s(min=-1.0e6,max=1.0e6,nominal=1.0e3) "specific entropy";
    SI.SpecificHeatCapacity cp(min=1.0,max=1.0e6,nominal=1000.0) "heat capacity at constant pressure";
    ...
    SI.RatioOfSpecificHeatCapacities kappa "ratio of cp/cv";
    SI.Velocity a(min=1.0,max=10000.0,nominal=300.0) "speed of sound";
    Real dudT(unit="m5/(kg.s2)") "derivative of inner energy by density at constant T";
...
"));
   ...
   // components of gibbsToProps_ph
    input GibbsDerivs g "dimensionless derivatives of Gibbs function";
    output ThermoProperties_ph pro "property record for pressure and specific enthalpy as dynamic states";
   protected
    Real vt(unit="m3.kg-1.K-1") "derivative of specific volume w.r.t. temperature";
    Real vp(unit="m4.kg-2.s2") "derivative of specific volume w.r.t. pressure";

   // algorithms and equations of gibbsToProps_ph
   algorithm
      pro.T:=g.T;
      pro.R:=g.R;
   ...
   // components of gibbsToProps_dT
    input GibbsDerivs g "dimensionless derivatives of Gibbs function";
    output ThermoProperties_dT pro "property record for density and temperature as dynamic states";
   protected
    Real vt(unit="m3.kg-1.K-1") "derivative of specific volume w.r.t. temperature";
    Real vp(unit="m4.kg-2.s2") "derivative of specific volume w.r.t. pressure";
    Modelica.SIunits.Density d;

   // algorithms and equations of gibbsToProps_dT
   algorithm
      pro.R:=g.R;
      pro.p:=g.p;
      pro.u:=g.T*g.R*(g.tau*g.gtau-g.pi*g.gpi);
   ...
   // components of gibbsToProps_pT
    input GibbsDerivs g "dimensionless derivatives of Gibbs function";
    output ThermoProperties_pT pro "property record for pressure and temperature as dynamic states";
   protected
    Real vt(unit="m3.kg-1.K-1") "derivative of specific volume w.r.t. temperature";
    Real vp(unit="m4.kg-2.s2") "derivative of specific volume w.r.t. pressure";

   // algorithms and equations of gibbsToProps_pT
   algorithm
      pro.R:=g.R;
   ...
   // components of helmholtzToProps_ph
    input HelmholtzDerivs f "dimensionless derivatives of Helmholtz function";
    output ThermoProperties_ph pro "property record for pressure and specific enthalpy as dynamic states";
   protected
    SI.Pressure p "pressure";
   ...
   // algorithms and equations of helmholtzToProps_ph
   algorithm
      pro.d:=f.d;
      pro.T:=f.T;
      pro.R:=f.R;
   ...
   // components of helmholtzToProps_pT
    input HelmholtzDerivs f "dimensionless derivatives of Helmholtz function";
    output ThermoProperties_pT pro "property record for pressure and temperature as dynamic states";
   protected
    DerPressureByDensity pd "derivative of pressure w.r.t. density";
   ...
   // algorithms and equations of helmholtzToProps_pT
   algorithm
      pro.d:=f.d;
      pro.R:=f.R;
      pro.s:=f.R*(f.tau*f.ftau-f.f);
   ...
   // components of helmholtzToProps_dT
    input HelmholtzDerivs f "dimensionless derivatives of Helmholtz function";
    output ThermoProperties_dT pro "property record for density and temperature as dynamic states";
   protected
    DerPressureByTemperature pt "derivative of pressure w.r.t. temperature";
   ...
   // algorithms and equations of helmholtzToProps_dT
   algorithm
   ...
   // components of TwoPhaseToProps_ph
    input SaturationProperties sat "saturation property record";
    output ThermoProperties_ph pro "property record for pressure and specific enthalpy as dynamic states";
   protected
    Real dht(unit="(J/kg)/K") "derivative of specific enthalpy w.r.t. temperature";
    Real dhd(unit="(J/kg)/(kg/m3)") "derivative of specific enthalpy w.r.t. density";
    Real detph(unit="m4.s4/(K.s8)") "thermodynamic determinant";

   // algorithms and equations of TwoPhaseToProps_ph
   algorithm
      pro.d:=sat.d;
      pro.T:=sat.T;
      pro.u:=sat.u;
   ...
   // components of TwoPhaseToProps_dT
    input SaturationProperties sat "saturation properties";
    output ThermoProperties_dT pro "property record for density and temperature as dynamic states";

   // algorithms and equations of TwoPhaseToProps_dT
   algorithm
      pro.p:=sat.p;
      pro.h:=sat.h;
      pro.u:=sat.u;
