...
package Utilities
 ...
 // algorithms and equations of checkBoundary
 algorithm
...
");
 ...
 // components of regRoot
  input Real x;
  input Real delta=0.01 "Range of significant deviation from sqrt(abs(x))*sgn(x)";
  output Real y;

 // algorithms and equations of regRoot
 algorithm
    y:=x/((x*x+delta*delta)^0.25);

 annotation(derivative(zeroDerivative=delta)=Utilities.regRoot_der,Documentation(info="<html>
...
<li><i>15 Mar 2005</i>
 ...
 // locally defined classes in regRoot2

 // components of regRoot2
 ...
 protected
  ...
  // components of regRoot2_utility
   input Real x;
  ...
  // algorithms and equations of regRoot2_utility
  algorithm
     if k2>0 then
       x2:=(-x1)*k2/(k1);
     elseif k1>0 then
     ...
     if x<=x2 then
       ...
       y:=y1*(if x>=0 then evaluatePoly3_derivativeAtZero(x/(x1),1,1,y1d*x1/(y1),y0d*x1/(y1)) else evaluatePoly3_derivativeAtZero(x/(x1),x2/(x1),y2/(y1),y2d*x1/(y1),y0d*x1/(y1)));
     end if;

  annotation(smoothOrder=2);
  end regRoot2_utility;

 // algorithms and equations of regRoot2
 algorithm
...
<li><i>Nov., 2005</i>
 ...
 // locally defined classes in regSquare2

 // components of regSquare2
  input Real x "abscissa value";
  input Real x_small(min=0)=0.01 "approximation of function for |x| <= x_small";
 ...
 protected
  ...
  // components of regSquare2_utility
   input Real x;
   input Real x1 "approximation of function abs(x) < x1";
  ...
  // algorithms and equations of regSquare2_utility
  algorithm
     x2:=-x1;
     if x<=x2 then
       y:=(-k2)*x^(2);
       ...
       y:=(if x>=0 then evaluatePoly3_derivativeAtZero(x,x1,y1,y1d,y0d) else evaluatePoly3_derivativeAtZero(x,x2,y2,y2d,y0d));
     end if;

  annotation(smoothOrder=2);
  end regSquare2_utility;

 // algorithms and equations of regSquare2
 algorithm
...
<li><i>Nov., 2005</i>
 ...
 // algorithms and equations of regStep
 algorithm
...
for a smooth transition from y1 to y2.
 ...
 // components of evaluatePoly3_derivativeAtZero
 ...
 // algorithms and equations of evaluatePoly3_derivativeAtZero
 algorithm
    a1:=x1*y0d;
    a2:=3*y1-x1*y1d-2*a1;
    a3:=y1-a2-a1;
 ...
 // algorithms and equations of regFun3
 algorithm
    assert(x0<x1,"regFun3(): Data points not sorted appropriately (x0 = "+String(x0)+" > x1 = "+String(x1)+"). Please flip arguments.");
    assert((y0d*y1d)>=0,"regFun3(): Deriavtives at data points do not allow co-monotone interpolation, as both are non-zero and of opposite sign (y0d = "+String(y0d)+", y1d = "+String(y1d)+"). Please correct arguments.");
    h0:=x1-x0;
    Delta0:=(y1-y0)/(h0);
...
<dd> <b>Piecewise cubic monotone interpolation with assigned slopes</b>.
 ...
 // algorithms and equations of cubicHermite
 algorithm
    h:=x2-x1;
    if abs(h)>0 then
      t:=(x-x1)/(h);
...
<li><i>May 2008</i>
 ...
 // components of cubicHermite_withDerivative
  input Real x "Abscissa value";
  input Real x1 "Lower abscissa value";
  input Real x2 "Upper abscissa value";
  input Real y1 "Lower ordinate value";
 ...
 // algorithms and equations of cubicHermite_withDerivative
 algorithm
    h:=x2-x1;
    if abs(h)>0 then
      t:=(x-x1)/(h);
