...
reference 01IS07022B). The project is started in October 2007 and ended in June 2010.
 ...
 package HeatTransfer
  ...
  package Channel
   ...
   // locally defined classes in kc_evenGapLaminar

   // components of kc_evenGapLaminar
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapLaminar_IN_con IN_con "Input record for function kc_evenGapLaminar" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapLaminar_IN_var IN_var "Input record for function kc_evenGapLaminar" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
    output Real failureStatus "0== boundary conditions fulfilled | 1== failure >> check if still meaningfull results" annotation(Dialog(group="Output"));
   protected
   ...
   protected
    ...
    Real prandtlMax=(if IN_con.target==TYP.UndevOne then 10 else (if IN_con.target==TYP.UndevBoth then 1000 else 0)) "Maximum Prandtl number";
    Real prandtlMin=(if (IN_con.target==TYP.UndevOne) or (IN_con.target==TYP.UndevBoth) then 0.1 else 0) "Minimum Prandtl number";
   ...
   // algorithms and equations of kc_evenGapLaminar
   algorithm
...
<a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.Channel.kc_evenGapLaminar\">See more information.</a> </p>
   ...
   // locally defined classes in kc_evenGapLaminar_KC

   // components of kc_evenGapLaminar_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapLaminar_IN_con IN_con "Input record for function kc_evenGapLaminar_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapLaminar_IN_var IN_var "Input record for function kc_evenGapLaminar_KC" annotation(Dialog(group="Variable inputs"));
   ...
   protected
    ...
    SI.NusseltNumber Nu_1=(if (IN_con.target==TYP.DevOne) or (IN_con.target==TYP.UndevOne) then 4.861 else (if (IN_con.target==TYP.DevBoth) or (IN_con.target==TYP.UndevBoth) then 7.541 else 0)) "First Nusselt number";
    SI.NusseltNumber Nu_2=1.841*(Re*Pr*d_hyd/(max(IN_con.L,MIN)))^(1/(3)) "Second Nusselt number";
    SI.NusseltNumber Nu_3=(if (IN_con.target==TYP.UndevOne) or (IN_con.target==TYP.UndevBoth) then (2/(1+22*Pr))^(1/(6))*(Re*Pr*d_hyd/(max(IN_con.L,MIN)))^0.5 else 0) "Third mean Nusselt number";
    SI.NusseltNumber Nu=(Nu_1^(3)+Nu_2^(3)+Nu_3^(3))^(1/(3));

   // algorithms and equations of kc_evenGapLaminar_KC
   algorithm
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapLaminar_KC\"> kc_evenGapLaminar_KC</a>.
   ...
   // locally defined classes in kc_evenGapOverall

   // components of kc_evenGapOverall
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapOverall_IN_con IN_con "Input record for function kc_evenGapOverall" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapOverall_IN_var IN_var "Input record for function kc_evenGapOverall" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
   ...
   // algorithms and equations of kc_evenGapOverall
   algorithm
      Pr:=abs(IN_var.eta*IN_var.cp/(max(MIN,IN_var.lambda)));
      ...
      Nu:=kc*d_hyd/(max(MIN,IN_var.lambda));
      fstatus[1]:=(if (IN_con.target==TYP.UndevOne) or (IN_con.target==TYP.UndevBoth) then (if (Pr>prandtlMax) or (Pr<prandtlMin) then 1 else 0) else 0);
   ...
   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,Documentation(info="<html>
<p>
...
"));
   ...
   // components of kc_evenGapOverall_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapOverall_IN_con IN_con "Input record for function kc_evenGapOverall_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapOverall_IN_var IN_var "Input record for function kc_evenGapOverall_KC" annotation(Dialog(group="Variable inputs"));
   ...
   // algorithms and equations of kc_evenGapOverall_KC
   algorithm
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapOverall_KC\"> kc_evenGapOverall_KC</a>.
   ...
   // components of kc_evenGapOverall_IN_var
...
<a href=\"Modelica://Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapOverall_KC\"> kc_evenGapOverall_KC</a>.
   ...
   // components of kc_evenGapTurbulent
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapTurbulent_IN_con IN_con "Input record for function kc_evenGapTurbulent" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapTurbulent_IN_var IN_var "Input record for function kc_evenGapTurbulent" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
   ...
   // algorithms and equations of kc_evenGapTurbulent
   algorithm
      Pr:=abs(IN_var.eta*IN_var.cp/(max(MIN,IN_var.lambda)));
      Re:=max(1,abs(IN_var.rho*velocity*d_hyd/(max(MIN,IN_var.eta))));
      kc:=Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapTurbulent_KC(IN_con,IN_var);
      Nu:=kc*d_hyd/(max(MIN,IN_var.lambda));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_evenGapTurbulent_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapTurbulent_IN_con IN_con "Input record for function kc_evenGapTurbulent_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapTurbulent_IN_var IN_var "Input record for function kc_evenGapTurbulent_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_evenGapTurbulent_KC";
   protected
    Real MIN=Modelica.Constants.eps;
    SI.Area A_cross=max(MIN,IN_con.s*IN_con.h) "Cross sectional area of gap";
    SI.Diameter d_hyd=2*IN_con.s "Hydraulic diameter";
    ...
    SI.NusseltNumber Nu=abs(zeta/(8)*Re*Pr/(1+12.7*(zeta/(8))^0.5*(Pr^(2/(3))-1))*(1+(d_hyd/(max(MIN,IN_con.L)))^(2/(3))));

   // algorithms and equations of kc_evenGapTurbulent_KC
   algorithm
      kc:=Nu*IN_var.lambda/(max(MIN,d_hyd));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   record kc_evenGapTurbulent_IN_con
    ...
    extends Modelica.Fluid.Dissipation.HeatTransfer.Channel.kc_evenGapOverall_IN_con(final target=2);
...
<p>Calculation of the mean convective heat transfer coefficient <b>kc </b>for a laminar fluid flow through an even gap at different fluid flow and heat transfer situations. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.Channel.kc_evenGapOverall\">See more information.</a></p>
  ...
  package General
   ...
   // locally defined classes in kc_approxForcedConvection

   // components of kc_approxForcedConvection
    input Modelica.Fluid.Dissipation.HeatTransfer.General.kc_approxForcedConvection_IN_con IN_con "Input record for function kc_approxForcedConvection" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.General.kc_approxForcedConvection_IN_var IN_var "Input record for function kc_approxForcedConvection" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
   ...
   // algorithms and equations of kc_approxForcedConvection
   algorithm
      Pr:=Modelica.Fluid.Dissipation.Utilities.Functions.General.PrandtlNumber(IN_var.cp,IN_var.eta,IN_var.lambda);
      Re:=max(1,Modelica.Fluid.Dissipation.Utilities.Functions.General.ReynoldsNumber(IN_con.A_cross,IN_con.perimeter,IN_var.rho,IN_var.eta,abs(IN_var.m_flow)));
      kc:=Modelica.Fluid.Dissipation.HeatTransfer.General.kc_approxForcedConvection_KC(IN_con,IN_var);
      Nu:=kc*d_hyd/(max(MIN,IN_var.lambda));
      fstatus[1]:=(if IN_con.target==TYP.Rough then (if (Pr>prandtlMax[1]) or (Pr<prandtlMin[1]) then 1 else 0) else (if IN_con.target==TYP.Middle then (if (Pr>prandtlMax[2]) or (Pr<prandtlMin[2]) then 1 else 0) else (if IN_con.target==TYP.Finest then (if (Pr>prandtlMax[3]) or (Pr<prandtlMin[3]) then 1 else 0) else 0)));
      fstatus[2]:=(if IN_con.target==TYP.Rough then (if (Re>reynoldsMax[1]) or (Re<reynoldsMin[1]) then 1 else 0) else (if IN_con.target==TYP.Middle then (if (Re>reynoldsMax[2]) or (Re<reynoldsMin[2]) then 1 else 0) else (if IN_con.target==TYP.Finest then (if (Re>reynoldsMax[3]) or (Re<reynoldsMin[3]) then 1 else 0) else 0)));
      failureStatus:=0;
      for i in 1:size(fstatus,1) loop
        if fstatus[i]==1 then
          failureStatus:=1;
...
Note that additionally a failure status is observed in this function to check if the intended boundary conditions are fulfilled.  <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.General.kc_approxForcedConvection\">See more information</a> .
   ...
   // locally defined classes in kc_approxForcedConvection_KC

   // components of kc_approxForcedConvection_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.General.kc_approxForcedConvection_IN_con IN_con "Input record for function kc_approxForcedConvection_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.General.kc_approxForcedConvection_IN_var IN_var "Input record for function kc_approxForcedConvection_KC" annotation(Dialog(group="Variable inputs"));
   ...
   protected
    ...
    SI.ReynoldsNumber Re=max(1,4*abs(IN_var.m_flow)/(max(MIN,IN_con.perimeter*IN_var.eta))) "Reynolds number";

   // algorithms and equations of kc_approxForcedConvection_KC
   algorithm
...
<a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.General.kc_approxForcedConvection\">See more information</a> .
   ...
   // components of kc_approxForcedConvection_IN_con
    parameter Real exp_Pr=0.4 "Exponent for Prandtl number w.r.t. Dittus/Boelter | 0.4 for heating | 0.3 for cooling" annotation(Dialog(group="Generic variables",enable=(if target==1 then true else false)));
...
<a href=\"Modelica://Modelica.Fluid.Dissipation.HeatTransfer.General.kc_approxForcedConvection_KC\"> kc_approxForcedConvection_KC</a>.
   ...
   // components of kc_approxForcedConvection_IN_var
    SI.DynamicViscosity eta_wall "Dynamic viscosity of fluid at wall temperature" annotation(Dialog(group="Fluid properties",enable=(if target==2 then true else false)));
    SI.MassFlowRate m_flow annotation(Dialog(group="Input"));
...
<p>Approximate calculation of the mean convective heat transfer coefficient <b> kc </b> for forced convection with a fully developed fluid flow in a turbulent regime. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.General.kc_approxForcedConvection\">See more information.</a> </p>
  ...
  package HeatExchanger
   ...
   // locally defined classes in kc_flatTube

   // components of kc_flatTube
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_flatTube_IN_con IN_con "Input record for function kc_flatTube" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_flatTube_IN_var IN_var "Input record for function kc_flatTube" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
    output Real failureStatus "0== boundary conditions fulfilled | 1== failure >> check if still meaningfull results" annotation(Dialog(group="Output"));
   protected
        type TYP = Modelica.Fluid.Dissipation.Utilities.Types.HTXGeometry_flatTubes;
   protected
    SI.Area A_c=(if IN_con.geometry==TYP.LouverFin then IN_con.A_fr*(IN_con.F_l-IN_con.delta_f)*(IN_con.F_p-IN_con.delta_f)/((IN_con.F_l+IN_con.D_m)*IN_con.F_p) else (if IN_con.geometry==TYP.RectangularFin then IN_con.A_fr*h*s/((h+t+IN_con.D_m)*(s+t)) else 0)) "Minimum flow cross-sectional area";
    SI.Length h=(if IN_con.geometry==TYP.RectangularFin then IN_con.D_h*(1+IN_con.alpha)/(2*IN_con.alpha) else 0) "Free flow height";
    SI.Length l=(if IN_con.geometry==TYP.RectangularFin then t/(IN_con.delta) else 0) "Fin length";
    SI.Length s=(if IN_con.geometry==TYP.RectangularFin then h*IN_con.alpha else 0) "Lateral fin spacing (free flow width)";
    SI.Length t=(if IN_con.geometry==TYP.RectangularFin then s*IN_con.gamma else 0) "Fin thickness";

   // algorithms and equations of kc_flatTube
   algorithm
      kc:=Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_flatTube_KC(IN_con,IN_var);
      ...
      failureStatus:=(if IN_con.geometry==TYP.LouverFin then (if (Re<100) or (Re>3000) then 1 else 0) else (if IN_con.geometry==TYP.RectangularFin then (if (Re<300) or (Re>5000) then 1 else 0) else 0));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // locally defined classes in kc_flatTube_KC

   // components of kc_flatTube_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_flatTube_IN_con IN_con "Input record for function kc_flatTube_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_flatTube_IN_var IN_var "Input record for function kc_flatTube_KC" annotation(Dialog(group="Variable inputs"));
   ...
   protected
    ...
    SI.ReynoldsNumber Re_Dh=max(1e-3,abs(IN_var.m_flow)*IN_con.D_h/(IN_var.eta*A_c)) "Reynolds number based on hydraulic diameter";
    SI.ReynoldsNumber Re_Lp=max(1e-3,abs(IN_var.m_flow)*IN_con.L_p/(IN_var.eta*A_c)) "Reynolds number based on louver pitch";
   ...
   // algorithms and equations of kc_flatTube_KC
   algorithm
      if IN_con.geometry==TYP.LouverFin then
        j:=Re_Lp^(-0.49)*(Phi/(90))^0.27*(IN_con.F_p/(IN_con.L_p))^(-0.14)*(IN_con.F_l/(IN_con.L_p))^(-0.29)*(IN_con.T_d/(IN_con.L_p))^(-0.23)*(IN_con.L_l/(IN_con.L_p))^0.68*(IN_con.T_p/(IN_con.L_p))^(-0.28)*(IN_con.delta_f/(IN_con.L_p))^(-0.05);
        kc:=j*Re_Lp*Pr^(1/(3))*IN_var.lambda/(IN_con.L_p);
      elseif IN_con.geometry==TYP.RectangularFin then
        j:=0.6522*Re_Dh^(-0.5403)*(s/(h))^(-0.1541)*(t/(l))^0.1499*(t/(s))^(-0.0678)*(1+5.269e-5*Re_Dh^1.340*(s/(h))^0.504*(t/(l))^0.456*(t/(s))^(-1.055))^0.1;
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // locally defined classes in kc_flatTube_IN_con

   // components of kc_flatTube_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.HTXGeometry_flatTubes geometry=Dissipation.Utilities.Types.HTXGeometry_flatTubes.LouverFin "Choice of fin geometry" annotation(Dialog(group="HeatExchanger"));
    SI.Area A_fr=0 "Frontal area" annotation(Dialog(group="HeatExchanger"));
    SI.Length D_h=0 "Hydraulic diameter" annotation(Dialog(group="HeatExchanger",enable=(if geometry==2 then true else false)));
    SI.Length D_m=0 "Major tube diameter for flat tube" annotation(Dialog(group="HeatExchanger"));
    SI.Length F_l=0 "Fin length" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
    SI.Length F_p=0 "Fin pitch, fin spacing + fin thickness" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
    SI.Length L_l=0 "Louver length" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
    SI.Length L_p=0 "Louver pitch" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
    SI.Length T_d=0 "Tube depth" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
    SI.Length T_p=0 "Tube pitch" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
    Real alpha=0 "Lateral fin spacing (s) / free flow height (h)" annotation(Dialog(group="HeatExchanger",enable=(if geometry==2 then true else false)));
    Real gamma=0 "Fin thickness (t) / lateral fin spacing (s)" annotation(Dialog(group="HeatExchanger",enable=(if geometry==2 then true else false)));
    Real delta=0 "Fin thickness (t) / Fin length (l)" annotation(Dialog(group="HeatExchanger",enable=(if geometry==2 then true else false)));
    SI.Length delta_f=0 "Fin thickness" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
    SI.Angle Phi=0 "Louver angle" annotation(Dialog(group="HeatExchanger",enable=(if geometry==1 then true else false)));
   protected
        type TYP = Modelica.Fluid.Dissipation.Utilities.Types.HTXGeometry_flatTubes;
...
<a href=\"Modelica://Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_flatTube_KC\"> kc_flatTube_KC</a>.
   ...
   // components of kc_flatTube_IN_var
    SI.MassFlowRate m_flow annotation(Dialog(group="Input"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_flatTube_KC\"> kc_flatTube_KC</a>.
   ...
   // locally defined classes in kc_roundTube

   // components of kc_roundTube
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_roundTube_IN_con IN_con "Input record for function kc_roundTube" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_roundTube_IN_var IN_var "Input record for function kc_roundTube" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
    output Real failureStatus "0== boundary conditions fulfilled | 1== failure >> check if still meaningfull results" annotation(Dialog(group="Output"));
   protected
        type TYP = Modelica.Fluid.Dissipation.Utilities.Types.HTXGeometry_roundTubes;
   protected
    SI.Area A_c=IN_con.A_fr*(IN_con.F_p*IN_con.P_t-IN_con.F_p*IN_con.D_c-(IN_con.P_t-IN_con.D_c)*IN_con.delta_f)/(IN_con.F_p*IN_con.P_t) "Minimum flow cross-sectional area";
    SI.Area A_tot=(if IN_con.geometry==TYP.LouverFin then IN_con.A_fr*(IN_con.N*PI*IN_con.D_c*(IN_con.F_p-IN_con.delta_f)+2*(IN_con.P_t*IN_con.L-IN_con.N*PI*IN_con.D_c^(2)/(4)))/(IN_con.P_t*IN_con.F_p) else 0) "Total heat transfer area";
    SI.Length D_h=(if IN_con.geometry==TYP.LouverFin then 4*A_c*IN_con.L/(A_tot) else 0) "Hydraulic diameter";

   // algorithms and equations of kc_roundTube
   algorithm
      kc:=Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_roundTube_KC(IN_con,IN_var);
      Pr:=abs(IN_var.eta*IN_var.cp/(IN_var.lambda));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // locally defined classes in kc_roundTube_KC

   // components of kc_roundTube_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_roundTube_IN_con IN_con "Input record for function kc_roundTube_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_roundTube_IN_var IN_var "Input record for function kc_roundTube_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_roundTube_KC";
   protected
        type TYP = Modelica.Fluid.Dissipation.Utilities.Types.HTXGeometry_roundTubes;
   protected
    Real MIN=Modelica.Constants.eps;
    SI.ReynoldsNumber Re_Dc=max(1e-3,abs(IN_var.m_flow)*IN_con.D_c/(IN_var.eta*A_c)) "Reynolds number based on fin collar diameter";
    SI.PrandtlNumber Pr=IN_var.eta*IN_var.cp/(IN_var.lambda) "Prandtl number";
    Real j "Colburn j faktor";
    SI.Area A_c=IN_con.A_fr*(IN_con.F_p*IN_con.P_t-IN_con.F_p*IN_con.D_c-(IN_con.P_t-IN_con.D_c)*IN_con.delta_f)/(IN_con.F_p*IN_con.P_t) "Minimum flow cross-sectional area";
    SI.Area A_tot=(if IN_con.geometry==TYP.LouverFin then IN_con.A_fr*(IN_con.N*PI*IN_con.D_c*(IN_con.F_p-IN_con.delta_f)+2*(IN_con.P_t*IN_con.L-IN_con.N*PI*IN_con.D_c^(2)/(4)))/(IN_con.P_t*IN_con.F_p) else 0) "Total heat transfer area";
   ...
   // algorithms and equations of kc_roundTube_KC
   algorithm
      if IN_con.geometry==TYP.PlainFin then
        j:=0.991*(2.24*Re_Dc^(-0.092)*(IN_con.N/(4))^(-0.031))^(0.607*(4-IN_con.N))*0.14*Re_Dc^(-0.328)*(IN_con.P_t/(IN_con.P_l))^(-0.502)*(IN_con.F_p/(IN_con.D_c))^0.0312*2.55*(IN_con.P_l/(IN_con.D_c))^(-1.28);
      ...
      elseif IN_con.geometry==TYP.LouverFin then
        if Re_Dc<900 then
          ...
          j:=14.3117*Re_Dc^J1*(IN_con.F_p/(IN_con.D_c))^J2*(IN_con.L_h/(IN_con.L_p))^J3*(IN_con.F_p/(IN_con.P_l))^J4*(IN_con.P_l/(IN_con.P_t))^(-1.724);
        elseif Re_Dc>1100 then
          J5:=-0.6027+0.02593*(IN_con.P_l/(D_h))^0.52*IN_con.N^(-0.5)*log(IN_con.L_h/(IN_con.L_p));
          J6:=-0.4776+0.40774*IN_con.N^0.7/(log(Re_Dc)-4.4);
          J7:=(-0.58655)*(IN_con.F_p/(D_h))^2.3*(IN_con.P_l/(IN_con.P_t))^(-1.6)*IN_con.N^(-0.65);
          J8:=0.0814*(log(Re_Dc)-3);
          j:=1.1373*Re_Dc^J5*(IN_con.F_p/(IN_con.P_l))^J6*(IN_con.L_h/(IN_con.L_p))^J7*(IN_con.P_l/(IN_con.P_t))^J8*IN_con.N^0.3545;
        else
          J1:=-0.991-0.1055*(IN_con.P_l/(IN_con.P_t))^3.1*log(IN_con.L_h/(IN_con.L_p));
          J2:=-0.7344+2.1059*IN_con.N^0.55/(log(Re_Dc)-3.2);
          J3:=0.08485*(IN_con.P_l/(IN_con.P_t))^(-4.4)*IN_con.N^(-0.68);
          ...
          J7:=(-0.58655)*(IN_con.F_p/(D_h))^2.3*(IN_con.P_l/(IN_con.P_t))^(-1.6)*IN_con.N^(-0.65);
          J8:=0.0814*(log(Re_Dc)-3);
          j:=SMOOTH(900,1100,Re_Dc)*14.3117*Re_Dc^J1*(IN_con.F_p/(IN_con.D_c))^J2*(IN_con.L_h/(IN_con.L_p))^J3*(IN_con.F_p/(IN_con.P_l))^J4*(IN_con.P_l/(IN_con.P_t))^(-1.724)+SMOOTH(1100,900,Re_Dc)*1.1373*Re_Dc^J5*(IN_con.F_p/(IN_con.P_l))^J6*(IN_con.L_h/(IN_con.L_p))^J7*(IN_con.P_l/(IN_con.P_t))^J8*IN_con.N^0.3545;
        end if;
        kc:=SMOOTH(100,0,Re_Dc)*j*Re_Dc*Pr^(1/(3))*IN_var.lambda/(IN_con.D_c);
      elseif IN_con.geometry==TYP.SlitFin then
        J1:=-0.674+0.1316*IN_con.N/(log(Re_Dc))-0.3769*IN_con.F_p/(IN_con.D_c)-1.8857*IN_con.N/(Re_Dc);
        J2:=-0.0178+0.996*IN_con.N/(log(Re_Dc))+26.7*IN_con.N/(Re_Dc);
        J3:=1.865+1244.03*IN_con.F_p/(Re_Dc*IN_con.D_c)-14.37/(log(Re_Dc));
        j:=1.6409*Re_Dc^J1*(IN_con.S_p/(IN_con.S_h))^1.16*(IN_con.P_t/(IN_con.P_l))^1.37*(IN_con.F_p/(IN_con.D_c))^J2*IN_con.N^J3;
        kc:=j*Re_Dc*Pr^(1/(3))*IN_var.lambda/(IN_con.D_c);
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_roundTube_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.HTXGeometry_roundTubes geometry=Dissipation.Utilities.Types.HTXGeometry_roundTubes.PlainFin "Choice of fin geometry" annotation(Dialog(group="HeatExchanger"));
    SI.Area A_fr=0 "Frontal area" annotation(Dialog(group="HeatExchanger"));
    SI.Length D_c=0 "Fin collar diameter" annotation(Dialog(group="HeatExchanger"));
    SI.Length F_p=0 "Fin pitch, fin spacing + fin thickness" annotation(Dialog(group="HeatExchanger"));
    SI.Length L=0 "Heat exchanger length" annotation(Dialog(group="HeatExchanger",enable=(if geometry==2 then true else false)));
    SI.Length L_h=0 "Louver height" annotation(Dialog(group="HeatExchanger",enable=(if geometry==2 then true else false)));
    SI.Length L_p=0 "Louver pitch" annotation(Dialog(group="HeatExchanger",enable=(if geometry==2 then true else false)));
    Integer N=0 "Number of tube rows" annotation(Dialog(group="HeatExchanger",enable=(if ((geometry==1) or (geometry==2)) or (geometry==3) then true else false)));
    SI.Length P_d=0 "Pattern depth of wavy fin, wave height" annotation(Dialog(group="HeatExchanger",enable=(if geometry==4 then true else false)));
    SI.Length P_l=0 "Longitudinal tube pitch" annotation(Dialog(group="HeatExchanger",enable=(if ((geometry==1) or (geometry==2)) or (geometry==3) then true else false)));
    SI.Length P_t=0 "Transverse tube pitch" annotation(Dialog(group="HeatExchanger"));
    SI.Length S_h=0 "Slit height" annotation(Dialog(group="HeatExchanger",enable=(if geometry==3 then true else false)));
    SI.Length S_p=0 "Slit pitch" annotation(Dialog(group="HeatExchanger",enable=(if geometry==3 then true else false)));
    SI.Length X_f=0 "Half wave length of wavy fin" annotation(Dialog(group="HeatExchanger",enable=(if geometry==4 then true else false)));
...
<a href=\"Modelica://Modelica.Fluid.Dissipation.HeatTransfer.HeatExchanger.kc_roundTube_KC\"> kc_roundTube_KC</a>.
   ...
   // components of kc_roundTube_IN_var
    SI.MassFlowRate m_flow annotation(Dialog(group="Input"));
...
"));
  ...
  package HelicalPipe
   ...
   // components of kc_laminar
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_laminar_IN_con IN_con "Input record for function kc_laminar" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_laminar_IN_var IN_var "Input record for function kc_laminar" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
   ...
   // algorithms and equations of kc_laminar
   algorithm
      Pr:=abs(IN_var.eta*IN_var.cp/(max(MIN,IN_var.lambda)));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_laminar_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_laminar_IN_con IN_con "Input record for function kc_laminar_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_laminar_IN_var IN_var "Input record for function kc_laminar_KC" annotation(Dialog(group="Variable inputs"));
   ...
   // algorithms and equations of kc_laminar_KC
   algorithm
      kc:=IN_var.lambda/(IN_con.d_hyd)*(3.66+0.08*(1+0.8*(IN_con.d_hyd/(d_coil))^0.9)*Re^m*Pr^(1/(3)));
...
<a href=\"Modelica://Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_laminar_KC\"> kc_laminar_KC</a>.
   ...
   // components of kc_overall
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_overall_IN_con IN_con "Input record for function kc_overall" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_overall_IN_var IN_var "Input record for function kc_overall" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
   ...
   // algorithms and equations of kc_overall
   algorithm
      Pr:=abs(IN_var.eta*IN_var.cp/(max(MIN,IN_var.lambda)));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_overall_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_overall_IN_con IN_con "Input record for function kc_overall_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_overall_IN_var IN_var "Input record for function kc_overall_KC" annotation(Dialog(group="Variable inputs"));
   ...
   protected
    ...
    SI.Velocity velocity=abs(IN_var.m_flow)/(max(MIN,IN_var.rho*A_cross)) "Mean velocity";
    SI.ReynoldsNumber Re=max(1e-3,IN_var.rho*velocity*IN_con.d_hyd/(max(MIN,IN_var.eta)));
    SI.PrandtlNumber Pr=abs(IN_var.eta*IN_var.cp/(max(MIN,IN_var.lambda)));

   // algorithms and equations of kc_overall_KC
   algorithm
      kc:=SMOOTH(Re_crit,turbulent,Re)*Dissipation.HeatTransfer.HelicalPipe.kc_laminar_KC(IN_con,IN_var)+SMOOTH(turbulent,Re_crit,Re)*Dissipation.HeatTransfer.HelicalPipe.kc_turbulent_KC(IN_con,IN_var);
...
<a href=\"Modelica://Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_overall_KC\"> kc_overall_KC</a>.
   ...
   // components of kc_overall_IN_var
    SI.MassFlowRate m_flow annotation(Dialog(group="Input"));
...
<a href=\"Modelica://Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_overall_KC\"> kc_overall_KC</a>.
   ...
   // components of kc_turbulent
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_con IN_con "Input record for function  kc_turbulent" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_var IN_var "Input record for function  kc_turbulent" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
   ...
   // algorithms and equations of kc_turbulent
   algorithm
      Pr:=abs(IN_var.eta*IN_var.cp/(IN_var.lambda));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_turbulent_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_con IN_con "Input record for function kc_turbulent_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_var IN_var "Input record for function kc_turbulent_KC" annotation(Dialog(group="Variable inputs"));
   ...
   protected
    ...
    SI.ReynoldsNumber Re=max(1e-3,IN_var.rho*velocity*IN_con.d_hyd/(max(MIN,IN_var.eta)));
    SI.PrandtlNumber Pr=abs(IN_var.eta*IN_var.cp/(max(MIN,IN_var.lambda)));
    Real zeta_TOT=0.3164*(max(turbulent,Re))^(-0.25)+0.03*sqrt(IN_con.d_hyd/(d_coil)) "Pressure loss coefficient";

   // algorithms and equations of kc_turbulent_KC
   algorithm
      kc:=IN_var.lambda/(IN_con.d_hyd)*zeta_TOT/(8)*Re*Pr/(1+12.7*sqrt(zeta_TOT/(8))*(Pr^(2/(3))-1));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.HelicalPipe.kc_overall\">See more information</a>.</p>
  ...
  package Plate
   ...
   // components of kc_laminar
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_laminar_IN_con IN_con "Input record for function kc_laminar" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_laminar_IN_var IN_var "Input record for function kc_laminar" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_laminar_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_laminar_IN_con IN_con "Input record for function kc_laminar_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_laminar_IN_var IN_var "Input record for function kc_laminar_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_laminar_KC";
   protected
    Real MIN=Modelica.Constants.eps;
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_laminar_KC\">  kc_laminar_KC</a>.
   ...
   // components of kc_overall
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_overall_IN_con IN_con "Input record for function kc_overall" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_overall_IN_var IN_var "Input record for function kc_overall" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_overall_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_overall_IN_con IN_con "Input record for function kc_overall_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_overall_IN_var IN_var "Input record for function kc_overall_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_overall_KC";
   protected
    SI.CoefficientOfHeatTransfer kc_lam=Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_laminar_KC(IN_con,IN_var);
    SI.CoefficientOfHeatTransfer kc_turb=Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_turbulent_KC(IN_con,IN_var);
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_overall_KC\"> kc_overall_KC</a>.
   ...
   // components of kc_turbulent
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_turbulent_IN_con IN_con "Input record for function kc_turbulent" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_turbulent_IN_var IN_var "Input record for function kc_turbulent" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_turbulent_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_turbulent_IN_con IN_con "Input record for function kc_turbulent_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.Plate.kc_turbulent_IN_var IN_var "Input record for function kc_turbulent_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_turbulent_KC";
   protected
    Real MIN=Modelica.Constants.eps;
    SI.Length L=max(MIN,IN_con.L);
   ...
   // algorithms and equations of kc_turbulent_KC
   algorithm
      kc:=abs(lambda/(L))*0.037*Re^0.8*Pr/(1+2.443/(max(Re^0.1,1e-6))*(Pr^(2/(3))-1));
...
Calculation of the mean convective heat transfer coefficient <b> kc </b> for an laminar and turbulent fluid flow over an even surface. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.Plate.kc_overall\">See more information</a>.
  ...
  package StraightPipe
   ...
   // components of kc_laminar
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_laminar_IN_con IN_con "Input record for function kc_laminar" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_laminar_IN_var IN_var "Input record for function kc_laminar" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // locally defined classes in kc_laminar_KC

   // components of kc_laminar_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_laminar_IN_con IN_con "Input record for function kc_laminar_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_laminar_IN_var IN_var "Input record for function kc_laminar_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_laminar_KC";
   protected
        type TYP = Modelica.Fluid.Dissipation.Utilities.Types.HeatTransferBoundary;
   protected
    Real MIN=Modelica.Constants.eps;
    SI.Area A_cross=PI*IN_con.d_hyd^(2)/(4) "Circular cross sectional area";
    Real Nu0=(if (IN_con.target==TYP.UWTuDFF) or (IN_con.target==TYP.UWTuUFF) then 0.7 else (if (IN_con.target==TYP.UHFuDFF) or (IN_con.target==TYP.UHFuUFF) then 0.6 else 0)) "Help variable for mean Nusselt number";
    Real Nu1=(if (IN_con.target==TYP.UWTuDFF) or (IN_con.target==TYP.UWTuUFF) then 3.66 else (if (IN_con.target==TYP.UHFuDFF) or (IN_con.target==TYP.UHFuUFF) then 4.364 else 0)) "Help variable for mean Nusselt number";
    SI.Velocity velocity=abs(IN_var.m_flow)/(max(MIN,IN_var.rho*A_cross)) "Mean velocity";
    SI.ReynoldsNumber Re=max(1e-3,IN_var.rho*velocity*IN_con.d_hyd/(max(MIN,IN_var.eta)));
    SI.PrandtlNumber Pr=abs(IN_var.eta*IN_var.cp/(max(MIN,IN_var.lambda)));
    SI.NusseltNumber Nu2=(if (IN_con.target==TYP.UWTuDFF) or (IN_con.target==TYP.UWTuUFF) then 1.615*(Re*Pr*IN_con.d_hyd/(IN_con.L))^(1/(3)) else (if (IN_con.target==TYP.UHFuDFF) or (IN_con.target==TYP.UHFuUFF) then 1.953*(Re*Pr*IN_con.d_hyd/(IN_con.L))^(1/(3)) else 0)) "Help variable for mean Nusselt number";
    SI.NusseltNumber Nu3=(if IN_con.target==TYP.UWTuUFF then (2/(1+22*Pr))^(1/(6))*(Re*Pr*IN_con.d_hyd/(IN_con.L))^0.5 else (if IN_con.target==TYP.UHFuUFF then 1.953*(Re*Pr*IN_con.d_hyd/(IN_con.L))^(1/(3)) else 0)) "Help variable for mean Nusselt number";
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_laminar_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.HeatTransferBoundary target=Dissipation.Utilities.Types.HeatTransferBoundary.UWTuDFF "Choice of heat transfer boundary condition" annotation(Dialog(group="Choices"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_laminar_KC\"> kc_laminar_KC</a>.
   ...
   // locally defined classes in kc_overall

   // components of kc_overall
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_overall_IN_con IN_con "Input record for function kc_overall" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_overall_IN_var IN_var "Input record for function kc_overall" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_overall_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_overall_IN_con IN_con "Input record for function kc_overall_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_overall_IN_var IN_var "Input record for function kc_overall_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_overall_KC";
   protected
    Real MIN=Modelica.Constants.eps;
    Real laminar=2200 "Maximum Reynolds number for laminar regime";
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_overall_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.HeatTransferBoundary target=Dissipation.Utilities.Types.HeatTransferBoundary.UWTuDFF "Choice of heat transfer boundary condition" annotation(Dialog(group="Choices"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_overall_KC\"> kc_overall_KC</a>.
   ...
   // locally defined classes in kc_turbulent

   // components of kc_turbulent
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_turbulent_IN_con IN_con "Input record for function kc_turbulent" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_turbulent_IN_var IN_var "Input record for function kc_turbulent" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Convective heat transfer coefficient" annotation(Dialog(group="Output"));
    output SI.PrandtlNumber Pr "Prandl number" annotation(Dialog(group="Output"));
    output SI.ReynoldsNumber Re "Reynolds number" annotation(Dialog(group="Output"));
    output SI.NusseltNumber Nu "Nusselt number" annotation(Dialog(group="Output"));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // locally defined classes in kc_turbulent_KC

   // components of kc_turbulent_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_turbulent_IN_con IN_con "Input record for function kc_turbulent_KC" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_turbulent_IN_var IN_var "Input record for function kc_turbulent_KC" annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Output for function kc_turbulent_KC";
   protected
        type TYP = Modelica.Fluid.Dissipation.Utilities.Types.Roughness;
   protected
   ...
   // algorithms and equations of kc_turbulent_KC
   algorithm
      kc:=(if IN_con.roughness==TYP.Neglected then abs(IN_var.lambda/(IN_con.d_hyd))*0.023*Re^0.8*Pr^(1/(3)) else (if IN_con.roughness==TYP.Considered then abs(IN_var.lambda/(IN_con.d_hyd))*abs(zeta)/(8)*abs(Re)*abs(Pr)/(1+12.7*(abs(zeta)/(8))^0.5*((abs(Pr))^(2/(3))-1))*(1+(IN_con.d_hyd/(IN_con.L))^(2/(3))) else 0));
...
"),smoothOrder(__Dymola_normallyConstant=IN_con)=2);
   ...
   // components of kc_turbulent_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.Roughness roughness=Dissipation.Utilities.Types.Roughness.Considered "Choice of considering surface roughness" annotation(Dialog(group="Choices"));
    SI.Length K=0 "Roughness (average height of surface asperities)" annotation(Dialog(group="Straight pipe",enable=roughness==Modelica.Fluid.Dissipation.Utilities.Types.Roughness.considered));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_turbulent_KC\">kc_turbulent_KC</a>.
   ...
   // components of kc_twoPhaseOverall_KC
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_twoPhaseOverall_KC_IN_con IN_con annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_twoPhaseOverall_KC_IN_var IN_var annotation(Dialog(group="Variable inputs"));
    output SI.CoefficientOfHeatTransfer kc "Local two phase heat transfer coefficient";
   protected
    Real MIN=Modelica.Constants.eps;

   // algorithms and equations of kc_twoPhaseOverall_KC
   algorithm
      kc:=(if IN_con.target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor then Modelica.Fluid.Dissipation.Utilities.Functions.HeatTransfer.TwoPhase.kc_twoPhase_boilingHorizontal_KC(IN_con,IN_var) else (if IN_con.target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilVer then Modelica.Fluid.Dissipation.Utilities.Functions.HeatTransfer.TwoPhase.kc_twoPhase_boilingVertical_KC(IN_con,IN_var) else (if IN_con.target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.CondHor then Modelica.Fluid.Dissipation.Utilities.Functions.HeatTransfer.TwoPhase.kc_twoPhase_condensationHorizontal_KC(IN_con,IN_var) else MIN)));
...
<h5>Overall flow</h5>
<p>Calculation of mean convective heat transfer coefficient <b> kc </b> of a straight pipe at an uniform wall temperature <b> or </b> uniform heat flux <b> and </b> for a hydrodynamically developed <b>or</b> undeveloped laminar and turbulent fluid flow with neglect <b> or </b> consideration of pressure loss influence. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.StraightPipe.kc_overall\">See more information</a>.</p>

<h5>Overall two phase flow</h5>
<p>Calculation of local <b>two phase</b> heat transfer coefficient <b>kc_2ph </b>for (horizontal/vertical) <b>boiling</b> or (horizontal) <b>condensation</b> for an overall flow regime. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.HeatTransfer.StraightPipe.kc_twoPhaseOverall\">See more information</a>.</p>
</html>"));
  end StraightPipe;
 ...
 package PressureLoss
  ...
  package Bend
   ...
   // components of dp_curvedOverall_DP
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_IN_con IN_con "Input record for function dp_curvedOverall_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_IN_var IN_var "Input record for function dp_curvedOverall_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    SI.ReynoldsNumber Re_lam_leave=min(Re_lam_max,max(1e2,754*Modelica.Math.exp((if k<=0.007 then 0.0065/(0.007) else 0.0065/(k))))) "Start of transition regime for increasing Reynolds number (leaving laminar regime)";
    Real A1=(if delta<=70 then 0.9*sin(delta*PI/(180)) else (if delta>=100 then 0.7+0.35*delta/(90) else 1.0)) "Coefficient considering effect for angle of turning on zeta_LOC";
    Real A2=(if frac_RD>2.0 then 6e2 else (if (frac_RD<=2.0) and (frac_RD>=0.7) then (if frac_RD>1.0 then 1e3 else (if (frac_RD<=1.0) and (frac_RD>0.7) then 3e3 else 6e3)) else 4e3)) "Coefficient considering laminar regime on zeta_LOC";
    ...
    Real C_Re=(if frac_RD>0.7 then 11.5/(Re^0.19) else (if (frac_RD<=0.7) and (frac_RD>=0.55) then 5.45/(Re^0.131) else 1+4400/(Re))) "Correction factor for hydraulically rough turbulent regime (Re_turb_min < Re < Re_turb_max)";
    TYP.LocalResistanceCoefficient zeta_LOC_sharp=(if Re<Re_lam_leave then A2/(Re)+zeta_LOC_sharp_turb else (if Re<Re_turb_min then SMOOTH(Re_lam_leave,Re_turb_min,Re)*(A2/(max(Re_lam_leave,Re))+zeta_LOC_sharp_turb)+SMOOTH(Re_turb_min,Re_lam_leave,Re)*C_Re*zeta_LOC_sharp_turb else (if Re<Re_turb_max then SMOOTH(Re_turb_min,Re_turb_max,Re)*C_Re*zeta_LOC_sharp_turb+SMOOTH(Re_turb_max,Re_turb_min,Re)*zeta_LOC_sharp_turb else zeta_LOC_sharp_turb))) "Local resistance coefficient for R_0/d_hyd < 3";
    TYP.LocalResistanceCoefficient zeta_LOC=zeta_LOC_sharp "Local resistance coefficient";
    TYP.DarcyFrictionFactor lambda_FRI_rough=0.25/((Modelica.Math.log10(k/(3.7*IN_con.d_hyd)+5.74/((min(1e6,max(Re_lam_leave,Re)))^0.9)))^(2)) "Darcy friction factor considering surface roughness";
    Real CF_fri=1+SMOOTH(Re_lam_max,Re_lam_leave,Re)*min(1.4,lambda_FRI_rough*L/(d_hyd)/(zeta_LOC))+SMOOTH(Re_lam_leave,Re_lam_max,Re) "Correction factor for surface roughness";
    TYP.PressureLossCoefficient zeta_TOT=max(1,CF_fri)*zeta_LOC "Pressure loss coefficient";

   // algorithms and equations of dp_curvedOverall_DP
   algorithm
      DP:=zeta_TOT*IN_var.rho/(2)*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(m_flow,m_flow_smooth,2)/(max(MIN,(IN_var.rho*A_cross)^(2)));

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_curvedOverall_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
...
<p>Generally this function is numerically best used for the <b>incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated. On the other hand the function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_MFLOW\">dp_curvedOverall_MFLOW</a> is numerically best used for the <b>compressible case </b>if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.Bend.dp_curvedOverall\">See more information</a>.</p>
   ...
   // components of dp_curvedOverall_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_IN_con IN_con "Input record for function dp_curvedOverall_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_IN_var IN_var "Input record for function dp_curvedOverall_MFLOW" annotation(Dialog(group="Variable inputs"));
   ...
   protected
    ...
    SI.ReynoldsNumber Re_turb_const=1e6 "Reynolds number for independence on pressure loss coefficient (1e6)";
    SI.ReynoldsNumber Re_lam_leave=min(Re_lam_max,max(1e2,754*Modelica.Math.exp((if k<=0.007 then 0.0065/(0.007) else 0.0065/(k))))) "Start of transition regime for increasing Reynolds number (leaving laminar regime)";
    Real A1=(if delta<=70 then 0.9*sin(delta/(180)*PI) else (if delta>=100 then 0.7+0.35*delta/(90) else 1.0)) "Coefficient considering effect for angle of turning on zeta_LOC";
    Real A2=(if frac_RD>2.0 then 6e2 else (if (frac_RD<=2.0) and (frac_RD>=0.7) then (if frac_RD>1.0 then 1e3 else (if (frac_RD<=1.0) and (frac_RD>0.7) then 3e3 else 6e3)) else 4e3)) "Coefficient considering laminar regime on zeta_LOC";
    Real B1=(if frac_RD>=1.0 then 0.21*frac_RD^(-0.5) else 0.21*frac_RD^(-2.5)) "Coefficient considering relative curvature radius (R_0/d_hyd) on zeta_LOC";
    Real C1=1.0 "Considering relative elongation of cross sectional area on zeta_LOC (here: circular cross sectional area)";
    TYP.LocalResistanceCoefficient zeta_LOC_sharp_turb=max(MIN,A1*B1*C1) "Local resistance coefficient for turbulent regime (Re > Re_turb_max)";
    SI.AbsolutePressure dp_lam_max=(zeta_LOC_sharp_turb+A2/(Re_lam_leave))*IN_var.rho/(2)*(Re_lam_leave*IN_var.eta/(IN_var.rho*d_hyd))^(2) "Maximum pressure loss for laminar regime";
    SI.AbsolutePressure dp_turb_min=zeta_LOC_sharp_turb*(if frac_RD>0.7 then 11.5/(Re_turb_min^0.19) else (if (frac_RD<=0.7) and (frac_RD>=0.55) then 5.45/(Re_turb_min^0.131) else 1+4400/(Re_turb_min)))*IN_var.rho/(2)*(Re_turb_min*IN_var.eta/(IN_var.rho*d_hyd))^(2) "Minimum pressure loss for turbulent regime";
    SI.AbsolutePressure dp_turb_max=zeta_LOC_sharp_turb*(if frac_RD>0.7 then 11.5/(Re_turb_max^0.19) else (if (frac_RD<=0.7) and (frac_RD>=0.55) then 5.45/(Re_turb_max^0.131) else 1+4400/(Re_turb_max)))*IN_var.rho/(2)*(Re_turb_max*IN_var.eta/(IN_var.rho*d_hyd))^(2) "Maximum pressure loss for turbulent regime";
    SI.AbsolutePressure dp_turb_const=zeta_LOC_sharp_turb*IN_var.rho/(2)*(Re_turb_const*IN_var.eta/(IN_var.rho*d_hyd))^(2) "Pressure loss for independence of Reynolds number on pressure loss coefficient";
    SI.Velocity v_lam=((-A2)/(2)*IN_var.eta+0.5*sqrt(max(MIN,(A2*IN_var.eta)^(2)+8*zeta_LOC_sharp_turb*abs(dp)*IN_var.rho*d_hyd^(2))))/(zeta_LOC_sharp_turb)/(IN_var.rho)/(d_hyd) "Mean velocity in laminar regime (Re < Re_lam_leave)";
    SI.Velocity v_tra=((-A2)/(2)*IN_var.eta+0.5*sqrt(max(MIN,(A2*IN_var.eta)^(2)+8*zeta_LOC_sharp_turb*abs(dp_lam_max)*IN_var.rho*d_hyd^(2))))/(zeta_LOC_sharp_turb)/(IN_var.rho)/(d_hyd) "Mean velocity in transition regime (Re_lam_leave < Re_turb_min)";
    SI.Velocity v_turb=(if frac_RD>0.7 then (max(MIN,abs(dp))/(IN_var.rho/(2)*11.5*zeta_LOC_sharp_turb)*(IN_var.rho*IN_con.d_hyd/(max(MIN,IN_var.eta)))^0.19)^(1/(2-0.19)) else (if (frac_RD>0.55) and (frac_RD<0.7) then (max(MIN,abs(dp))/(IN_var.rho/(2)*5.45*zeta_LOC_sharp_turb)*(IN_var.rho*IN_con.d_hyd/(max(MIN,IN_var.eta)))^0.131)^(1/(2-0.131)) else (-2200)/(IN_var.rho*IN_con.d_hyd/(IN_var.eta))+(((-2200)/(IN_var.rho*IN_con.d_hyd/(max(MIN,IN_var.eta))))^(2)+2*abs(max(MIN,dp))/(max(MIN,IN_var.rho)))^0.5)) "Mean velocity in turbulent regime with dependence on pressure loss coefficient (Re_turb_min < Re < Re_turb_max)";
    SI.Velocity v_turb_const=sqrt(max(MIN,2*abs(dp)/(IN_var.rho*zeta_LOC_sharp_turb))) "Mean velocity in turbulent regime with independence on pressure loss coefficient (Re > Re_turb_max)";
    SI.Velocity v_smooth=(if dp<dp_lam_max then v_lam else (if dp<dp_turb_min then SMOOTH(dp_lam_max,dp_turb_min,dp)*v_lam+SMOOTH(dp_turb_min,dp_lam_max,dp)*v_turb else (if dp<dp_turb_max then v_turb else SMOOTH(dp_turb_max,dp_turb_const,dp)*v_turb+SMOOTH(dp_turb_const,dp_turb_max,dp)*v_turb_const))) "Mean velocity under smooth conditions for R_0/d_hyd < 3";
    SI.ReynoldsNumber Re_smooth=max(Re_min,IN_var.rho*v_smooth*d_hyd/(IN_var.eta)) "Reynolds number under smooth conditions";
    TYP.DarcyFrictionFactor lambda_FRI_rough=0.25/((Modelica.Math.log10(k/(3.7*IN_con.d_hyd)+5.74/((min(1e6,max(Re_lam_leave,Re_smooth)))^0.9)))^(2)) "Darcy friction factor considering surface roughness";
    TYP.DarcyFrictionFactor lambda_FRI_smooth=0.25/((Modelica.Math.log10(5.74/((max(Re_lam_leave,Re_smooth))^0.9)))^(2)) "Darcy friction factor neglecting surface roughness";
    Real CF_3=1+SMOOTH(6e3,1e3,Re_smooth)*min(1.4,lambda_FRI_rough*L/(d_hyd)/(zeta_LOC_sharp_turb))+SMOOTH(1e3,6e3,Re_smooth) "Correction factor for surface roughness";
    SI.Velocity velocity=v_smooth/((max(1,CF_3))^0.5) "Corrected velocity considering surface roughness";

   // algorithms and equations of dp_curvedOverall_MFLOW
   algorithm
      M_FLOW:=sign(dp)*IN_var.rho*A_cross*abs(velocity);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_curvedOverall_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
...
and <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_curvedOverall_MFLOW\"> dp_curvedOverall_MFLOW </a>.
   ...
   // components of dp_edgedOverall_DP
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_edgedOverall_IN_con IN_con "Input record for function dp_edgedOverall_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_edgedOverall_IN_var IN_var "Input record for function dp_edgedOverall_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    Real delta=IN_con.delta*180/(PI) "Angle of turning";
    SI.ReynoldsNumber Re_min=1 "Minium Reynolds number";
    SI.ReynoldsNumber Re_lam_min=1e3 "Minimum Reynolds number for laminar regime (1e2)";
    SI.ReynoldsNumber Re_lam_max=4e4 "Maximum Reynolds number for laminar regime (1e3)";
    SI.ReynoldsNumber Re_turb_min=1e5 "Minimum Reynolds number for turbulent regime (1e5)";
    SI.ReynoldsNumber Re_turb_max=3e5 "Maximum Reynolds number for turbulent regime (2e5)";
    SI.ReynoldsNumber Re_turb_const=1e6 "Reynolds number for independence on pressure loss coefficient (1e6)";
    SI.ReynoldsNumber Re_lam_leave=min(Re_lam_max,max(Re_lam_min,754*Modelica.Math.exp((if k<=0.007 then 0.0065/(0.007) else 0.0065/(k))))) "Start of transition regime for increasing Reynolds number (leaving laminar regime)";
    ...
    TYP.LocalResistanceCoefficient zeta_LOC=max(MIN,0.95*(sin(PI/(180)*delta/(2)))^(2)+2.05*(sin(PI/(180)*delta/(2)))^(4)) "Local resistance coefficient";
    Real B=0.0292*delta^(2)+1.1995*delta "Coefficient considering effect of Reynolds number in laminar regime";
    ...
    TYP.DarcyFrictionFactor lambda_FRI_rough=0.25/((Modelica.Math.log10(k/(3.7*IN_con.d_hyd)+5.74/((min(Re_turb_const,max(Re_lam_leave,Re)))^0.9)))^(2)) "Darcy friction factor considering surface roughness";
    TYP.DarcyFrictionFactor lambda_FRI_smooth=0.25/((Modelica.Math.log10(5.74/((min(Re_turb_const,max(Re_lam_leave,Re)))^0.9)))^(2)) "Darcy friction factor neglecting surface roughness";
    Real CF_fri=(if delta<=45 then max(1,min(1.4,lambda_FRI_rough/(lambda_FRI_smooth))) else 1) "Correction factor for surface roughness";
    Real CF_Re=SMOOTH(Re_min,Re_lam_leave,Re)*B/(Re^exp)+SMOOTH(Re_lam_leave,Re_min,Re) "Correction factor for Reynolds number";
    TYP.PressureLossCoefficient zeta_TOT=A*C1*zeta_LOC*CF_fri*CF_Re "Pressure loss coefficient";

   // algorithms and equations of dp_edgedOverall_DP
   algorithm
      DP:=zeta_TOT*IN_var.rho/(2)*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(velocity,v_min,2);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_edgedOverall_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
<a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.Bend.dp_edgedOverall\">See more information</a> .</p>
   ...
   // components of dp_edgedOverall_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_edgedOverall_IN_con IN_con "Input record for function dp_edgedOverall_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Bend.dp_edgedOverall_IN_var IN_var "Input record for function dp_edgedOverall_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    Real C1=1 "Considering relative elongation of cross sectional area on zeta_LOC (here: circular cross sectional area)";
    TYP.LocalResistanceCoefficient zeta_LOC=max(MIN,0.95*(sin(PI/(180)*delta/(2)))^(2)+2.05*(sin(PI/(180)*delta/(2)))^(4)) "Local resistance coefficient";
    Real B=0.0292*delta^(2)+1.1995*delta "Coefficient considering effect of Reynolds number on zeta_TOT";
    Real exp=1 "Exponent for Reynolds number correction in laminar regime";
    Real pow=2-exp "pressure loss = f(mass flow rate^pow)";
    SI.Pressure dp_min=1 "Linear smoothing of mass flow rate for decreasing pressure loss";
    SI.Velocity v_lam=Re_lam_leave*IN_var.eta/(IN_var.rho*d_hyd) "Maximum mean velocity in laminar regime (Re < Re_lam_leave)";
    Real dp_lam=A*C1*zeta_LOC*B/(2)*(d_hyd/(IN_var.eta))^(-exp)*IN_var.rho^(1-exp)*v_lam^pow "Maximum pressure loss in laminar regime (Re < Re_lam_leave)";
    SI.Velocity v_smooth=(if abs(dp)>dp_lam then (2*abs(dp))^0.5*(A*C1*zeta_LOC*IN_var.rho)^(-0.5) else (2*(d_hyd/(IN_var.eta))^exp/(A*C1*zeta_LOC*B*IN_var.rho^(1-exp)))^(1/(pow))*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(abs(dp),dp_min,1/(pow))) "Mean velocity under smooth conditions";
    SI.ReynoldsNumber Re_smooth=max(Re_min,IN_var.rho*v_smooth*d_hyd/(IN_var.eta)) "Reynolds number under smooth conditions";
    TYP.DarcyFrictionFactor lambda_FRI_rough=0.25/((Modelica.Math.log10(k/(3.7*IN_con.d_hyd)+5.74/((min(Re_turb_const,max(Re_lam_leave,Re_smooth)))^0.9)))^(2)) "Darcy friction factor considering surface roughness";
    TYP.DarcyFrictionFactor lambda_FRI_smooth=0.25/((Modelica.Math.log10(5.74/((min(Re_turb_const,max(Re_lam_leave,Re_smooth)))^0.9)))^(2)) "Darcy friction factor neglecting surface roughness";
    Real CF_fri=(if delta<=45 then max(1,min(1.4,lambda_FRI_rough/(lambda_FRI_smooth))) else 1) "Correction factor for surface roughness";
    SI.Velocity velocity=v_smooth/((max(1,CF_fri))^0.5) "Corrected velocity considering surface roughness";

   // algorithms and equations of dp_edgedOverall_MFLOW
   algorithm
      M_FLOW:=sign(dp)*IN_var.rho*A_cross*velocity;

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_edgedOverall_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of pressure loss in edged bends with sharp corners at overall flow regime for incompressible and single-phase fluid flow through circular cross sectional area considering surface roughness.
</p>
...
<a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.Bend.dp_edgedOverall\">See more information</a> .</p>
  ...
  package Channel
   ...
   // locally defined classes in dp_internalFlowOverall_DP

   // components of dp_internalFlowOverall_DP
    input Modelica.Fluid.Dissipation.PressureLoss.Channel.dp_internalFlowOverall_IN_con IN_con "Input record for function dp_internalFlowOverall_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Channel.dp_internalFlowOverall_IN_var IN_var "Input record for function dp_internalFlowOverall_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
    output SI.Pressure DP "Output for function dp_internalFlowOverall_DP";
   protected
        type TYP = Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow;
   protected
    Real MIN=Modelica.Constants.eps;
    SI.Area A_cross=max(MIN,(if IN_con.geometry==TYP.Annular then PI/(4)*(IN_con.D_ann^(2)-IN_con.d_ann^(2)) else (if IN_con.geometry==TYP.Circular then PI/(4)*IN_con.d_cir^(2) else (if IN_con.geometry==TYP.Elliptical then PI*IN_con.a_ell*IN_con.b_ell else (if IN_con.geometry==TYP.Rectangular then IN_con.a_rec*IN_con.b_rec else (if IN_con.geometry==TYP.Isosceles then 0.5*IN_con.a_tri*IN_con.h_tri else 0)))))) "Cross sectional area";
    SI.Length perimeter=max(MIN,(if IN_con.geometry==TYP.Annular then PI*(IN_con.D_ann+IN_con.d_ann) else (if IN_con.geometry==TYP.Circular then PI*IN_con.d_cir else (if IN_con.geometry==TYP.Elliptical then PI*(IN_con.a_ell+IN_con.b_ell) else (if IN_con.geometry==TYP.Rectangular then 2*(IN_con.a_rec+IN_con.b_rec) else (if IN_con.geometry==TYP.Isosceles then IN_con.a_tri+2*(IN_con.h_tri^(2)+(IN_con.a_tri/(2))^(2))^0.5 else 0)))))) "Perimeter";
    SI.Diameter d_hyd=4*A_cross/(perimeter) "Hydraulic diameter";
    Real beta=IN_con.beta*180/(PI) "Top angle";
    Real Dd_ann=min(max(MIN,IN_con.d_ann),IN_con.D_ann)/(max(MIN,max(IN_con.d_ann,IN_con.D_ann))) "Ratio of small to large diameter of annular geometry";
    ...
    Real CF_ell=(-169.2211)*ab_ell^(4)+260.9028*ab_ell^(3)-113.7890*ab_ell^(2)+9.2588*ab_ell^(1)+78.7124 "Correction factor for elliptical geometry";
    Real CF_tri=(-0.0013)*(min(90,beta))^(2)+0.1577*min(90,beta)+48.5575 "Correction factor for triangular geometry";
    Real CF_lam=(if IN_con.geometry==TYP.Annular then CF_ann else (if IN_con.geometry==TYP.Circular then 64 else (if IN_con.geometry==TYP.Elliptical then CF_ell else (if IN_con.geometry==TYP.Rectangular then CF_rec else (if IN_con.geometry==TYP.Isosceles then CF_tri else 0))))) "Correction factor for laminar flow";
    Real k=max(MIN,abs(IN_con.K)/(d_hyd)) "Relative roughness";
    ...
    Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_con IN_2_con(final roughness=IN_con.roughness,final d_hyd=d_hyd,final K=IN_con.K,final L=IN_con.L) "Input record for turbulent regime" annotation(Placement(transformation(extent={{-100,20},{-80,40}})));
    Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_var IN_2_var(final eta=IN_var.eta,final rho=IN_var.rho) "Input record for turbulent regime" annotation(Placement(transformation(extent={{-80,20},{-60,40}})));

   // algorithms and equations of dp_internalFlowOverall_DP
   algorithm
      DP:=SMOOTH(Re_lam_min,Re_lam_max,Re)*CF_lam/(2)*IN_con.L/(d_hyd^(2))*velocity*IN_var.eta+SMOOTH(Re_lam_max,Re_lam_min,Re)*Dissipation.PressureLoss.StraightPipe.dp_turbulent_DP(IN_2_con,IN_2_var,m_flow_turb);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_internalFlowOverall_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
"));
   ...
   // locally defined classes in dp_internalFlowOverall_MFLOW

   // components of dp_internalFlowOverall_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.Channel.dp_internalFlowOverall_IN_con IN_con "Input record for function dp_internalFlowOverall_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Channel.dp_internalFlowOverall_IN_var IN_var "Input record for function dp_internalFlowOverall_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
    output SI.MassFlowRate M_FLOW "Output of function dp_overall_MFLOW";
   protected
        type TYP1 = Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow;
        type TYP2 = Modelica.Fluid.Dissipation.Utilities.Types.Roughness;
   protected
    Real MIN=Modelica.Constants.eps;
    SI.Area A_cross=max(MIN,(if IN_con.geometry==TYP1.Annular then PI/(4)*(IN_con.D_ann^(2)-IN_con.d_ann^(2)) else (if IN_con.geometry==TYP1.Circular then PI/(4)*IN_con.d_cir^(2) else (if IN_con.geometry==TYP1.Elliptical then PI*IN_con.a_ell*IN_con.b_ell else (if IN_con.geometry==TYP1.Rectangular then IN_con.a_rec*IN_con.b_rec else (if IN_con.geometry==TYP1.Isosceles then 0.5*IN_con.a_tri*IN_con.h_tri else 0)))))) "Cross sectional area";
    SI.Length perimeter=max(MIN,(if IN_con.geometry==TYP1.Annular then PI*(IN_con.D_ann+IN_con.d_ann) else (if IN_con.geometry==TYP1.Circular then PI*IN_con.d_cir else (if IN_con.geometry==TYP1.Elliptical then PI*(IN_con.a_ell+IN_con.b_ell) else (if IN_con.geometry==TYP1.Rectangular then 2*(IN_con.a_rec+IN_con.b_rec) else (if IN_con.geometry==TYP1.Isosceles then IN_con.a_tri+2*(IN_con.h_tri^(2)+(IN_con.a_tri/(2))^(2))^0.5 else 0)))))) "Perimeter";
    SI.Diameter d_hyd=4*A_cross/(perimeter) "Hydraulic diameter";
    Real beta=IN_con.beta*180/(PI) "Top angle";
    Real Dd_ann=min(max(MIN,IN_con.d_ann),IN_con.D_ann)/(max(MIN,max(IN_con.d_ann,IN_con.D_ann))) "Ratio of small to large diameter of annular geometry";
    Real CF_ann=98.7378*Dd_ann^0.0589 "Correction factor for annular geometry";
    ...
    Real CF_ell=(-169.2211)*ab_ell^(4)+260.9028*ab_ell^(3)-113.7890*ab_ell^(2)+9.2588*ab_ell^(1)+78.7124 "Correction factor for elliptical geometry";
    Real CF_tri=(-0.0013)*(min(90,beta))^(2)+0.1577*min(90,beta)+48.5575 "Correction factor for triangular geometry";
    Real CF_lam=(if IN_con.geometry==TYP1.Annular then CF_ann else (if IN_con.geometry==TYP1.Circular then 64 else (if IN_con.geometry==TYP1.Elliptical then CF_ell else (if IN_con.geometry==TYP1.Rectangular then CF_rec else (if IN_con.geometry==TYP1.Isosceles then CF_tri else 0))))) "Correction factor for laminar flow";
    Real k=max(MIN,abs(IN_con.K)/(d_hyd)) "Relative roughness";
    SI.ReynoldsNumber Re_lam_min=1e3 "Minimum Reynolds number for laminar regime";
    SI.ReynoldsNumber Re_lam_max=2090*(1/(max(0.007,k)))^0.0635 "Maximum Reynolds number for laminar regime";
    SI.ReynoldsNumber Re_turb_min=4e3 "Minimum Reynolds number for turbulent regime";
    SI.ReynoldsNumber Re_lam_leave=min(Re_lam_max,max(Re_lam_min,754*Modelica.Math.exp((if k<=0.007 then 0.0065/(0.007) else 0.0065/(k))))) "Start of transition regime for increasing Reynolds number (leaving laminar regime)";
    TYP.DarcyFrictionFactor lambda_FRI_calc=2*abs(dp)*d_hyd^(3)*IN_var.rho/(IN_con.L*IN_var.eta^(2)) "Adapted Darcy friction factor";
    SI.ReynoldsNumber Re_lam=lambda_FRI_calc/(CF_lam) "Reynolds number assuming laminar regime";
    SI.ReynoldsNumber Re_turb=(if IN_con.roughness==TYP2.Neglected then (max(MIN,lambda_FRI_calc)/(0.3164))^(1/(1.75)) else (-2)*sqrt(max(lambda_FRI_calc,MIN))*Modelica.Math.log10(2.51/(sqrt(max(lambda_FRI_calc,MIN)))+k/(3.7))) "Reynolds number assuming turbulent regime";
    SI.ReynoldsNumber Re_check=(if Re_lam<Re_lam_leave then Re_lam else Re_turb);
    SI.ReynoldsNumber Re_trans=(if Re_lam>=Re_lam_leave then Modelica.Fluid.Dissipation.Utilities.Functions.General.CubicInterpolation_DP(Re_check,Re_lam_leave,Re_turb_min,k,lambda_FRI_calc) else 0);
    SI.ReynoldsNumber Re=(if Re_lam<Re_lam_leave then Re_lam else (if Re_turb>Re_turb_min then Re_turb else Re_trans));
    Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_con IN_2_con(final roughness=IN_con.roughness,final d_hyd=d_hyd,final K=IN_con.K,final L=IN_con.L) "Input record for turbulent regime" annotation(Placement(transformation(extent={{-100,20},{-80,40}})));
    Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_var IN_2_var(final eta=IN_var.eta,final rho=IN_var.rho) "Input record for turbulent regime" annotation(Placement(transformation(extent={{-80,20},{-60,40}})));

   // algorithms and equations of dp_internalFlowOverall_MFLOW
   algorithm
      M_FLOW:=SMOOTH(Re_lam_min,Re_turb,Re)*IN_var.rho*A_cross*dp*2/(CF_lam)*d_hyd^(2)/(IN_con.L)*1/(IN_var.eta)+SMOOTH(Re_turb,Re_lam_min,Re)*A_cross/(PI/(4)*d_hyd^(2))*Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_turbulent_MFLOW(IN_2_con,IN_2_var,dp);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_internalFlowOverall_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of pressure loss for an internal flow through different geometries at overall flow regime for incompressible and single-phase fluid flow considering surface roughness.
</p>
...
The pressure loss calculation for internal fluid flow in different geometries is further documented  <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.Channel.dp_internalFlowOverall\">here</a>.
   ...
   // components of dp_internalFlowOverall_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.Roughness roughness=Dissipation.Utilities.Types.Roughness.Considered "Choice of considering surface roughness" annotation(Dialog(group="Channel"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.Channel.dp_internalFlowOverall\">See more information</a>.
  ...
  package General
   ...
   // components of dp_idealGas_DP
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_idealGas_IN_con IN_con "Input record for function dp_idealGas_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_idealGas_IN_var IN_var "Input record for function dp_idealGas_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    SI.VolumeFlowRate V_flow_min=(IN_con.R_s/(Km_internal))^(1/(IN_con.exp))*rho_internal^(1/(IN_con.exp)-1)*IN_con.dp_smooth^(1/(IN_con.exp)) "Start of approximation for decreasing volume flow rate";

   // algorithms and equations of dp_idealGas_DP
   algorithm
      DP:=Km_internal/(IN_con.R_s)*rho_internal^(IN_con.exp-1)*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(V_flow,V_flow_min,IN_con.exp);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_idealGas_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
Generally this  function is numerically best used for the <b> incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated. On the other hand the  function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_idealGas_MFLOW\">dp_idealGas_MFLOW</a> is numerically best used for the <b> compressible case </b> if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated.  <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.General.dp_idealGas\">See more information</a>.
   ...
   // components of dp_idealGas_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_idealGas_IN_con IN_con "Input record for function dp_idealGas_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_idealGas_IN_var IN_var "Input record for function dp_idealGas_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   // algorithms and equations of dp_idealGas_MFLOW
   algorithm
      M_FLOW:=(IN_con.R_s/(Km_internal))^(1/(IN_con.exp))*rho_internal^(1/(IN_con.exp))*Dissipation.Utilities.Functions.General.SmoothPower(dp,IN_con.dp_smooth,1/(IN_con.exp));

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_idealGas_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of a generic pressure loss for an <b> ideal gas </b> using mean density.
</p>
...
Generally this  function is numerically best used for the <b> compressible case </b> if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated. On the other hand the  function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_idealGas_DP\">dp_idealGas_DP</a> is numerically best used for the <b> incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated.  <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.General.dp_idealGas\">See more information</a>.
   ...
   // components of dp_idealGas_IN_con
    SI.Pressure dp_smooth(min=Modelica.Constants.eps)=1 "Start linearisation for smaller pressure loss" annotation(Dialog(group="Linearisation"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_idealGas_MFLOW\"> dp_idealGas_MFLOW</a>.
   ...
   // components of dp_nominalDensityViscosity_DP
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalDensityViscosity_IN_con IN_con "Input record for function dp_nominalDensityViscosity_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalDensityViscosity_IN_var IN_var "Input record for function dp_nominalDensityViscosity_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
    output SI.Pressure DP "Output for function dp_nominalDensityViscosity_DP";
   protected
    SI.MassFlowRate m_flow_smooth=(max(1,0.01*IN_con.dp_nom)*IN_var.rho/(IN_con.rho_nom)*(1/(IN_var.eta)*IN_con.eta_nom)^IN_con.exp_eta*1/(IN_con.m_flow_nom))^(1/(IN_con.exp)) "Start of approximation for decreasing mass flow rate";

   // algorithms and equations of dp_nominalDensityViscosity_DP
   algorithm
      DP:=(if (IN_con.exp>1.0) or (IN_con.exp<1.0) then Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(m_flow,m_flow_smooth,IN_con.exp)*(IN_var.eta/(IN_con.eta_nom))^IN_con.exp_eta*IN_con.rho_nom/(IN_var.rho)*IN_con.dp_nom*(1/(IN_con.m_flow_nom))^IN_con.exp else m_flow/(IN_con.m_flow_nom)*(IN_var.eta/(IN_con.eta_nom))^IN_con.exp_eta*IN_con.rho_nom/(IN_var.rho)*IN_con.dp_nom);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_nominalDensityViscosity_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
"));
   ...
   // components of dp_nominalDensityViscosity_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalDensityViscosity_IN_con IN_con "Input record for function dp_nominalDensityViscosity_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalDensityViscosity_IN_var IN_var "Input record for function dp_nominalDensityViscosity_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
    output SI.MassFlowRate M_FLOW "Output for function dp_nominalDensityViscosity_MFLOW";

   // algorithms and equations of dp_nominalDensityViscosity_MFLOW
   algorithm
      M_FLOW:=(if (IN_con.exp>1.0) or (IN_con.exp<1.0) then Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(dp,0.01*IN_con.dp_nom,1/(IN_con.exp))*(IN_con.eta_nom/(IN_var.eta))^(IN_con.exp_eta/(IN_con.exp))*(1/(IN_con.dp_nom)*IN_var.rho/(IN_con.rho_nom))^(1/(IN_con.exp))*IN_con.m_flow_nom else dp/(IN_con.dp_nom)*(IN_con.eta_nom/(IN_var.eta))^IN_con.exp_eta*IN_var.rho/(IN_con.rho_nom)*IN_con.m_flow_nom);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_nominalDensityViscosity_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of a generic pressure loss in dependence of nominal fluid variables (e.g., nominal density, nominal dynamic viscosity) at an operation point via interpolation.
This generic function considers the pressure loss law via a pressure loss exponent and the influence of density and dynamic viscosity on pressure loss.
</p>
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalDensityViscosity_MFLOW\"> dp_nominalDensityViscosity_MFLOW</a>.
   ...
   // components of dp_nominalPressureLossLawDensity_DP
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalPressureLossLawDensity_IN_con IN_con "Input record for function dp_nominalPressureLossLawDensity_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalPressureLossLawDensity_IN_var IN_var "Input record for function dp_nominalPressureLossLawDensity_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
    output SI.Pressure DP "Output for function dp_nominalPressureLossLawDensity_yesAJac_DP";
   protected
    Real exp_density=(if IN_con.target==Dissipation.Utilities.Types.MassOrVolumeFlowRate.MassFlowRate then 1-IN_con.exp else 1) "Exponent of density fraction (rho/rho_nom)";
    SI.MassFlowRate m_flow_nom=(if IN_con.target==Dissipation.Utilities.Types.MassOrVolumeFlowRate.MassFlowRate then IN_con.m_flow_nom else IN_var.rho*IN_con.V_flow_nom) "Nominal mean flow velocity at operation point";
    SI.MassFlowRate m_flow_linear=(0.01*IN_con.zeta_TOT_nom/(IN_var.zeta_TOT)*(IN_con.rho_nom/(IN_var.rho))^exp_density*(IN_con.A_cross/(IN_con.A_cross_nom))^IN_con.exp*IN_con.m_flow_nom)^(1/(IN_con.exp)) "Start of approximation for decreasing mass flow rate";

   // algorithms and equations of dp_nominalPressureLossLawDensity_DP
   algorithm
      DP:=(if (IN_con.exp>1.0) or (IN_con.exp<1.0) then Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(m_flow,m_flow_linear,IN_con.exp)*IN_con.dp_nom*IN_var.zeta_TOT/(IN_con.zeta_TOT_nom)*(IN_var.rho/(IN_con.rho_nom))^exp_density*(IN_con.A_cross_nom/(IN_con.A_cross))^IN_con.exp*(1/(IN_con.m_flow_nom))^IN_con.exp else IN_con.dp_nom*IN_var.zeta_TOT/(IN_con.zeta_TOT_nom)*(IN_var.rho/(IN_con.rho_nom))^exp_density*(IN_con.A_cross_nom/(IN_con.A_cross))^(1)*(m_flow/(IN_con.m_flow_nom))^(1));

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_nominalPressureLossLawDensity_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
Generally this function is numerically best used for the <b> incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated. On the other hand the function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalPressureLossLawDensity_MFLOW\">dp_nominalPressureLossLawDensity_MFLOW</a> is numerically best used for the <b> compressible case </b> if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.General.dp_nominalPressureLossLawDensity\">See more information</a>.
   ...
   // components of dp_nominalPressureLossLawDensity_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalPressureLossLawDensity_IN_con IN_con "Input record for function dp_nominalPressureLossLawDensity_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalPressureLossLawDensity_IN_var IN_var "Input record for function dp_nominalPressureLossLawDensity_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   // algorithms and equations of dp_nominalPressureLossLawDensity_MFLOW
   algorithm
      M_FLOW:=(if (IN_con.exp>1.0) or (IN_con.exp<1.0) then Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(dp,0.01*IN_con.dp_nom,1/(IN_con.exp))*IN_con.m_flow_nom*IN_con.A_cross/(IN_con.A_cross_nom)*(IN_con.rho_nom/(IN_var.rho))^(exp_density/(IN_con.exp))*(1/(IN_con.dp_nom)*IN_con.zeta_TOT_nom/(IN_var.zeta_TOT))^(1/(IN_con.exp)) else IN_con.m_flow_nom*IN_con.A_cross/(IN_con.A_cross_nom)*(IN_con.rho_nom/(IN_var.rho))^(exp_density/(1))*(dp/(IN_con.dp_nom)*IN_con.zeta_TOT_nom/(IN_var.zeta_TOT))^(1/(1)));

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_nominalPressureLossLawDensity_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of a generic pressure loss in dependence of nominal fluid variables (e.g., nominal density) via interpolation from an operation point.
This generic function considers the pressure loss law via a nominal pressure loss (dp_nom), a pressure loss coefficient (zeta_TOT) and a pressure loss law exponent (exp) as well as the influence of density on pressure loss.
</p>
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalPressureLossLawDensity_MFLOW\"> dp_nominalPressureLosslawDensity_MFLOW</a>.
   ...
   // components of dp_nominalPressureLossLawDensity_IN_var
    SI.Density rho "Density of fluid" annotation(Dialog(group="Fluid properties"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_nominalPressureLossLawDensity_MFLOW\"> dp_nominalPressureLosslawDensity_MFLOW</a>.
   ...
   // components of dp_pressureLossCoefficient_DP
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_pressureLossCoefficient_IN_con IN_con "Input record for dp_pressureLossCoefficient_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_pressureLossCoefficient_IN_var IN_var "Input record for dp_pressureLossCoefficient_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
    output SI.Pressure DP "Output for function dp_pressureLossCoefficient_DP";

   // algorithms and equations of dp_pressureLossCoefficient_DP
   algorithm
      DP:=0.5*IN_var.zeta_TOT*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(m_flow,(IN_con.dp_smooth/(0.5*IN_var.zeta_TOT*IN_var.rho))^0.5*IN_var.rho*IN_con.A_cross,2)/(IN_var.rho*IN_con.A_cross^(2));

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_pressureLossCoefficient_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
Generally this function is numerically best used for the <b> incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated. On the other hand the  function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_pressureLossCoefficient_MFLOW\">dp_pressureLossCoefficient_MFLOW</a> is numerically best used for the <b> compressible case </b> if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.General.dp_pressureLossCoefficient\">See more information</a>.
   ...
   // components of dp_pressureLossCoefficient_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_pressureLossCoefficient_IN_con IN_con "Input record for function dp_pressureLossCoefficient_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_pressureLossCoefficient_IN_var IN_var "Input record for function dp_pressureLossCoefficient_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
    output SI.MassFlowRate M_FLOW "Output for function dp_pressureLossCoefficientt_MFLOW";

   // algorithms and equations of dp_pressureLossCoefficient_MFLOW
   algorithm
      M_FLOW:=IN_var.rho*IN_con.A_cross*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(dp,IN_con.dp_smooth,0.5)/((0.5*IN_var.zeta_TOT*IN_var.rho)^0.5);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=dp_pressureLossCoefficient_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of a generic pressure loss in dependence of  a pressure loss coefficient.
</p>
...
"));
   ...
   // components of dp_pressureLossCoefficient_IN_con
    SI.Area A_cross=Modelica.Constants.pi*0.1^(2)/(4) "Cross sectional area" annotation(Dialog(group="Generic variables"));
    SI.Pressure dp_smooth=1 "Start linearisation for decreasing pressure loss" annotation(Dialog(group="Linearisation"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_pressureLossCoefficient_MFLOW\"> dp_pressureLossCoefficient_MFLOW</a>.
   ...
   // components of dp_pressureLossCoefficient_IN_var
    TYP.PressureLossCoefficient zeta_TOT=0.02*1/(0.1) "Pressure loss coefficient" annotation(Dialog(group="Generic variables"));
    SI.Density rho "Density of fluid" annotation(Dialog(group="FluidProperties"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_pressureLossCoefficient_MFLOW\"> dp_pressureLossCoefficient_MFLOW</a>.
   ...
   // components of dp_volumeFlowRate_DP
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_volumeFlowRate_IN_con IN_con "Input record for function dp_volumeFlowRate_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_volumeFlowRate_IN_var IN_var "Input record for function dp_volumeFlowRate_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    SI.VolumeFlowRate V_flow_smooth=(if IN_con.a>0 then -(b/(2*a)+(((-b)/(2*a))^(2)+dp_min/(a))^0.5) else dp_min/(b)) "Start of approximation for decreasing volume flow rate";

   // algorithms and equations of dp_volumeFlowRate_DP
   algorithm
      DP:=a*Dissipation.Utilities.Functions.General.SmoothPower(V_flow,V_flow_smooth,2)+b*V_flow;

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_volumeFlowRate_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
Generally this  function is numerically best used for the <b> incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated. On the other hand the  function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_volumeFlowRate_MFLOW\">dp_volumeFlowRate_MFLOW</a> is numerically best used for the <b> compressible case </b> if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated. <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.General.dp_volumeFlowRate\">See more information</a>.
   ...
   // components of dp_volumeFlowRate_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_volumeFlowRate_IN_con IN_con "Input record for function dp_volumeFlowRate_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.General.dp_volumeFlowRate_IN_var IN_var "Input record for function dp_volumeFlowRate_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   // algorithms and equations of dp_volumeFlowRate_MFLOW
   algorithm
      M_FLOW:=IN_var.rho*((-b)/(2*a)+Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower((b/(2*a))^(2)+1/(a)*dp,(b/(2*a))^(2)+1/(a)*dp_min,0.5));

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_volumeFlowRate_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of a generic pressure loss with linear or quadratic dependence on volume flow rate.
The function can be used to calculate pressure loss at known mass flow rate <b> or </b> mass flow rate at known pressure loss.
</p>
...
"));
   ...
   // components of dp_volumeFlowRate_IN_con
...
This record is used as <b> input record </b> for the pressure loss function
 ...
 <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.General.dp_volumeFlowRate_MFLOW\"> dp_volumeFlowRate_MFLOW</a>.
   ...
   // components of dp_volumeFlowRate_IN_var
    SI.Density rho "Density of fluid" annotation(Dialog(group="Fluid properties"));
...
Calculation of a generic pressure loss with linear or quadratic dependence on volume flow rate.
 <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.General.dp_volumeFlowRate\">See more information</a>.
  ...
  package Orifice
   ...
   // components of dp_suddenChange_DP
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_suddenChange_IN_con IN_con "Input record for function dp_suddenChange_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_suddenChange_IN_var IN_var "Input record for function dp_suddenChange_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    TYP.LocalResistanceCoefficient zeta_LOC=zeta_LOC_exp*SMOOTH(Re_min,0,Re)+zeta_LOC_con*SMOOTH(-Re_min,0,Re)+zeta_LOC_min*SMOOTH(0,Re_min,abs(Re));

   // algorithms and equations of dp_suddenChange_DP
   algorithm
      DP:=zeta_LOC*IN_var.rho/(2)*(IN_var.eta/(IN_var.rho)/(d_hyd))^(2)*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(Re,Re_min,2);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_suddenChange_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
"));
   ...
   // components of dp_suddenChange_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_suddenChange_IN_con IN_con "Input record for function dp_suddenChange_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_suddenChange_IN_var IN_var "Input record for function dp_suddenChange_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    TYP.LocalResistanceCoefficient zeta_LOC_exp=max(zeta_LOC_min,(1-A_1/(A_2))^(2));
    TYP.LocalResistanceCoefficient zeta_LOC_con=max(zeta_LOC_min,0.5*(1-A_1/(A_2))^0.75);
    TYP.LocalResistanceCoefficient zeta_LOC=max(zeta_LOC_min,zeta_LOC_exp*SMOOTH(dp_min,0,dp)+zeta_LOC_con*SMOOTH(-dp_min,0,dp))+zeta_LOC_min*SMOOTH(0,dp_min,abs(dp));

   // algorithms and equations of dp_suddenChange_MFLOW
   algorithm
      M_FLOW:=IN_var.rho*A_1*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(dp,dp_min,0.5)*(max(MIN,2/(IN_var.rho*zeta_LOC)))^0.5;

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_suddenChange_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of the local pressure loss at a sudden change of the cross sectional areas (sudden expansion or sudden contraction) with sharp corners at turbulent flow regime for incompressible and single-phase fluid flow through arbitrary shaped cross sectional area (square, circular, etc.) considering a smooth surface. The flow direction determines the type of the transition. In case of the design flow a sudden expansion will be considered. At flow reversal a sudden contraction will be considered.
</p>
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_suddenChange_MFLOW\"> dp_suddenChange_MFLOW</a>.
   ...
   // components of dp_thickEdgedOverall_DP
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_thickEdgedOverall_IN_con IN_con "Input record for function dp_thickEdgedOverall_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_thickEdgedOverall_IN_var IN_var "Input record for function dp_thickEdgedOverall_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    TYP.PressureLossCoefficient zeta_TOT_1=max(MIN,(0.5*(1-A_0/(A_1))^0.75+tau*(1-A_0/(A_1))^1.375+(1-A_0/(A_1))^(2)+lambda_FRI*l/(d_hyd_0))*(A_1/(A_0))^(2)) "Pressure loss coefficient w.r.t. to flow velocity in large cross sectional area";
   ...
   // algorithms and equations of dp_thickEdgedOverall_DP
   algorithm
      DP:=zeta_TOT_1*IN_var.rho/(2)*(IN_var.eta/(IN_var.rho)/(d_hyd_1))^(2)*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(Re,Re_min,2)*(d_hyd_1/(d_hyd_0)*A_0/(A_1))^(2);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_thickEdgedOverall_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
"));
   ...
   // components of dp_thickEdgedOverall_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_thickEdgedOverall_IN_con IN_con "Input record for function dp_thickEdgedOverall_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Orifice.dp_thickEdgedOverall_IN_var IN_var "Input record for function dp_thickEdgedOverall_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   // algorithms and equations of dp_thickEdgedOverall_MFLOW
   algorithm
      M_FLOW:=IN_var.rho*A_1*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(dp,IN_con.dp_smooth,0.5)/((0.5*IN_var.rho*zeta_TOT_1)^0.5);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_thickEdgedOverall_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of pressure loss in thick edged orifices with sharp corners at overall flow regime for incompressible and single-phase fluid flow through an arbitrary shaped cross sectional area (square, circular, etc.) considering constant influence of surface roughness.
</p>
...
"));
   ...
   // components of dp_thickEdgedOverall_IN_con
    SI.Pressure dp_smooth(min=Modelica.Constants.eps)=1 "Start linearisation for decreasing pressure loss" annotation(Dialog(group="Linearisation"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.Orifice.dp_thickEdgedOverall\">See more information</a>.
  ...
  package StraightPipe
   ...
   // components of dp_laminar_DP
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_laminar_IN_con IN_con "Input record for function dp_laminar_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_laminar_IN_var IN_var "Input record for function dp_laminar_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   // algorithms and equations of dp_laminar_DP
   algorithm
      DP:=32*IN_var.eta*velocity*IN_con.L/(d_hyd^(2));

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_laminar_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
"));
   ...
   // components of dp_laminar_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_laminar_IN_con IN_con "Input record for function dp_laminar_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_laminar_IN_var IN_var "Input record for function dp_laminar_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   // algorithms and equations of dp_laminar_MFLOW
   algorithm
      M_FLOW:=IN_var.rho*A_cross*dp*d_hyd^(2)/(32*IN_var.eta*IN_con.L);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_laminar_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of pressure loss in a straight pipe for <b> laminar </b> flow regime of an incompressible and single-phase fluid flow only.
</p>
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_laminar_MFLOW\"> dp_laminar_MFLOW</a>.
   ...
   // components of dp_overall_DP
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_con IN_con "Input record for function dp_overall_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_var IN_var "Input record for function dp_overall_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    SI.ReynoldsNumber Re_lam_max=2090*(1/(max(0.007,k)))^0.0635 "Maximum Reynolds number for laminar regime";
    SI.ReynoldsNumber Re_lam_leave=min(Re_lam_max,max(Re_lam_min,754*Modelica.Math.exp((if k<=0.007 then 0.0065/(0.007) else 0.0065/(k))))) "Start of transition regime for increasing Reynolds number (leaving laminar regime)";
    SI.ReynoldsNumber Re=Modelica.Fluid.Dissipation.Utilities.Functions.General.ReynoldsNumber(A_cross,perimeter,IN_var.rho,IN_var.eta,m_flow);

   // algorithms and equations of dp_overall_DP
   algorithm
   ...
   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_overall_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
Generally this  function is numerically best used for the <b> incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated. On the other hand the  function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_MFLOW\">dp_overall_MFLOW</a> is numerically best used for the <b> compressible case </b> if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated.   <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.StraightPipe.dp_overall\">See more information</a>.
   ...
   // components of dp_overall_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_con IN_con "Input record for function dp_overall_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_var IN_var "Input record for function dp_overall_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
    output SI.MassFlowRate M_FLOW "Output of function dp_overall_MFLOW";
   protected
    Real MIN=Modelica.Constants.eps;
    ...
    SI.ReynoldsNumber Re_turb=(if IN_con.roughness==1 then (max(MIN,lambda_FRI_calc)/(0.3164))^(1/(1.75)) else (-2)*sqrt(max(lambda_FRI_calc,MIN))*Modelica.Math.log10(2.51/(sqrt(max(lambda_FRI_calc,MIN)))+k/(3.7))) "Reynolds number assuming turbulent regime";
    SI.ReynoldsNumber Re_check=(if Re_lam<Re_lam_leave then Re_lam else Re_turb);
    SI.ReynoldsNumber Re_trans=(if Re_lam>=Re_lam_leave then Modelica.Fluid.Dissipation.Utilities.Functions.General.CubicInterpolation_DP(Re_check,Re_lam_leave,Re_turb_min,k,lambda_FRI_calc) else 0);
    SI.ReynoldsNumber Re=(if Re_lam<Re_lam_leave then Re_lam else (if Re_turb>Re_turb_min then Re_turb else Re_trans));

   // algorithms and equations of dp_overall_MFLOW
   algorithm
   ...
   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_overall_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of pressure loss in a straight pipe for <b> overall </b> flow regime of an incompressible and single-phase fluid flow only considering surface roughness.
</p>
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_MFLOW\"> dp_overall_MFLOW</a>.
   ...
   // locally defined classes in dp_turbulent_DP

   // components of dp_turbulent_DP
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_turbulent_IN_con IN_con "Input record for function dp_turbulent_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_turbulent_IN_var IN_var "Input record for function dp_turbulent_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    TYP.DarcyFrictionFactor lambda_FRI_rough=0.25*(max(Re,Re_lam_leave)/(Modelica.Math.log10(k/(3.7)+5.74/((max(Re,Re_lam_leave))^0.9))))^(2) "Darcy friction factor considering surface roughness";
    TYP.DarcyFrictionFactor lambda_FRI=(if IN_con.roughness==TYP1.Neglected then lambda_FRI_smooth else lambda_FRI_rough) "Darcy friction factor";
    TYP.DarcyFrictionFactor lambda_FRI_calc=(if Re<Re_lam_leave then 64/(Re) else (if Re>Re_turb_min then lambda_FRI/(Re^(2)) else Modelica.Fluid.Dissipation.Utilities.Functions.General.CubicInterpolation_MFLOW(Re,Re_lam_leave,Re_turb_min,k)/(Re^(2)))) "Darcy friction factor";
    TYP.PressureLossCoefficient zeta_TOT=lambda_FRI_calc*IN_con.L/(d_hyd) "Pressure loss coefficient";

   // algorithms and equations of dp_turbulent_DP
   algorithm
      DP:=zeta_TOT*IN_var.rho/(2)*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(velocity,v_min,2);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_turbulent_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
Generally this  function is numerically best used for the <b> incompressible case </b>, where the mass flow rate (m_flow) is known (as state variable) in the used model and the corresponding pressure loss (DP) has to be calculated. On the other hand the  function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_turbulent_MFLOW\">dp_turbulent_MFLOW</a> is numerically best used for the <b> compressible case </b> if the pressure loss (dp) is known (out of pressures as state variable) and the mass flow rate (M_FLOW) has to be calculated.   <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.StraightPipe.dp_turbulent\">See more information</a>.
   ...
   // locally defined classes in dp_turbulent_MFLOW

   // components of dp_turbulent_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_turbulent_IN_con IN_con "Input record for function dp_turbulent_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_turbulent_IN_var IN_var "Input record for function dp_turbulent_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    TYP.DarcyFrictionFactor lambda_FRI_calc=2*abs(dp)*d_hyd^(3)*IN_var.rho/(IN_con.L*IN_var.eta^(2)) "Adapted Darcy friction factor";
    SI.ReynoldsNumber Re_lam=lambda_FRI_calc/(64) "Reynolds number assuming laminar regime";
    SI.ReynoldsNumber Re_turb=(if IN_con.roughness==TYP1.Neglected then (max(MIN,lambda_FRI_calc)/(0.3164))^(1/(1.75)) else (-2)*sqrt(max(lambda_FRI_calc,MIN))*Modelica.Math.log10(2.51/(sqrt(max(lambda_FRI_calc,MIN)))+k/(3.7))) "Reynolds number assuming turbulent regime";
    SI.ReynoldsNumber Re_check=(if Re_lam<Re_lam_leave then Re_lam else Re_turb);
    SI.ReynoldsNumber Re_trans=(if Re_lam>=Re_lam_leave then Modelica.Fluid.Dissipation.Utilities.Functions.General.CubicInterpolation_DP(Re_check,Re_lam_leave,Re_turb_min,k,lambda_FRI_calc) else 0);
    SI.ReynoldsNumber Re=(if Re_lam<Re_lam_leave then Re_lam else (if Re_turb>Re_turb_min then Re_turb else Re_trans));
    SI.Velocity velocity=(if dp>=0 then Re else -Re)*IN_var.eta/(IN_var.rho*d_hyd) "Mean velocity";

   // algorithms and equations of dp_turbulent_MFLOW
   algorithm
      M_FLOW:=IN_var.rho*A_cross*velocity;

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_turbulent_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of pressure loss in a straight pipe for <b> turbulent </b> flow regime of an incompressible and single-phase fluid flow only considering surface roughness.
</p>
...
"));
   ...
   // components of dp_turbulent_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.Roughness roughness=Dissipation.Utilities.Types.Roughness.Neglected "Choice of considering surface roughness" annotation(Dialog(group="Straight pipe"));
    SI.Length K=0 "Roughness (average height of surface asperities)" annotation(Dialog(group="Straight pipe"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_turbulent_MFLOW\"> dp_turbulent_MFLOW</a>.
   ...
   // locally defined classes in dp_twoPhaseOverall_DP

   // components of dp_twoPhaseOverall_DP
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_twoPhaseOverall_IN_con IN_con "Input record for function dp_twoPhaseOverall_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_twoPhaseOverall_IN_var IN_var "Input record for function dp_twoPhaseOverall_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    ...
    Real epsilon=Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.VoidFraction(IN_con.voidFractionApproach,true,IN_var.rho_g,IN_var.rho_l,x_flow) "Void fraction";
    SI.Pressure DP_fric=(if IN_con.frictionalPressureLoss==TYP.Friedel then Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.dp_twoPhaseFriedel_DP(IN_con,IN_var,m_flow) else (if IN_con.frictionalPressureLoss==TYP.Chisholm then Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.dp_twoPhaseChisholm_DP(IN_con,IN_var,m_flow) else 0)) "Frictional pressure loss";
    SI.Pressure DP_mom=(if IN_con.momentumPressureLoss then Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.dp_twoPhaseMomentum_DP(IN_con.voidFractionApproach,IN_con.massFlowRateCorrection,IN_con.A_cross,IN_con.perimeter,IN_var.rho_g,IN_var.rho_l,IN_var.x_flow_end,IN_var.x_flow_sta,abs(m_flow)) else 0) "Momentum pressure loss";
    SI.Pressure DP_geo=(if IN_con.geodeticPressureLoss then Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.dp_twoPhaseGeodetic_DP(IN_con.voidFractionApproach,true,IN_con.length,IN_con.phi,IN_var.rho_g,IN_var.rho_l,IN_var.x_flow) else 0) "Geodetic pressure loss";

   // algorithms and equations of dp_twoPhaseOverall_DP
   algorithm
...
"));
   ...
   // components of dp_twoPhaseOverall_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseFrictionalPressureLoss frictionalPressureLoss=Dissipation.Utilities.Types.TwoPhaseFrictionalPressureLoss.Friedel "Choice of frictional pressure loss approach" annotation(Dialog(group="Choices"));
    Modelica.Fluid.Dissipation.Utilities.Types.Roughness voidFractionApproach=Dissipation.Utilities.Types.VoidFractionApproach.Homogeneous "Choice of void fraction approach" annotation(Dialog(group="Choices"));
    Boolean momentumPressureLoss=false "Considering momentum pressure loss" annotation(Dialog(group="Choices"));
    Boolean massFlowRateCorrection=false "Consider heterogeneous mass flow rate correction" annotation(Dialog(group="Choices",enable=(if momentumPressureLoss then true else false)));
    Boolean geodeticPressureLoss=false "Considering geodetic pressure loss" annotation(Dialog(group="Choices"));
    SI.Angle phi=0 "Tilt angle to horizontal" annotation(Dialog(group="Geometry"));
...
This record is used as <b> input record </b> for the pressure loss function <a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_twoPhaseOverall_DP\"> dp_twoPhaseOverall_DP</a>.
   ...
   record dp_twoPhaseOverall_IN_var
    "Input record for function dp_twoPhaseOverall_DP"
    extends Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_var(final x_flow=(x_flow_end+x_flow_sta)/(2));

   // components of dp_twoPhaseOverall_IN_var
    Real x_flow_end=0 "Mass flow rate quality at end of length" annotation(Dialog(group="Fluid properties"));
    Real x_flow_sta=0 "Mass flow rate quality at start of length" annotation(Dialog(group="Fluid properties"));
...
Calculation of pressure loss for <b>two phase flow</b> in a horizontal <b>or</b> vertical straight pipe for an overall flow regime considering frictional, momentum and geodetic pressure loss.
 <a href=\"modelica://Modelica.Fluid.Dissipation.Utilities.SharedDocumentation.PressureLoss.StraightPipe.dp_twoPhaseOverall\">See more information</a>.
  ...
  package Valve
   ...
   // locally defined classes in dp_severalGeometryOverall_DP

   // components of dp_severalGeometryOverall_DP
    input Modelica.Fluid.Dissipation.PressureLoss.Valve.dp_severalGeometryOverall_IN_con IN_con "Input record for function dp_severalGeometryOverall_DP" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Valve.dp_severalGeometryOverall_IN_var IN_var "Input record for function dp_severalGeometryOverall_DP" annotation(Dialog(group="Variable inputs"));
    input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
   ...
   protected
    Real MIN=Modelica.Constants.eps;
    SI.Area Av=(if IN_con.valveCoefficient==TYP1.AV then IN_con.Av else (if IN_con.valveCoefficient==TYP1.KV then IN_con.Kv*27.7e-6 else (if IN_con.valveCoefficient==TYP1.CV then IN_con.Cv*24e-6 else (if IN_con.valveCoefficient==TYP1.OP then IN_con.m_flow_nominal/(max(MIN,IN_con.opening_nominal*(IN_con.rho_nominal*IN_con.dp_nominal)^0.5)) else MIN)))) "Av (metric) flow coefficient [Av]=m^2";
    TYP.PressureLossCoefficient zeta_bal=SMOOTH(0.05,0,IN_var.opening)*(10)^((-3.8397)*IN_var.opening+2.9449)+SMOOTH(0,0.05,IN_var.opening)*IN_con.zeta_TOT_max "Ball valves";
    TYP.PressureLossCoefficient zeta_dia=SMOOTH(0.05,0,IN_var.opening)*(10)^(2.2596*exp((-1.8816)*IN_var.opening))+SMOOTH(0,0.05,IN_var.opening)*IN_con.zeta_TOT_max "Diaphragm valves";
    TYP.PressureLossCoefficient zeta_but=SMOOTH(0.05,0,IN_var.opening)*619.81*exp((-7.3211)*IN_var.opening)+SMOOTH(0,0.05,IN_var.opening)*IN_con.zeta_TOT_max "Butterfly valves";
    TYP.PressureLossCoefficient zeta_gat=SMOOTH(0.05,0,IN_var.opening)*51.45*exp((-6.046)*IN_var.opening)+SMOOTH(0,0.05,IN_var.opening)*IN_con.zeta_TOT_max "Gate valves";
    TYP.PressureLossCoefficient zeta_slu=SMOOTH(0.05,0,IN_var.opening)*248.89*exp((-7.8265)*IN_var.opening)+SMOOTH(0,0.05,IN_var.opening)*IN_con.zeta_TOT_max "Sluice valves";
    TYP.PressureLossCoefficient zeta_TOT=(if IN_con.geometry==TYP2.Ball then zeta_bal else (if IN_con.geometry==TYP2.Diaphragm then zeta_dia else (if IN_con.geometry==TYP2.Butterfly then zeta_but else (if IN_con.geometry==TYP2.Gate then zeta_gat else (if IN_con.geometry==TYP2.Sluice then zeta_slu else 0))))) "Pressure loss coefficient of chosen valve";
    Real valveCharacteristic=(2/(min(IN_con.zeta_TOT_max,max(MIN,max(IN_con.zeta_TOT_min,abs(zeta_TOT))))))^0.5 "Valve characteristic considering opening of chosen valve";
    SI.MassFlowRate m_flow_small=valveCharacteristic*Av*IN_var.rho^0.5*IN_con.dp_small^0.5 "Mass flow rate at linearisation";

   // algorithms and equations of dp_severalGeometryOverall_DP
   algorithm
      DP:=1/((valveCharacteristic*Av)^(2)*IN_var.rho)*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(m_flow,m_flow_small,2);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(m_flow=FD.dp_severalGeometryOverall_MFLOW(IN_con,IN_var,DP)),Documentation(info="<html>
<p>
...
"));
   ...
   // locally defined classes in dp_severalGeometryOverall_MFLOW

   // components of dp_severalGeometryOverall_MFLOW
    input Modelica.Fluid.Dissipation.PressureLoss.Valve.dp_severalGeometryOverall_IN_con IN_con "Input record for function dp_severalGeometryOverall_MFLOW" annotation(Dialog(group="Constant inputs"));
    input Modelica.Fluid.Dissipation.PressureLoss.Valve.dp_severalGeometryOverall_IN_var IN_var "Input record for function dp_severalGeometryOverall_MFLOW" annotation(Dialog(group="Variable inputs"));
    input SI.Pressure dp "Pressure loss" annotation(Dialog(group="Input"));
    output SI.MassFlowRate M_FLOW "Mass flow rate";
   protected
        type TYP1 = Modelica.Fluid.Dissipation.Utilities.Types.ValveCoefficient;
   ...
   // algorithms and equations of dp_severalGeometryOverall_MFLOW
   algorithm
      M_FLOW:=valveCharacteristic*Av*IN_var.rho^0.5*Modelica.Fluid.Dissipation.Utilities.Functions.General.SmoothPower(dp,IN_con.dp_small,0.5);

   annotation(smoothOrder(__Dymola_normallyConstant=IN_con)=2,inverse(dp=FD.dp_severalGeometryOverall_DP(IN_con,IN_var,M_FLOW)),Documentation(info="<html>
<p>
Calculation of pressure loss for a valve with different geometries at overall flow regime for incompressible and single-phase fluid flow in dependence of its opening.
</p>
...
"));
   ...
   // components of dp_severalGeometryOverall_IN_con
    Modelica.Fluid.Dissipation.Utilities.Types.ValveGeometry geometry=Dissipation.Utilities.Types.ValveGeometry.Ball "Choice of geometry for valve" annotation(Dialog(group="Valve"));
    Modelica.Fluid.Dissipation.Utilities.Types.ValveCoefficient valveCoefficient=Modelica.Fluid.Dissipation.Utilities.Types.ValveCoefficient.AV "Choice of valve coefficient" annotation(Dialog(group="Valve"));
    Real Av=PI*0.1^(2)/(4) "Av (metric) flow coefficient [Av]=m^2" annotation(Dialog(group="Valve",enable=(if valveCoefficient==1 then true else false)));
    Real Kv=Av/(27.7e-6) "Kv (metric) flow coefficient [Kv]=m^3/h" annotation(Dialog(group="Valve",enable=(if valveCoefficient==2 then true else false)));
    Real Cv=Av/(24.6e-6) "Cv (US) flow coefficient [Cv]=USG/min" annotation(Dialog(group="Valve",enable=(if valveCoefficient==3 then true else false)));
    SI.Pressure dp_nominal=1e3 "Nominal pressure loss" annotation(Dialog(group="Valve",enable=(if valveCoefficient==4 then true else false)));
    SI.MassFlowRate m_flow_nominal=opening_nominal*Av*(rho_nominal*dp_nominal)^0.5 "Nominal mass flow rate" annotation(Dialog(group="Valve",enable=(if valveCoefficient==4 then true else false)));
    SI.Density rho_nominal=1000 "Nominal inlet density" annotation(Dialog(group="Valve",enable=(if valveCoefficient==4 then true else false)));
    Real opening_nominal=0.5 "Nominal opening" annotation(Dialog(group="Valve",enable=(if valveCoefficient==4 then true else false)));
    Real zeta_TOT_min=1e-3 "Minimal pressure loss coefficient at full opening" annotation(Dialog(group="Valve"));
    Real zeta_TOT_max=1e8 "Maximum pressure loss coefficient at closed opening" annotation(Dialog(group="Valve"));
    SI.Pressure dp_small=0.01*dp_nominal "Linearisation for a pressure loss smaller then dp_small" annotation(Dialog(group="Linearisation"));
...
<a href=\"modelica://Modelica.Fluid.Dissipation.PressureLoss.Valve.dp_severalGeometryOverall_MFLOW\"> dp_severalGeometryOverall_MFLOW</a>.
   ...
   // components of dp_severalGeometryOverall_IN_var
    Real opening=1 "Opening of valve | 0==closed and 1== fully openend" annotation(Dialog(group="Valve"));
    SI.DynamicViscosity eta "Dynamic viscosity of fluid" annotation(Dialog(group="Fluid properties"));
    SI.Density rho "Density of fluid" annotation(Dialog(group="Fluid properties"));
...
<dt>Wischhusen, S.:</dt>
    ...
    PhD thesis, Technische Universit?t Hamburg-Harburg, 2005.</dd>
</dl>
  ...
  package Functions
   ...
   package PressureLoss
    ...
    package TwoPhase
     ...
     // components of dp_twoPhaseChisholm_DP
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_con IN_con annotation(Dialog(group="Constant inputs"));
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_var IN_var annotation(Dialog(group="Variable inputs"));
      input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
      output SI.Pressure DP "Output for function dp_twoPhaseChisholm_DP";
     protected
      Real MIN=Modelica.Constants.eps;
      Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_con IN_con_1ph(final roughness=Dissipation.Utilities.Types.Roughness.Neglected,final d_hyd=4*abs(IN_con.A_cross)/(max(MIN,abs(IN_con.perimeter))),final K=0,final L=abs(IN_con.length));
      Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_IN_var IN_var_1ph(final eta=IN_var.eta_l,final rho=IN_var.rho_l);

     // algorithms and equations of dp_twoPhaseChisholm_DP
     algorithm
        DP:=Modelica.Fluid.Dissipation.PressureLoss.StraightPipe.dp_overall_DP(IN_con_1ph,IN_var_1ph,m_flow)*Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.TwoPhaseMultiplierChisholm(IN_con,IN_var,m_flow);
     ...
     // components of dp_twoPhaseFriedel_DP
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_con IN_con annotation(Dialog(group="Constant inputs"));
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_var IN_var annotation(Dialog(group="Variable inputs"));
      input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
     ...
     protected
      ...
      TYP.DarcyFrictionFactor lambda_FRI_turb=(0.86859*Modelica.Math.log(max(1,Re_liq/(max(MIN,1.964*Modelica.Math.log(Re_liq)-3.8215)))))^(-2) "Darcy friction factor for turbulent regime";
      TYP.DarcyFrictionFactor lambda_FRI=lambda_FRI_lam*SMOOTH(Re_lam_leave,Re_turb,Re_liq)+lambda_FRI_turb*SMOOTH(Re_turb,Re_lam_leave,Re_liq);
     ...
     // algorithms and equations of dp_twoPhaseFriedel_DP
     algorithm
        DP:=SMOOTH2(Re_smooth,1,0)*DP_liq*Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.TwoPhaseMultiplierFriedel(IN_con,IN_var,m_flow);
     ...
     // components of dp_twoPhaseGeodetic_DP
      input Modelica.Fluid.Dissipation.Utilities.Types.VoidFractionApproach voidFractionApproach=Dissipation.Utilities.Types.VoidFractionApproach.Homogeneous "Choice of void fraction approach";
      input Boolean crossSectionalAveraged=true "true == cross sectional averaged void fraction | false == volumetric" annotation(Dialog);
      input SI.Length length=1 "Length in fluid flow direction" annotation(Dialog(group="Geometry"));
      input SI.Angle phi=0 "Tilt angle to horizontal" annotation(Dialog(group="Geometry"));
      input SI.Density rho_g(min=Modelica.Constants.eps) "Density of gaseous phase" annotation(Dialog(group="Fluid properties"));
      input SI.Density rho_l(min=Modelica.Constants.eps) "Density of liquid phase" annotation(Dialog(group="Fluid properties"));
      input Real x_flow(min=0,max=1)=0 "Mass flow rate quality" annotation(Dialog(group="Fluid properties"));
      output SI.Pressure DP_geo "Geodetic pressure loss";
     protected
      Real xflow=min(1,max(0,abs(x_flow))) "Mass flow rate quality";
      Real eps=Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.VoidFraction(voidFractionApproach,crossSectionalAveraged,rho_g,rho_l,xflow) "Void fraction";

     // algorithms and equations of dp_twoPhaseGeodetic_DP
     algorithm
     ...
     // components of dp_twoPhaseMomentum_DP
      input Modelica.Fluid.Dissipation.Utilities.Types.Roughness voidFractionApproach=Modelica.Fluid.Dissipation.Utilities.Types.VoidFractionApproach.Homogeneous "Choice of void fraction approach" annotation(Dialog(group="Choices"));
      input Boolean massFlowRateCorrection=false "Consider heterogeneous mass flow rate correction" annotation(Dialog(group="Choices"));
      input Real A_cross(min=Modelica.Constants.eps)=PI*0.1^(2)/(4) "Cross sectional area" annotation(Dialog(group="Geometry"));
      input SI.Length perimeter(min=Modelica.Constants.eps)=PI*0.1 "Perimeter" annotation(Dialog(group="Geometry"));
      input SI.Density rho_g(min=Modelica.Constants.eps) "Density of gas" annotation(Dialog(group="Fluid properties"));
      input SI.Density rho_l(min=Modelica.Constants.eps) "Density of liquid" annotation(Dialog(group="Fluid properties"));
      input Real x_flow_end(min=0,max=1)=0 "Mass flow rate quality at end of length" annotation(Dialog(group="Fluid properties"));
      input Real x_flow_sta(min=0,max=1)=0 "Mass flow rate quality at start of length" annotation(Dialog(group="Fluid properties"));
      input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
     ...
     protected
      ...
      Real eps_end=Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.VoidFraction(voidFractionApproach,true,rho_g,rho_l,xflowEnd) "Void fraction at end of length";
      Real eps_sta=Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.VoidFraction(voidFractionApproach,true,rho_g,rho_l,xflowSta) "Void fraction at start of length";
      SI.Density rho_end=Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.TwoPhaseDensity(voidFractionApproach,massFlowRateCorrection,rho_g,rho_l,eps_end,xflowEnd) "Mean two phase density at end of lenght";
      SI.Density rho_sta=Modelica.Fluid.Dissipation.Utilities.Functions.PressureLoss.TwoPhase.TwoPhaseDensity(voidFractionApproach,massFlowRateCorrection,rho_g,rho_l,eps_sta,xflowSta) "Mean two phase density at start of lenght";
      ...
      Real SR=Dissipation.Utilities.Functions.PressureLoss.TwoPhase.SlipRatio(voidFractionApproach,rho_g,rho_l,xflowMean) "Slip ratio for velocity void fraction approach";
      ...
      SI.Pressure dp_mom_cor=SMOOTH(delta_xflow,0.05,0)*abs(mdot_A*meanVelEnd+mdotCorEnd*deltaVelEnd/(Across))-abs(mdot_A*meanVelEnd+mdotCorEnd*deltaVelEnd/(Across)) "Momentum pressure loss using mass flow rate correction";

     // algorithms and equations of dp_twoPhaseMomentum_DP
     algorithm
        DP_mom:=(if massFlowRateCorrection then dp_mom_cor else mdot_A^(2)*SMOOTH(delta_xflow,0.05,0)*abs(1/(max(MIN,rho_end))-1/(max(MIN,rho_sta))));
     ...
     // components of TwoPhaseMultiplierFriedel
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_con IN_con annotation(Dialog(group="Constant inputs"));
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_var IN_var(final sigma=0) annotation(Dialog(group="Variable inputs"));
      input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
     ...
     protected
      ...
      TYP.DarcyFrictionFactor lambda_turb_g=1/((max(MIN,0.86859*log(max(1,Re_g/(max(MIN,1.964*log(Re_g)-3.8215))))))^(2)) "Darcy friction factor of gas for assumed turbulent regime";
      TYP.DarcyFrictionFactor lambda_g=lambda_lam_g*SMOOTH(Re_lam_max,Re_turb_min,Re_g)+lambda_turb_g*SMOOTH(Re_turb_min,Re_lam_max,Re_g) "Darcy friction factor of gas for overall regime";
      TYP.DarcyFrictionFactor lambda_lam_l=64/(Re_l) "Darcy friction factor of liquid for assumed laminar regime";
      TYP.DarcyFrictionFactor lambda_turb_l=1/((max(MIN,0.86859*log(max(1,Re_l/(max(MIN,1.964*log(Re_l)-3.8215))))))^(2)) "Darcy friction factor of liquid for assumed turbulent regime";
      TYP.DarcyFrictionFactor lambda_l=lambda_lam_l*SMOOTH(Re_lam_max,Re_turb_min,Re_l)+lambda_turb_l*SMOOTH(Re_turb_min,Re_lam_max,Re_l) "Darcy friction factor of liquid for overall regime";
      Real A=(1-x_flow)^(2)+x_flow^(2)*IN_var.rho_l/(max(MIN,IN_var.rho_g))*lambda_g/(max(MIN,lambda_l)) "Summand for two phase multiplier";
      Real phi_vdo=A+38.5*x_flow^0.76*(1-x_flow)^0.314*(IN_var.rho_l/(max(MIN,IN_var.rho_g)))^0.86*(IN_var.eta_g/(max(MIN,IN_var.eta_l)))^0.73*(1-IN_var.eta_g/(max(MIN,IN_var.eta_l)))^6.84*1/(Fr_l^0.0001)*1/(We_l^0.087);
      Real phi_vup=A+3.43*x_flow^0.685*(1-x_flow)^0.24*(IN_var.rho_l/(max(MIN,IN_var.rho_g)))^0.8*(IN_var.eta_g/(max(MIN,IN_var.eta_l)))^0.22*(1-IN_var.eta_g/(max(MIN,IN_var.eta_l)))^0.89*1/(Fr_l^0.048)*1/(We_l^0.0334);

     // algorithms and equations of TwoPhaseMultiplierFriedel
     algorithm
        phi:=phi_vup;
     ...
     // components of TwoPhaseMultiplierChisholm
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_con IN_con annotation(Dialog(group="Constant inputs"));
      input Modelica.Fluid.Dissipation.Utilities.Records.General.TwoPhaseFlow_var IN_var annotation(Dialog(group="Variable inputs"));
      input SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
     ...
     protected
      ...
      Real gamma=max(1,(abs(IN_var.rho_l/(max(MIN,IN_var.rho_g))))^0.5*(IN_var.eta_g/(max(MIN,IN_var.eta_l)))^(n_exp/(2)));
      Real B_gamma_1=SMOOTH(450,550,mdot_A)*4.8+SMOOTH(550,450,mdot_A)*2400/(max(MIN,mdot_A))-SMOOTH(1950,1850,mdot_A)*2400/(max(MIN,mdot_A))+SMOOTH(1950,1850,mdot_A)*55/(max(MIN,mdot_A^0.5)) "Coefficient B for gamma <= 9.5";
      Real B_gamma_2=SMOOTH(550,650,mdot_A)*520/(max(1,max(9.5,gamma)*mdot_A^0.5))+SMOOTH(650,550,mdot_A)*21/(max(9.5,gamma)) "Coefficient B for 9.5 <= gamma <= 28";
      Real B_gamma=SMOOTH(9.0,10,gamma)*B_gamma_1+SMOOTH(10,9.0,gamma)*B_gamma_2-SMOOTH(28.5,27.7,gamma)*B_gamma_2+SMOOTH(28.5,27.5,gamma)*15000/(max(MIN,gamma^(2)*mdot_A^0.5)) "Coefficient B for gamma";

     // algorithms and equations of TwoPhaseMultiplierChisholm
     algorithm
        phi:=1+(gamma^(2)-1)*(B_gamma*x_flow^((2-n_exp)/(2))*(1-x_flow)^((2-n_exp)/(2))+x_flow^(2-n_exp));
     end TwoPhaseMultiplierChisholm;

     function TwoPhaseDensity
      "Calculation of mean density for two phase flow"

     // components of TwoPhaseDensity
      input Modelica.Fluid.Dissipation.Utilities.Types.Roughness voidFractionApproach=Modelica.Fluid.Dissipation.Utilities.Types.VoidFractionApproach.Homogeneous "Choice of void fraction approach" annotation(Dialog(group="Choices"));
      input Boolean massFlowRateCorrection=false "Consider heterogeneous mass flow rate correction" annotation(Dialog(group="Choices"));
      input SI.Density rho_g(min=Modelica.Constants.eps) "Density of gaseous phase" annotation(Dialog);
      input SI.Density rho_l(min=Modelica.Constants.eps) "Density of liquid phase" annotation(Dialog);
      input Real epsilon_A(min=0,max=1) "Void fraction (cross sectional averaged)" annotation(Dialog(enable=not twoPhaseDensityApproach==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseDensityApproach.Homogeneous));
     ...
     protected
      ...
      Real xflow=min(1,max(0,abs(x_flow))) "Mass flow rate quality";
      SI.Density rho_hom=1/(max(MIN,x_flow/(max(MIN,rho_g))+(1-x_flow)/(max(MIN,rho_l))));
      SI.Density rho_mom=1/(max(MIN,x_flow^(2)/(max(MIN,rho_g*epsilonA))+(1-x_flow)^(2)/(max(MIN,rho_l*(1-epsilonA)))));
      SI.Density rho_kin=1/(max(MIN,rho_hom*(x_flow^(3)/(max(MIN,rho_g^(2)*epsilonA^(2)))+(1-x_flow)^(3)/(max(MIN,rho_l^(2)*(1-epsilonA)^(2))))));

     // algorithms and equations of TwoPhaseDensity
     algorithm
        rho_2ph:=(if massFlowRateCorrection then rho_hom else (if voidFractionApproach==Modelica.Fluid.Dissipation.Utilities.Types.VoidFractionApproach.Homogeneous then rho_hom else (if voidFractionApproach==Modelica.Fluid.Dissipation.Utilities.Types.VoidFractionApproach.Momentum then rho_mom else (if voidFractionApproach==Modelica.Fluid.Dissipation.Utilities.Types.VoidFractionApproach.Energy then rho_kin else MIN))));
...
<dl>
 <dt>VDI:</dt>
    ...
    Springer Verlag, 10th edition, 2006.</dd>
     ...
     // components of VoidFraction
      input Modelica.Fluid.Dissipation.Utilities.Types.VoidFractionApproach voidFractionApproach=Dissipation.Utilities.Types.VoidFractionApproach.Homogeneous "Choice of void fraction approach" annotation(Dialog(group="Choices"));
      input Boolean crossSectionalAveraged=true "true == cross sectional averaged void fraction | false == volumetric" annotation(Dialog);
      input SI.Density rho_g(min=Modelica.Constants.eps) "Density of gaseous phase" annotation(Dialog);
      input SI.Density rho_l(min=Modelica.Constants.eps) "Density of liquid phase" annotation(Dialog);
      input Real x_flow(min=0,max=1)=0 "Mass flow rate quality" annotation(Dialog);
      output Real epsilon "Void fraction";
      output Real slipRatio "Slip ratio";
     protected
      Real MIN=Modelica.Constants.eps;
      Real xflow=min(1,max(0,abs(x_flow))) "Mass flow rate quality";
      Real SR=Dissipation.Utilities.Functions.PressureLoss.TwoPhase.SlipRatio(voidFractionApproach,rho_g,rho_l,xflow) "Slip ratio for void fraction approach";
      Real epsilon_A=rho_l*x_flow/(max(MIN,rho_l*x_flow+rho_g*(1-x_flow)*SR));

     // algorithms and equations of VoidFraction
     algorithm
        epsilon:=(if crossSectionalAveraged then epsilon_A else epsilon_A/(1/(max(MIN,SR))*(1-epsilon_A)+epsilon_A));
        slipRatio:=SR;
     end VoidFraction;
   ...
   package HeatTransfer
    ...
    package TwoPhase
     ...
     // components of kc_twoPhase_condensationHorizontal_KC
      input Modelica.Fluid.Dissipation.Utilities.Records.HeatTransfer.TwoPhaseFlowHT_IN_con IN_con annotation(Dialog(group="Constant inputs"));
      input Modelica.Fluid.Dissipation.Utilities.Records.HeatTransfer.TwoPhaseFlowHT_IN_var IN_var annotation(Dialog(group="Variable inputs"));
      output SI.CoefficientOfHeatTransfer kc "Local two phase heat transfer coefficient";
     protected
      Real MIN=Modelica.Constants.eps;
      SI.Area A_cross=max(MIN,IN_con.A_cross) "Cross sectional area";
      ...
      SI.ReynoldsNumber Re_l=max(1,IN_var.rho_l*velocity*d_hyd/(max(MIN,IN_var.eta_l))) "Reynolds number assuming (total) mass flux flowing as liquid";
      ...
      SI.CoefficientOfHeatTransfer kc_1ph=0.023*Re_l^0.8*Pr_l^0.4*IN_var.lambda_l/(d_hyd);

     // algorithms and equations of kc_twoPhase_condensationHorizontal_KC
     algorithm
        kc:=kc_1ph*((1-x_flow)^0.8+3.8*x_flow^0.76*(1-x_flow)^0.04/(p_red^0.38));
     ...
     // components of kc_twoPhase_boilingVertical_KC
      input Modelica.Fluid.Dissipation.Utilities.Records.HeatTransfer.TwoPhaseFlowHT_IN_con IN_con annotation(Dialog(group="Constant inputs"));
      input Modelica.Fluid.Dissipation.Utilities.Records.HeatTransfer.TwoPhaseFlowHT_IN_var IN_var annotation(Dialog(group="Variable inputs"));
      output SI.CoefficientOfHeatTransfer kc "Local two phase heat transfer coefficient";
     protected
      ...
      Real X_tt=abs(((1-x_flow)/(max(MIN,x_flow)))^0.9*(IN_var.rho_g/(max(MIN,IN_var.rho_l)))^0.5*(IN_var.eta_l/(max(MIN,IN_var.eta_g)))^0.1) "Martinelli parameter";
      ...
      SI.CoefficientOfHeatTransfer kc_fc=0.023*Re_l^0.8*Pr_l^0.4*IN_var.lambda_l/(d_hyd) "Convective heat transfer coefficient assuming liquid mass flow rate only";
      SI.CoefficientOfHeatTransfer kc_nb=55*p_red^0.12*1/((max(MIN,Modelica.Math.log10(1/(p_red))))^0.55)*1/((max(MIN,IN_con.MM))^0.5)*IN_var.qdot_A^0.67 "Nucleate boiling heat transfer coefficient";

     // algorithms and equations of kc_twoPhase_boilingVertical_KC
     algorithm
     ...
     // components of kc_twoPhase_boilingHorizontal_KC
      input Modelica.Fluid.Dissipation.Utilities.Records.HeatTransfer.TwoPhaseFlowHT_IN_con IN_con annotation(Dialog(group="Constant inputs"));
      input Modelica.Fluid.Dissipation.Utilities.Records.HeatTransfer.TwoPhaseFlowHT_IN_var IN_var annotation(Dialog(group="Variable inputs"));
      output SI.CoefficientOfHeatTransfer kc "Local two phase heat transfer coefficient";
     protected
      ...
      Real Bo=abs(IN_var.qdot_A)/(max(MIN,mdot_A*IN_var.dh_lg)) "Boiling number";
      Real X_tt=abs(((1-x_flow)/(max(MIN,x_flow)))^0.9*(IN_var.rho_g/(max(MIN,IN_var.rho_l)))^0.5*(IN_var.eta_l/(max(MIN,IN_var.eta_g)))^0.1) "Martinelli parameter";
      Real E_fc=1+24000*Bo^1.16+1.37*(1/(max(MIN,X_tt)))^0.86 "Enhancement factor for forced convetion";
      Real S_nb=1/(max(MIN,1+1.15e-6*E_fc^(2)*Re_l^1.17)) "Suppression factor for nucleate boiling";
      Real E_fc_hor=SMOOTH(0.049,0.051,Fr_l)*Fr_l^(max(0,abs(0.1-2*Fr_l)))+SMOOTH(0.051,0.049,Fr_l) "Correction of enhancement factor for forced convection in horizontal pipes";
      Real S_nb_hor=SMOOTH(0.049,0.051,Fr_l)*Fr_l^0.5+SMOOTH(0.051,0.049,Fr_l) "Correction of suppression factor for nucleate boiling in horizontal pipes";
      SI.CoefficientOfHeatTransfer kc_fc=0.023*Re_l^0.8*Pr_l^0.4*IN_var.lambda_l/(d_hyd) "Convective heat transfer coefficient assuming liquid mass flow rate only";
      SI.CoefficientOfHeatTransfer kc_nb=55*p_red^0.12*1/((max(MIN,Modelica.Math.log10(1/(p_red))))^0.55)*1/(max(MIN,IN_con.MM^0.5))*(abs(IN_var.qdot_A))^0.67 "Nucleate boiling heat transfer coefficient";

     // algorithms and equations of kc_twoPhase_boilingHorizontal_KC
     algorithm
   ...
   package General
    ...
    // components of CubicInterpolation_DP
    ...
    // algorithms and equations of CubicInterpolation_DP
    algorithm
       Re:=Re1*(lambda2/(lambda2_1))^(1+dx*(c2+dx*c3));

    annotation(smoothOrder=1,smoothOrder=5);
    ...
    // components of CubicInterpolation_MFLOW
    ...
    // algorithms and equations of CubicInterpolation_MFLOW
    algorithm
    ...
    // components of LambertW
     input Real y "f(x)";
     output Real x "W(y)";
    protected
     Real xl;

    // algorithms and equations of LambertW
    algorithm
       if y<=500.0 then
         ...
         xl:=0;
         x:=Modelica.Math.log(y-4.0)-(1.0-1.0/(Modelica.Math.log(y)))*Modelica.Math.log(Modelica.Math.log(y));
       end if;
...
For y > 10 and higher values the relative deviation is smaller 2%.
    ...
    // algorithms and equations of LambertWIter
    algorithm
       w:=(if y>0.1 then Modelica.Fluid.Dissipation.Utilities.Functions.General.LambertW(y) else sqrt(5.43656*max(y,(-1)/(Modelica.Math.exp(1)))+2)-1);
       dev:=1;
       i:=0;
...
within &infin; > y > -1/e. Please note, that for negative inputs <b>two</b> solutions exists. The function currently delivers the result x = -1 ... 0 for that particular range.
    ...
    // algorithms and equations of PrandtlNumber
    algorithm
    ...
    // algorithms and equations of SmoothPower
    algorithm
       y:=(if x>=adeltax then x^pow else (if x<=-adeltax then -(-x)^pow else (C1+C3*x*x)*x));

    annotation(derivative=SmoothPower_der,Documentation(info="<html>
<p>
...
<dt>ThermoFluid Library</dt>
    ...
    // algorithms and equations of SmoothPower_der
    algorithm
       adeltax:=abs(deltax);
    ...
    // algorithms and equations of Stepsmoother
    algorithm
    ...
    annotation(derivative=Stepsmoother_der,Coordsys(extent=[-100, -100; 100, 100],grid=[2, 2],component=[20, 20]),Window(x=0.01,y=0.09,width=0.66,height=0.6),Documentation(info="<html>
<p>
...
<dt>Wischhusen, St.</dt>
    ...
    // components of Stepsmoother_der
    ...
    // algorithms and equations of Stepsmoother_der
    algorithm
       dresult:=(if ((((x>=(0.999*(func-nofunc)+nofunc)) and (func>nofunc)) or ((x<=(0.999*(func-nofunc)+nofunc)) and (nofunc>func))) or ((x<=(0.001*(func-nofunc)+nofunc)) and (func>nofunc))) or ((x>=(0.001*(func-nofunc)+nofunc)) and (nofunc>func)) then 0 else (1-(Modelica.Math.tanh(Modelica.Math.tan(m*x+b)))^(2))*(1+(Modelica.Math.tan(m*x+b))^(2))*m*dx);
  ...
  package Icons
   ...
   package HeatTransfer
    ...
    partial model Gap1_d
     "Geometry figure for gap"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-100,-40},{60,-60}},lineColor={0,0,0},fillPattern=FillPattern.Forward,fillColor={255,255,170},lineThickness=1),Polygon(points={{60,-40},{60,-60},{100,-20},{100,0},{60,-40}},lineColor={0,0,0},lineThickness=1,smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward),Rectangle(extent={{-100,40},{60,20}},lineColor={0,0,0},fillPattern=FillPattern.Forward,fillColor={255,255,170},lineThickness=1),Polygon(points={{60,40},{60,20},{100,60},{100,80},{60,40}},lineColor={0,0,0},lineThickness=1,smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward),Polygon(points={{100,60},{100,0},{60,0},{60,20},{100,60}},lineThickness=1,smooth=Smooth.None,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None,lineColor={0,0,0}),Polygon(points={{-100,-40},{-100,20},{60,20},{60,0},{-60,0},{-100,-40}},lineThickness=1,smooth=Smooth.None,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None,lineColor={0,0,0}),Polygon(points={{-100,40},{-60,80},{100,80},{60,40},{-100,40}},lineColor={0,0,0},lineThickness=1,smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward),Line(points={{-100,20},{60,20},{100,60}},thickness=1,smooth=Smooth.None,color={0,0,0}),Line(points={{-100,-40},{60,-40},{100,0}},thickness=1,smooth=Smooth.None),Line(points={{20,80},{-20,40}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},thickness=0.5),Rectangle(extent={{-4,66},{4,56}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineThickness=1,pattern=LinePattern.None),Text(extent={{-10,66},{10,54}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="h"),Line(points={{60,-66},{-100,-66}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},thickness=0.5),Rectangle(extent={{-22,-62},{-14,-72}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineThickness=1,pattern=LinePattern.None),Text(extent={{-28,-62},{-8,-74}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="L"),Polygon(points={{-100,-40},{-60,0},{100,0},{60,-40},{-100,-40}},lineColor={0,0,0},lineThickness=1,smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward),Line(points={{-80,20},{-80,-40}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},thickness=0.5),Rectangle(extent={{-84,-4},{-76,-14}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-90,-2},{-70,-14}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="s"),Line(points={{26,-10},{-24,-10}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.None},thickness=1),Rectangle(extent={{-4,-4},{4,-14}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineThickness=1,pattern=LinePattern.None),Text(extent={{-10,-2},{10,-14}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="v"),Ellipse(extent={{78,52},{82,48}},pattern=LinePattern.None,lineThickness=0.5,fillColor={0,0,0},fillPattern=FillPattern.Solid,lineColor={0,0,0}),Ellipse(extent={{78,-28},{82,-32}},pattern=LinePattern.None,lineThickness=0.5,fillColor={0,0,0},fillPattern=FillPattern.Solid,lineColor={0,0,0}),Line(points={{80,-28},{80,48}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},thickness=0.5),Rectangle(extent={{76,16},{84,6}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineThickness=1,pattern=LinePattern.None),Text(extent={{70,16},{90,4}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="T_wall"),Text(extent={{-30,36},{-10,24}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="wall 1"),Text(extent={{-30,-44},{-10,-56}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="wall 2")}));
    end Gap1_d;

    partial model HelicalPipe1_d
     "Geometry figure for helical pipe"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Line(points={{-60,82},{-60,-84}},color={0,0,0},thickness=0.5,smooth=Smooth.None,pattern=LinePattern.Dash),Line(points={{-62,-26},{60,-36}},pattern=LinePattern.None,thickness=1,smooth=Smooth.None),Line(points={{-62,24},{60,14}},pattern=LinePattern.None,thickness=1,smooth=Smooth.None),Line(points={{-60,-16},{62,-26}},pattern=LinePattern.None,thickness=1,smooth=Smooth.None),Line(points={{-60,34},{62,24}},pattern=LinePattern.None,thickness=1,smooth=Smooth.None),Ellipse(extent={{38,64},{78,24}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Ellipse(extent={{38,14},{78,-26}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Ellipse(extent={{-78,24},{-38,-16}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Ellipse(extent={{-78,-26},{-38,-66}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Polygon(points={{-60,-26},{60,-36},{60,-76},{-60,-66},{-60,-26}},lineThickness=0.5,smooth=Smooth.None,fillPattern=FillPattern.Forward,fillColor={255,255,170},pattern=LinePattern.None),Line(points={{-60,-46},{60,-56}},color={0,0,0},thickness=0.5,smooth=Smooth.None,pattern=LinePattern.Dash),Polygon(points={{-60,24},{60,14},{60,-26},{-60,-16},{-60,24}},lineThickness=0.5,smooth=Smooth.None,fillPattern=FillPattern.Forward,fillColor={255,255,170},pattern=LinePattern.None),Line(points={{-60,4},{60,-6}},color={0,0,0},thickness=0.5,smooth=Smooth.None,pattern=LinePattern.Dash),Ellipse(extent={{40,-36},{80,-76}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Polygon(points={{-62,74},{58,64},{58,24},{-62,34},{-62,74}},lineThickness=0.5,smooth=Smooth.None,fillPattern=FillPattern.Forward,fillColor={255,255,170},pattern=LinePattern.None),Line(points={{-60,54},{60,44}},color={0,0,0},thickness=0.5,smooth=Smooth.None,pattern=LinePattern.Dash),Ellipse(extent={{-80,74},{-40,34}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Polygon(points={{-60,24},{60,64},{60,24},{-60,-16},{-60,24}},lineThickness=0.5,smooth=Smooth.None,fillPattern=FillPattern.Forward,fillColor={255,255,170},pattern=LinePattern.None),Line(points={{8,28},{-8,22}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled},thickness=1),Line(points={{12,-24},{-4,-30}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled},thickness=1),Polygon(points={{-60,-26},{60,14},{60,-26},{-60,-66},{-60,-26}},lineThickness=0.5,smooth=Smooth.None,fillPattern=FillPattern.Forward,fillColor={255,255,170},pattern=LinePattern.None),Line(points={{60,24},{-60,-16}},smooth=Smooth.None,color={0,0,0},thickness=0.5),Line(points={{60,14},{-60,-26}},thickness=0.5,smooth=Smooth.None),Line(points={{60,-26},{-60,-66}},thickness=0.5,smooth=Smooth.None,color={0,0,0}),Line(points={{-60,74},{60,64}},thickness=0.5,smooth=Smooth.None),Line(points={{-60,24},{60,64}},thickness=0.5,smooth=Smooth.None,color={0,0,0}),Line(points={{-60,-66},{60,-76}},thickness=0.5,smooth=Smooth.None),Line(points={{10,-22},{-6,-28}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled},thickness=1),Ellipse(extent={{46,-42},{74,-70}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-74,68},{-46,40}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid),Line(points={{-74,78},{-46,78}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Line(points={{2,3},{-14,3}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled},thickness=1,origin={-38,55},rotation=180),Line(points={{0,82},{0,-86}},color={0,0,0},thickness=0.5,smooth=Smooth.None,pattern=LinePattern.DashDot),Line(points={{60,-32},{60,-82}},thickness=0.5,smooth=Smooth.None,color={0,0,0},pattern=LinePattern.DashDot),Line(points={{-60,-82},{60,-82}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-18,-76},{18,-88}},fillPattern=FillPattern.Solid,fillColor={255,255,255},lineThickness=0.5,pattern=LinePattern.None),Text(extent={{-14,-76},{12,-86}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None,textString="d_mean"),Line(points={{46,-56},{88,-56}},color={0,0,0},thickness=0.5,smooth=Smooth.None,pattern=LinePattern.DashDot),Line(points={{60,18},{60,-30}},thickness=0.5,smooth=Smooth.None,color={0,0,0},pattern=LinePattern.DashDot),Line(points={{46,-6},{88,-6}},color={0,0,0},thickness=0.5,smooth=Smooth.None,pattern=LinePattern.DashDot),Line(points={{84,-6},{84,-56}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{74,-24},{98,-36}},fillPattern=FillPattern.Solid,fillColor={255,255,255},lineThickness=0.5,pattern=LinePattern.None),Line(points={{-74,82},{-74,52}},thickness=0.5,smooth=Smooth.None,color={0,0,0},pattern=LinePattern.DashDot),Line(points={{-46,82},{-46,52}},thickness=0.5,smooth=Smooth.None,color={0,0,0},pattern=LinePattern.DashDot),Text(extent={{72,-26},{98,-36}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="h"),Rectangle(extent={{-68,84},{-50,76}},fillPattern=FillPattern.Solid,fillColor={255,255,255},lineThickness=0.5,pattern=LinePattern.None),Text(extent={{-72,86},{-50,78}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="d_hyd"),Rectangle(extent={{-36,56},{-30,48}},fillPattern=FillPattern.Solid,fillColor={255,255,255},lineThickness=0.5,pattern=LinePattern.None),Line(points={{-40,54},{-40,50}},color={0,0,0},smooth=Smooth.None,thickness=1),Text(extent={{-44,56},{-22,48}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="L")}));
    end HelicalPipe1_d;

    partial model Plate1_d
     "Geometry figure 1 for plate"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-100,10},{100,-10}},lineColor={0,0,0},fillPattern=FillPattern.Forward,fillColor={255,255,170}),Line(points={{-100,-20},{100,-20}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Text(extent={{-14,-20},{12,-30}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="L"),Line(points={{-20,16},{20,16}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled}),Text(extent={{-14,26},{12,16}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="velocity")}));
    end Plate1_d;

    partial model Plate2_d
     "Geometry figure 2 for plate"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-100,-20},{60,-40}},fillPattern=FillPattern.Forward,fillColor={255,255,170},lineThickness=0.5,lineColor={0,0,0}),Polygon(points={{-100,-20},{-60,20},{100,20},{60,-20},{-100,-20}},lineColor={0,0,0},smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward,lineThickness=0.5),Polygon(points={{60,-20},{60,-40},{100,0},{100,20},{60,-20}},lineColor={0,0,0},lineThickness=0.5,smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward),Line(points={{-20,0},{20,0}},thickness=1,smooth=Smooth.None,color={0,0,0},arrow={Arrow.None,Arrow.Filled}),Text(extent={{-14,10},{12,0}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="v"),Line(points={{-100,-48},{60,-48}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-26,-44},{-18,-54}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineThickness=1,pattern=LinePattern.None),Text(extent={{-34,-44},{-8,-54}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="L")}));
    end Plate2_d;

    partial model Channel_i
     "Icon for heat transfer in a channel"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/heatTransfer/channel/icon_channel.png")}));
    end Channel_i;

    partial model General_i
     "Icon for general heat transfer component"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/heatTransfer/general/icon_general.png")}));
    end General_i;

    partial model HeatExchanger_i
     "Icon for heat transfer of a heat exchanger"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/heatTransfer/heatExchanger/icon_heatExchanger.png")}));
    end HeatExchanger_i;

    partial model HelicalPipe_i
     "Icon for heat transfer in a helical pipe"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/heatTransfer/helicalPipe/icon_helicalPipe.png")}));
    end HelicalPipe_i;

    partial model Plate_i
     "Icon for heat transfer of a plate"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/heatTransfer/plate/icon_plate.png")}));
    end Plate_i;

    partial model StraightPipe_i
     "Icon for heat transfer of a straight pipe"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/heatTransfer/straightPipe/icon_straightPipe.png")}));
    end StraightPipe_i;
   end HeatTransfer;
   ...
   package PressureLoss
    ...
    partial model BendEdged_d
     "Geometry figure of edged bend"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-100,10},{0,-20}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Polygon(points={{0,10},{100,-48},{100,-80},{0,-20},{0,10}},smooth=Smooth.None,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Ellipse(extent={{-6,16},{8,2}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid),Polygon(points={{0,40},{100,-20},{100,-48},{0,10},{0,40}},smooth=Smooth.None,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Rectangle(extent={{-100,40},{0,10}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Polygon(points={{-100,-40},{-100,-20},{0,-20},{0,-40},{-100,-40}},smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward,lineColor={0,0,0}),Polygon(points={{0,-40},{0,-20},{100,-80},{100,-100},{0,-40}},smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward,lineColor={0,0,0}),Polygon(points={{0,40},{0,60},{100,0},{100,-20},{0,40}},smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward,lineColor={0,0,0}),Polygon(points={{-100,40},{-100,60},{0,60},{0,40},{-100,40}},smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward,lineColor={0,0,0}),Line(points={{0,10},{100,-48}},color={0,0,0},smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled},thickness=0.5),Line(points={{-100,10},{0,10}},color={0,0,0},smooth=Smooth.None,thickness=0.5,arrow={Arrow.Filled,Arrow.None}),Line(points={{-78,40},{-78,-20}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-90,32},{-62,18}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-90,30},{-64,20}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="d_hyd"),Text(extent={{-10,20},{16,10}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="delta"),Rectangle(extent={{-54,18},{-44,2}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-58,14},{-40,4}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="L"),Ellipse(extent={{-2,6},{0,4}},lineColor={0,0,0},lineThickness=0.5,fillColor={0,0,0},fillPattern=FillPattern.Solid),Line(points={{-2,4},{2,10}},color={0,0,0},thickness=0.5,smooth=Smooth.None)}));
    end BendEdged_d;

    partial model Channel_d
     "Geometry figure for channel"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Line(points={{-92,80},{-60,80}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot),Ellipse(extent={{20,80},{-20,40}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Ellipse(extent={{80,74},{40,46}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Ellipse(extent={{14,74},{-14,46}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid),Line(points={{0,74},{0,46}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-10,66},{10,56}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-9,66},{9,56}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="d_cir"),Ellipse(extent={{-40,80},{-80,40}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineThickness=0.5),Ellipse(extent={{-46,74},{-74,46}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Line(points={{0,14},{0,-14}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={-60,36},rotation=90),Line(points={{-92,40},{-60,40}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot),Line(points={{-90,80},{-90,40}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-98,64},{-82,54}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-99,65},{-81,55}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="D_ann"),Line(points={{-16,0},{16,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={-74,46},rotation=270),Line(points={{-16,0},{16,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={-46,46},rotation=90),Rectangle(extent={{-68,38},{-52,28}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-69,39},{-51,29}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="d_ann"),Ellipse(extent={{78,72},{42,48}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid),Line(points={{-22,0},{22,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={60,60},rotation=90),Line(points={{6.12325e-016,4},{0,-14}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={46,42},rotation=90),Rectangle(extent={{46,46},{55,39}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{46,46},{55,39}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="a_ell"),Line(points={{-2.71927e-032,4.4409e-016},{22,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={42,38},rotation=90),Line(points={{60,72},{92,72}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot),Line(points={{38,60},{92,60}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot),Line(points={{6.12325e-016,4},{0,-8}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={88,68},rotation=360),Rectangle(extent={{82,68},{94,65}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{83,69},{95,63}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="b_ell"),Rectangle(extent={{-60,24},{-20,-6}},fillColor={255,255,170},fillPattern=FillPattern.Forward,lineColor={0,0,0},lineThickness=0.5),Rectangle(extent={{-56,20},{-24,-2}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineColor={0,0,0},lineThickness=0.5),Line(points={{-16,0},{16,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={-56,-2},rotation=270),Line(points={{-16,0},{16,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={-24,-2},rotation=270),Line(points={{0,14},{2.4493e-016,-18}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={-42,-14},rotation=90),Rectangle(extent={{-50,-10},{-30,-20}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-48,-9},{-31,-18}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="a_rec"),Line(points={{-16,0},{16,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={-31,20},rotation=360),Line(points={{-16,0},{16,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={-31,-2},rotation=360),Line(points={{2.44929e-016,12},{-1.07621e-016,-10}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={-16,10},rotation=180),Rectangle(extent={{-19,15},{0,3}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-18,13},{-1,4}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="b_rec"),Polygon(points={{20,-10},{40,30},{60,-10},{20,-10}},lineColor={0,0,0},lineThickness=0.5,smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward),Polygon(points={{26,-6},{40,22},{54,-6},{26,-6}},lineColor={0,0,0},lineThickness=0.5,smooth=Smooth.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Line(points={{-16,0},{-4,-1.40092e-015}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={26,-22},rotation=270),Line(points={{0,20},{-7.3479e-016,-8}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={46,-16},rotation=90),Line(points={{-16,0},{-4,-1.40092e-015}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={54,-22},rotation=270),Line(points={{-16,0},{12,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={56,22},rotation=360),Line(points={{-16,0},{12,0}},color={0,0,0},smooth=Smooth.None,thickness=0.5,pattern=LinePattern.Dot,origin={56,-6},rotation=360),Line(points={{0,20},{-7.3479e-016,-8}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={64,14},rotation=180),Rectangle(extent={{55,15},{74,3}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{56,13},{73,4}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="h_tri"),Rectangle(extent={{31,-11},{50,-23}},fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{32,-11},{49,-20}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="a_tri"),Line(points={{36,14},{38,12},{42,12},{44,14}},color={0,0,0},thickness=0.5,smooth=Smooth.None),Line(points={{-12,-2},{-7.3479e-016,-8}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.None},origin={40,24},rotation=360),Text(extent={{12,27},{29,18}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="beta")}));
    end Channel_d;

    partial model OrificeSuddenChangeSection_d
     "Geometry figure for orifice with sudden change of cross sectional area"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-100,60},{100,-60}},lineColor={0,0,0},fillColor={255,255,170},fillPattern=FillPattern.Backward),Rectangle(extent={{-100,20},{0,-20}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid),Rectangle(extent={{0,40},{100,-42}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineColor={0,0,0}),Text(extent={{-80,88},{86,76}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="sudden expansion"),Text(extent={{-82,-76},{86,-88}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="sudden contraction"),Line(points={{-20,0},{20,0}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled},origin={0,-72},rotation=180),Line(points={{-20,0},{20,0}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={-48,0},rotation=90),Line(points={{-62,-7.7932e-015},{20,0}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={52,20},rotation=90),Rectangle(extent={{42,6},{62,-8}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{38,4},{64,-6}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="A_2"),Rectangle(extent={{-58,6},{-38,-8}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-62,4},{-36,-6}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="A_1"),Rectangle(extent={{0,20},{0,-20}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-44,10},{40,2}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="design flow direction"),Line(points={{-24,-4},{16,-4}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled}),Line(points={{-20,70},{20,70}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.None,Arrow.Filled})}));
    ...
    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-100,60},{100,-60}},lineColor={0,0,0},fillColor={255,255,170},fillPattern=FillPattern.Backward),Rectangle(extent={{-40,20},{40,-20}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid),Rectangle(extent={{40,40},{100,-42}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineColor={0,0,0}),Line(points={{-20,0},{20,0}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={-48,0},rotation=90),Line(points={{-62,-7.7932e-015},{20,0}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={70,20},rotation=90),Rectangle(extent={{60,6},{80,-8}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{56,4},{82,-6}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="A_1"),Rectangle(extent={{0,20},{0,-20}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Line(points={{-40,-32},{40,-32}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-100,40},{-40,-42}},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineColor={0,0,0}),Line(points={{-62,-7.7932e-015},{20,0}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={-70,20},rotation=90),Rectangle(extent={{-80,6},{-60,-8}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-84,4},{-58,-6}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="A_1"),Line(points={{-42,-2.05161e-015},{-2,4.88624e-016}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled},origin={0,22},rotation=90),Rectangle(extent={{-2,6},{18,-8}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-14,4},{12,-6}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="A_0"),Rectangle(extent={{-44,20},{-40,-20}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Rectangle(extent={{40,20},{44,-20}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Rectangle(extent={{-4,-28},{4,-38}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-12,-28},{14,-38}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="L")}));
    end OrificeThickEdged_d;

    partial model StraightPipe_d
     "Geometry figure for straight pipe"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Ellipse(extent={{98,46},{62,-54}},lineColor={0,0,0},lineThickness=0.5,fillPattern=FillPattern.Forward,fillColor={255,255,170}),Polygon(points={{-80,-54},{-80,46},{80,46},{80,-54},{-80,-54}},smooth=Smooth.None,fillColor={255,255,170},fillPattern=FillPattern.Forward,lineThickness=0.5,pattern=LinePattern.None),Line(points={{-80,52},{80,52}},color={0,0,0},smooth=Smooth.None,thickness=0.5,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-4,58},{6,48}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-8,58},{10,48}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,textString="L"),Ellipse(extent={{90,26},{70,-34}},lineColor={0,0,0},pattern=LinePattern.Dash,lineThickness=0.5),Line(points={{0,26},{0,-34}},color={0,0,0},thickness=0.5,smooth=Smooth.None,arrow={Arrow.Filled,Arrow.Filled}),Rectangle(extent={{-6,-74},{10,-86}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Ellipse(extent={{-62,46},{-98,-54}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Forward),Ellipse(extent={{-70,26},{-90,-34}},lineColor={0,0,0},fillColor={255,255,255},fillPattern=FillPattern.Solid,lineThickness=0.5),Line(points={{-80,46},{80,46}},pattern=LinePattern.None,thickness=0.5,smooth=Smooth.None),Line(points={{-80,-54},{80,-54}},pattern=LinePattern.None,thickness=0.5,smooth=Smooth.None),Line(points={{-80,26},{80,26}},pattern=LinePattern.Dash,thickness=0.5,smooth=Smooth.None,color={0,0,0}),Line(points={{-80,-34},{80,-34}},thickness=0.5,smooth=Smooth.None,color={0,0,0},pattern=LinePattern.Dash),Line(points={{-62,-4},{98,-4}},color={0,0,0},smooth=Smooth.None,thickness=0.5),Rectangle(extent={{-12,2},{10,-8}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Text(extent={{-14,2},{12,-8}},lineColor={0,0,0},lineThickness=0.5,fillColor={255,255,170},fillPattern=FillPattern.Solid,textString="d_hyd")}));
    end StraightPipe_d;

    partial model Valve_d
     "Icon for valve"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics),Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{-6,-74},{10,-86}},lineThickness=0.5,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Line(points={{-60,0},{-100,0}},color={0,127,255}),Polygon(points={{-60,50},{-60,-50},{60,-50},{60,50},{-60,50}},smooth=Smooth.None,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Line(points={{60,0},{100,0}},color={0,127,255}),Line(points={{-60,50},{-60,-50},{60,50},{60,-50},{-60,50}},color={0,0,0},smooth=Smooth.None,thickness=0.5)}));
    end Valve_d;

    model FlowModel
     "Icon for flow model in Modelica.Fluid applications"

    annotation(Icon(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}}),graphics={Polygon(points={{-60,50},{-60,-50},{60,-50},{60,50},{-60,50}},smooth=Smooth.None,fillColor={255,255,255},fillPattern=FillPattern.Solid,pattern=LinePattern.None),Line(points={{-60,0},{-100,0}},color={0,127,255}),Line(points={{-60,50},{-60,-50},{60,50},{60,-50},{-60,50}},color={0,0,0},smooth=Smooth.None,thickness=0.5),Line(points={{60,0},{100,0}},color={0,127,255})}));
    end FlowModel;

    partial model Bend_i
     "Icon for bend"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/pressureLoss/bend/icon_bend.png")}));
    end Bend_i;

    partial model Channel_i
     "Icon for channel"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/pressureLoss/channel/icon_channel.png")}));
    end Channel_i;

    partial model General_i
     "Icon for general pressure drop"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/pressureLoss/general/icon_general.png")}));
    end General_i;

    partial model HeatExchanger_i

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/pressureLoss/heatExchanger/icon_heatExchanger.png")}));
    end HeatExchanger_i;

    partial model Orifice_i
     "Icon for orifice"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/pressureLoss/orifice/icon_orifice.png")}));
    end Orifice_i;

    partial model StraightPipe_i
     "Icon for straight pipe"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/pressureLoss/straightPipe/icon_StraightPipe.png")}));
    end StraightPipe_i;

    partial model Valve_i
     "Icon for valve"

    annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Bitmap(extent={{-100,100},{100,-100}},fileName="modelica://Modelica/Resources/Images/Fluid/Dissipation/pressureLoss/Valve/icon_Valve.png")}));
    end Valve_i;
   end PressureLoss;
  end Icons;
  ...
  package Records
   ...
   package General
    ...
    // components of FluidProperties
     SI.SpecificHeatCapacityAtConstantPressure cp "Specific heat capacity of fluid at constant pressure" annotation(Dialog(group="Fluid properties"));
     SI.DynamicViscosity eta "Dynamic viscosity of fluid" annotation(Dialog(group="Fluid properties"));
     SI.ThermalConductivity lambda "Thermal conductivity of fluid" annotation(Dialog(group="Fluid properties"));
     SI.Density rho "Density of fluid" annotation(Dialog(group="Fluid properties"));
    end FluidProperties;
    ...
    // components of IdealGas_con
     Real exp=2 "Exponent of pressure loss law" annotation(Dialog(group="Generic variables"));
     SI.SpecificHeatCapacity R_s "Specific gas constant of ideal gas" annotation(Dialog(group="Fluid properties"));
     Real Km=6824.86 "Coefficient for pressure loss law [(Pa)^2/{(kg/s)^exp*K}]" annotation(Dialog(group="Generic variables"));
    end IdealGas_con;
    ...
    // components of IdealGas_var
     SI.Density rho_m "Mean density of ideal gas" annotation(Dialog(group="Fluid properties",enable=useMeanDensity));
     SI.Temp_K T_m "Mean temperature of ideal gas" annotation(Dialog(group="Fluid properties",enable=not useMeanDensity));
     SI.Pressure p_m "Mean pressure of ideal gas" annotation(Dialog(group="Fluid properties",enable=not useMeanDensity));
    end IdealGas_var;
    ...
    // components of NominalDensityViscosity
     SI.Pressure dp_nom=2 "Nominal pressure loss (at nominal values of mass flow rate and density)" annotation(Dialog(group="Generic variables"));
     Real exp=2 "Exponent of pressure loss law" annotation(Dialog(group="Generic variables"));
     SI.MassFlowRate m_flow_nom=1 "Nominal mass flow rate (at nominal values of pressure loss and density)" annotation(Dialog(group="Generic variables"));
     SI.Density rho_nom "Nominal density (at nominal values of mass flow rate and pressure loss)" annotation(Dialog(group="Generic variables"));
     Real exp_eta=1 "Exponent for dynamic viscosity dependence" annotation(Dialog(group="Generic variables"));
     SI.DynamicViscosity eta_nom "Dynamic viscosity at nominal pressure loss" annotation(Dialog(group="Generic variables"));
    ...
    // components of NominalPressureLossLawDensity_con
     Modelica.Fluid.Dissipation.Utilities.Types.MassOrVolumeFlowRate target=Dissipation.Utilities.Types.MassOrVolumeFlowRate.MassFlowRate "1 == use nominal mass flow rate | 2 == use nominal volume flow rate" annotation(Dialog(group="Generic variables"));
     SI.Area A_cross=A_cross_nom "Cross sectional area" annotation(Dialog(group="Generic variables"));
     SI.Area A_cross_nom=Modelica.Constants.pi*0.1^(2)/(4) "Nominal cross sectional area" annotation(Dialog(group="Generic variables"));
     SI.Pressure dp_nom=2 "Nominal pressure loss (at nominal values of mass flow rate and density)" annotation(Dialog(group="Generic variables"));
     SI.MassFlowRate m_flow_nom=1 "Nominal mass flow rate (at nominal values of pressure loss and density)" annotation(Dialog(group="Generic variables",enable=target==Modelica.Fluid.Dissipation.Utilities.Types.MassOrVolumeFlowRate.MassFlowRate));
     Real exp=2 "Exponent of pressure loss law" annotation(Dialog(group="Generic variables"));
     SI.VolumeFlowRate V_flow_nom=m_flow_nom/(rho_nom) "Nominal volume flow rate (at nominal values of pressure loss and density)" annotation(Dialog(group="Generic variables",enable=target==Modelica.Fluid.Dissipation.Utilities.Types.MassOrVolumeFlowRate.VolumeFlowRate));
     SI.Density rho_nom "Nominal density (at nominal values of mass flow rate and pressure loss)" annotation(Dialog(group="Generic variables"));
     Types.PressureLossCoefficient zeta_TOT_nom=0.02*1/(0.1) "Nominal pressure loss coefficient (for nominal values)" annotation(Dialog(group="Generic variables"));
    ...
    // components of NominalPressureLossLawDensity_var
     TYP.PressureLossCoefficient zeta_TOT=0.2 "Pressure loss coefficient" annotation(Dialog(group="Generic variables"));
    ...
    // components of QuadraticVFLOW
     Real a(unit="(Pa.s2)/m6")=15 "Coefficient for quadratic term" annotation(Dialog(group="Generic variables"));
     Real b(unit="(Pa.s)/m3")=0 "Coefficient for linear term" annotation(Dialog(group="Generic variables"));
    end QuadraticVFLOW;

    record TwoPhaseFlow_con
     "Base record for two phase Flow"
     extends Modelica.Icons.Record;

    // components of TwoPhaseFlow_con
     SI.Area A_cross=PI*0.1^(2)/(4) "Cross sectional area" annotation(Dialog(group="Geometry"));
     SI.Length perimeter=PI*0.1 "Wettet perimeter" annotation(Dialog(group="Geometry"));
     SI.Length length=1 "Length in fluid flow direction" annotation(Dialog(group="Geometry"));
    end TwoPhaseFlow_con;

    record TwoPhaseFlow_var
     "Base record for two phase flow"
     extends Modelica.Icons.Record;

    // components of TwoPhaseFlow_var
     SI.Density rho_g "Density of gas" annotation(Dialog(group="Fluid properties"));
     SI.Density rho_l "Density of liquid" annotation(Dialog(group="Fluid properties"));
     SI.DynamicViscosity eta_g "Dynamic viscosity of gas" annotation(Dialog(group="Fluid properties"));
     SI.DynamicViscosity eta_l "Dynamic viscosity of liquid" annotation(Dialog(group="Fluid properties"));
     SI.SurfaceTension sigma "Surface tension" annotation(Dialog(group="Fluid properties"));
     Real x_flow=0 "Mean mass flow rate quality over length" annotation(Dialog(group="Input"));
    ...
    // components of IdealGas
     parameter Real exp(min=Modelica.Constants.eps)=2 "Exponent of pressure loss law" annotation(Dialog(group="Generic variables"));
     parameter SI.SpecificHeatCapacity R_s(min=1) "Specific gas constant of ideal gas" annotation(Dialog(group="Fluid properties"));
     Real Km(min=Modelica.Constants.eps)=R_s*2e3/((10)^exp/(rho_m)) "Coefficient for pressure loss law [(Pa)^2/{(kg/s)^exp*K}]" annotation(Dialog(group="Generic variables"));
     SI.Density rho_m=p_m/(R_s*T_m) "Mean density of ideal gas" annotation(Dialog(group="Fluid properties",enable=useMeanDensity));
     SI.Temp_K T_m "Mean temperature of ideal gas" annotation(Dialog(group="Fluid properties",enable=not useMeanDensity));
     SI.Pressure p_m "Mean pressure of ideal gas" annotation(Dialog(group="Fluid properties",enable=not useMeanDensity));
    end IdealGas;
    ...
    // components of NominalPressureLossLawDensity
     Modelica.Fluid.Dissipation.Utilities.Types.MassOrVolumeFlowRate target=Dissipation.Utilities.Types.MassOrVolumeFlowRate.MassFlowRate "1 == use nominal mass flow rate | 2 == use nominal volume flow rate" annotation(Dialog(group="Generic variables"));
     parameter SI.Area A_cross=A_cross_nom "Cross sectional area" annotation(Dialog(group="Generic variables"));
     parameter SI.Area A_cross_nom=Modelica.Constants.pi*0.1^(2)/(4) "Nominal cross sectional area" annotation(Dialog(group="Generic variables"));
     parameter SI.Pressure dp_nom(min=Modelica.Constants.eps)=2 "Nominal pressure loss (at nominal values of mass flow rate and density)" annotation(Dialog(group="Generic variables"));
     parameter SI.MassFlowRate m_flow_nom(min=Modelica.Constants.eps)=1 "Nominal mass flow rate (at nominal values of pressure loss and density)" annotation(Dialog(group="Generic variables",enable=target==Modelica.Fluid.Dissipation.Utilities.Types.MassOrVolumeFlowRate.MassFlowRate));
     parameter Real exp(min=Modelica.Constants.eps)=2 "Exponent of pressure loss law" annotation(Dialog(group="Generic variables"));
     SI.VolumeFlowRate V_flow_nom(min=Modelica.Constants.eps)=m_flow_nom/(rho_nom) "Nominal volume flow rate (at nominal values of pressure loss and density)" annotation(Dialog(group="Generic variables",enable=not target==Modelica.Fluid.Dissipation.Utilities.Types.MassOrVolumeFlowRate.MassFlowRate));
     SI.Density rho_nom(min=Modelica.Constants.eps) "Nominal density (at nominal values of mass flow rate and pressure loss)" annotation(Dialog(group="Generic variables"));
     Types.PressureLossCoefficient zeta_TOT=zeta_TOT_nom "Pressure loss coefficient" annotation(Dialog(group="Generic variables"));
     parameter Types.PressureLossCoefficient zeta_TOT_nom=0.02*1/(0.1) "Nominal pressure loss coefficient (for nominal values)" annotation(Dialog(group="Generic variables"));
    ...
    // components of TwoPhaseFlow
     SI.Density rho_l "Density of liquid" annotation(Dialog(group="Fluid properties"));
     SI.Density rho_g "Density of gas" annotation(Dialog(group="Fluid properties",enable=(if (KC==1) or (KC==2) then true else false)));
     SI.DynamicViscosity eta_l "Dynamic viscosity of liquid" annotation(Dialog(group="Fluid properties"));
     SI.DynamicViscosity eta_g "Dynamic viscosity of gas" annotation(Dialog(group="Fluid properties",enable=(if (KC==1) or (KC==2) then true else false)));
     SI.MassFraction x=0.5 "Vapour fraction" annotation(Dialog(group="Fluid properties"));
     SI.SurfaceTension sigma "Surface Tension" annotation(Dialog(group="Fluid properties",enable=(if DP_fric==1 then true else false)));
     Real n=0.25 "Exponent in Blasius equation (0.2-0.25)" annotation(Dialog(group="others",enable=(if DP_fric==2 then true else false)));
    end TwoPhaseFlow;
   end General;
   ...
   package HeatTransfer
    ...
    // components of EvenGap
     Modelica.Fluid.Dissipation.Utilities.Types.kc_evenGap target=Dissipation.Utilities.Types.kc_evenGap.DevBoth "Target variable of calculation" annotation(Dialog(group="Even gap"));
     SI.Length h=0.1 "Height of cross sectional area" annotation(Dialog(group="Even gap"));
     SI.Length s=0.05 "Distance between parallel plates in cross sectional area" annotation(Dialog(group="Even gap"));
    ...
    // components of General
     Modelica.Fluid.Dissipation.Utilities.Types.kc_general target=Dissipation.Utilities.Types.kc_general.Finest "Target correlation" annotation(Dialog(group="Generic variables"));
     SI.Area A_cross=Modelica.Constants.pi*0.1^(2)/(4) "Cross sectional area" annotation(Dialog(group="Generic variables"));
     SI.Length perimeter=Modelica.Constants.pi*0.1 "Wetted perimeter" annotation(Dialog(group="Generic variables"));
    end General;
    ...
    // components of HelicalPipe
     Real n_nt=1 "Total number of turns" annotation(Dialog(group="HelicalPipe"));
     SI.Diameter d_hyd=0.1 "Hydraulic diameter" annotation(Dialog(group="HelicalPipe"));
     SI.Length h=0.01 "Distance between turns" annotation(Dialog(group="HelicalPipe"));
     SI.Length L=1 "Total length of helical pipe" annotation(Dialog(group="HelicalPipe"));
    end HelicalPipe;
    ...
    // components of StraightPipe
     SI.Diameter d_hyd=0.1 "Hydraulic diameter" annotation(Dialog(group="Straight pipe"));
    ...
    // components of TwoPhaseFlowHT_IN_con
     Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget target=Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor "Choice of (horizontal/vertical) boiling or (horizontal) condensation in pipe" annotation(Dialog(group="Choices"));
     SI.Area A_cross=Modelica.Constants.pi*0.1^(2)/(4) "Cross sectional area" annotation(Dialog(group="Geometry"));
     SI.Length perimeter=Modelica.Constants.pi*0.1 "Wettet perimeter" annotation(Dialog(group="Geometry"));
     SI.MolarMass MM=0.018015268 "Molar mass of fluid" annotation(Dialog(group="Fluid properties",enable=(if (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor) or (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilVer) then true else false)));
     SI.Pressure p_crit "Critical pressure of fluid" annotation(Dialog(group="Fluid properties"));
    ...
    // components of TwoPhaseFlowHT_IN_var
     Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget target=Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor "Choice of (horizontal/vertical) boiling or (horizontal) condensation in pipe" annotation(Dialog(group="Choices"));
     SI.SpecificHeatCapacityAtConstantPressure cp_l "Specific heat capacity of liquid" annotation(Dialog(group="Fluid properties"));
     SI.ThermalConductivity lambda_l "Thermal conductivity of liquid" annotation(Dialog(group="Fluid properties"));
     SI.Density rho_g "Density of gas" annotation(Dialog(group="Fluid properties",enable=(if (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor) or (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilVer) then true else false)));
     SI.Density rho_l "Density of liquid" annotation(Dialog(group="Fluid properties"));
     SI.DynamicViscosity eta_g "Dynamic viscosity of gas" annotation(Dialog(group="Fluid properties",enable=(if (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor) or (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilVer) then true else false)));
     SI.DynamicViscosity eta_l "Dynamic viscosity of liquid" annotation(Dialog(group="Fluid properties"));
     SI.Pressure pressure "Mean pressure of fluid" annotation(Dialog(group="Fluid properties"));
     SI.SpecificEnthalpy dh_lg "Evaporation enthalpy of fluid" annotation(Dialog(group="Fluid properties",enable=(if (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor) or (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilVer) then true else false)));
     SI.MassFlowRate m_flow "Mass flow rate" annotation(Dialog(group="Input"));
     SI.HeatFlux qdot_A=0 "Heat flux at boiling" annotation(Dialog(group="Input",enable=(if (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilHor) or (target==Modelica.Fluid.Dissipation.Utilities.Types.TwoPhaseHeatTransferTarget.BoilVer) then true else false)));
   ...
   package PressureLoss
    ...
    // components of Geometry
     Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow geometry=Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.Circular "Choice of geometry for internal flow" annotation(Dialog(group="Channel"));
     SI.Length K=0 "Roughness (average height of surface asperities)" annotation(Dialog(group="Channel"));
     SI.Length L=1 "Length" annotation(Dialog(group="Channel"));
     SI.Diameter d_ann=d_cir "Small diameter" annotation(Dialog(group="Annular cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.annular));
     SI.Diameter D_ann=2*d_ann "Large diameter" annotation(Dialog(group="Annular cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.annular));
     SI.Diameter d_cir=0.1 "Internal diameter" annotation(Dialog(group="Circular cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.circular));
     SI.Length a_ell=3/(4)*d_cir "Half length of long base line" annotation(Dialog(group="Elliptical cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.elliptical));
     SI.Length b_ell=0.5*a_ell "Half length of short base line" annotation(Dialog(group="Elliptical cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.elliptical));
     SI.Length a_rec=d_cir "Horizontal length" annotation(Dialog(group="Rectangular cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.rectangular));
     SI.Length b_rec=a_rec "Vertical length" annotation(Dialog(group="Rectangular cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.rectangular));
     SI.Length a_tri=d_cir*(1+(2)^0.5) "Length of base line" annotation(Dialog(group="Rectangular cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.rectangular));
     SI.Length h_tri=0.5*a_tri "Heigth to top angle perpendicular to base line" annotation(Dialog(group="Triangle cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.isosceles));
     SI.Angle beta=90*PI/(180) "Top angle" annotation(Dialog(group="Triangle cross sectional area",enable=geometry==Modelica.Fluid.Dissipation.Utilities.Types.GeometryOfInternalFlow.isosceles));
    end Geometry;
    ...
    // components of Orifice
     SI.Area A_0=0.1*A_1 "Cross sectional area of vena contraction" annotation(Dialog(group="Orifice"));
     SI.Length C_0=0.1*C_1 "Perimeter of vena contraction" annotation(Dialog(group="Orifice"));
     SI.Area A_1=PI*0.01^(2)/(4) "Large cross sectional area of orifice" annotation(Dialog(group="Orifice"));
     SI.Length C_1=PI*0.01 "Large perimeter of orifice" annotation(Dialog(group="Orifice"));
     SI.Length L=1e-3 "Length of vena contraction" annotation(Dialog(group="Orifice"));
    end Orifice;
    ...
    // components of PressureLossInput
     Modelica.Fluid.Dissipation.Utilities.Types.PressureLossTarget target=Dissipation.Utilities.Types.PressureLossTarget.PressureLoss "Target variable of calculation" annotation(Dialog(group="Input"));
     SI.Pressure dp=0 "Pressure loss" annotation(Dialog(group="Input",enable=target==Modelica.Fluid.Dissipation.Utilities.Types.PressureLossTarget.pressureLoss));
     SI.MassFlowRate m_flow=0 "Mass flow rate" annotation(Dialog(group="Input",enable=target==Modelica.Fluid.Dissipation.Utilities.Types.PressureLossTarget.massFlowRate));
    ...
    // components of Tjunction
     Boolean united_converging_crossection=true "true == A_cross_total = 2*A_cross_branch | false == A_cross_total > 2*A_cross_branch" annotation(Dialog(group="T-junction"));
     Boolean velocity_reference_branches=true "true == pressure loss coefficents w.r.t. velocity in each passage | false == w.r.t. velocity in total passage" annotation(Dialog(group="T-junction"));
     Integer alpha=90 "Angle of branching" annotation(Dialog(group="T-junction"));
     SI.Diameter d_hyd[3]=ones(3)*0.1 "Hydraulic diameter of passages [side,straight,total]" annotation(Dialog(group="T-junction"));
     SI.MassFlowRate m_flow_min=1e-3 "Restriction for smoothing at reverse fluid flow" annotation(Dialog(group="Restrictions"));
     SI.Velocity v_max=2e2 "Restriction for maximum fluid flow velocity" annotation(Dialog(group="Restrictions"));
     Real zeta_TOT_max=1e3 "Restriction for maximum value of pressure loss coefficient" annotation(Dialog(group="Restrictions"));
    end Tjunction;
    ...
    // components of Diffuser
     SI.Angle alpha=PI*45/(180) "Diffuser diverging angle" annotation(Dialog(group="Diffuser"));
     SI.Area A_1=PI*0.01^(2)/(4) "Small constant cross sectional area before diffuser section" annotation(Dialog(group="Diffuser"));
     SI.Area A_2=2*A_1 "Large constant cross sectional area after diffuser section" annotation(Dialog(group="Diffuser"));
     SI.Length C_1=PI*0.01 "Small perimeter before diffuser section" annotation(Dialog(group="Diffuser"));
     SI.Length C_2=2*C_1 "Large perimeter after diffuser section" annotation(Dialog(group="Diffuser"));
     SI.Length L_1=0.1 "Length of straight pipe before diffuser section" annotation(Dialog(group="Diffuser"));
     SI.Length L_2=L_1 "Length of straight pipe after diffuser section" annotation(Dialog(group="Diffuser"));
     SI.Length L_d=L_1 "Lenght of diffuser section (parallel to bulk fluid flow)" annotation(Dialog(group="Diffuser"));
     SI.Pressure dp_small=1 "Linearisation for a pressure loss smaller then dp_small" annotation(Dialog(group="Numerical aspects"));
     Real zeta_TOT_min=1e-3 "Minimal pressure loss coefficient for infinite Reynolds number" annotation(Dialog(group="Numerical aspects"));
     Real zeta_TOT_max=1e8 "Maximum pressure loss coefficient for Reynolds number approaching zero" annotation(Dialog(group="Numerical aspects"));
    end Diffuser;
    ...
    // components of EdgedBend
     SI.Diameter d_hyd(min=Modelica.Constants.eps)=0.1 "Hydraulic diameter" annotation(Dialog(group="Bend"));
     SI.Angle delta=90*PI/(180) "Angle of turning" annotation(Dialog(group="Bend"));
     SI.Length K=0 "Roughness (absolute average height of surface asperities)" annotation(Dialog(group="Bend"));
    end EdgedBend;
    ...
    // components of SuddenChange
     SI.Area A_1=PI*0.01^(2)/(4) "Small cross sectional area of orifice" annotation(Dialog(group="Orifice"));
     SI.Area A_2=A_1 "Large cross sectional area of orifice" annotation(Dialog(group="Orifice"));
     SI.Length C_1=PI*0.01 "Small perimeter of orifice" annotation(Dialog(group="Orifice"));
     SI.Length C_2=C_1 "Large perimeter of orifice" annotation(Dialog(group="Orifice"));
    end SuddenChange;
   end PressureLoss;
  end Records;
  ...
  package Types
   "Package for types"
   extends Modelica.Icons.Package;
      type DarcyFrictionFactor = Modelica.Icons.TypeReal(final quantity="Darcy friction factor | lambda_fri = zeta_fri / (length/diameter)",final unit="1",min=0,max=1111);
      type FrictionalResistanceCoefficient = Modelica.Icons.TypeReal(final quantity="Frictional resistance coefficient | zeta_fri",final unit="1",min=0,max=1111);
      type LocalResistanceCoefficient = Modelica.Icons.TypeReal(final quantity="Local resistance coefficient | zeta_loc",final unit="1",min=0,max=1111);
      type PressureLossCoefficient = Modelica.Icons.TypeReal(final quantity="Pressure loss coefficient | zeta_tot = zeta_loc + zeta_fri",final unit="1",min=0,max=1111);

   type TwoPhaseFrictionalPressureLoss = enumeration(Friedel, Chisholm);

   type Roughness = enumeration(Neglected, Considered);

   type TwoPhaseHeatTransferTarget = enumeration(BoilHor, BoilVer, CondHor);

   type PressureLossTarget = enumeration(PressureLoss, MassFlowRate);

   type GeometryOfInternalFlow = enumeration(Annular, Circular, Elliptical, Rectangular, Isosceles);

   type kc_evenGap = enumeration(DevOne, DevBoth, UndevOne, UndevBoth);

   type kc_general = enumeration(Rough, Middle, Finest);

   type HeatTransferBoundary = enumeration(UWTuDFF, UHFuDFF, UWTuUFF, UHFuUFF);

   type MassOrVolumeFlowRate = enumeration(MassFlowRate, VolumeFlowRate);

   type VoidFractionApproach = enumeration(Homogeneous, Momentum, Energy, Chisholm);

   type OrificeGeometry = enumeration(SharpEdged, ThickEdged, TiltedEdged, RoundedEdged);

   type ValveGeometry = enumeration(Ball, Diaphragm, Butterfly, Gate, Sluice);

   type ValveCoefficient = enumeration(AV, KV, CV, OP);

   type FluidFlowRegime = enumeration(Laminar, Overall, Turbulent);

   type HTXGeometry_flatTubes = enumeration(LouverFin, RectangularFin);

   type HTXGeometry_roundTubes = enumeration(PlainFin, LouverFin, SlitFin, WavyFin);
  end Types;
 end Utilities;

annotation(classOrder={"UsersGuide","Examples","HeatTransfer","PressureLoss","Utilities"},Documentation(info="<html>
