...
package Nonlinear
 ...
 package Examples
  ...
  // components of quadratureLobatto1
  ...
  protected
   ...
   Real I_err[size(I_numerical,1)] "Absolute errors between numerical and analytical integral values";

  // algorithms and equations of quadratureLobatto1
  algorithm
     I_numerical[1]:=Modelica.Math.Nonlinear.quadratureLobatto(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun4(),0,1,tolerance);
     I_analytical[1]:=-cos(1)+cos(0);
     I_numerical[2]:=Modelica.Math.Nonlinear.quadratureLobatto(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun5(w=5),0,13,tolerance);
     I_analytical[2]:=(-cos(5*13))/(5)+cos(5*0)/(5);
     I_numerical[3]:=Modelica.Math.Nonlinear.quadratureLobatto(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun6(k=1/(sqrt(2))),0,Modelica.Constants.pi/(2),tolerance);
     ...
     print("Analytical integral value = "+String(I_analytical[1],format="2.16f"));
     print("Numerical integral value  = "+String(I_numerical[1],format="2.16f"));
     print("Absolute difference       = "+String(I_err[1],format="2.0e"));
     print("");
     print("Function 2 (integral(sin(5*x)*dx) from x=0 to x=13): ");
     print("Analytical integral value = "+String(I_analytical[2],format="2.16f"));
     print("Numerical integral value  = "+String(I_numerical[2],format="2.16f"));
     print("Absolute difference       = "+String(I_err[2],format="2.0e"));
     print("");
     print("Function 3 (Elliptic integral from x=0 to pi/2): ");
     print("Analytical integral value = "+String(I_analytical[3],format="2.16f"));
     print("Numerical integral value  = "+String(I_numerical[3],format="2.16f"));
...
<li> elliptic integral from x=0 to pi/2 </li>
  ...
  // components of quadratureLobatto2
   input Real Tolerance=1e-5 "Error tolerance of integral value" annotation(Dialog(group="General"));
   ...
   input Real a3=0 "Lower limit" annotation(Dialog(group="Elliptic integral"));
   input Real b3=Modelica.Constants.pi/(2) "Upper limit" annotation(Dialog(group="Elliptic integral"));
   input Real k=1/(sqrt(2)) "Modul" annotation(Dialog(group="Elliptic integral"));
  protected
   Real I[3] "Numerical integral values";

  // algorithms and equations of quadratureLobatto2
  algorithm
     I[1]:=Modelica.Math.Nonlinear.quadratureLobatto(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun4(),a1,b1,Tolerance);
     I[2]:=Modelica.Math.Nonlinear.quadratureLobatto(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun5(w=w),a2,b2,Tolerance);
     I[3]:=Modelica.Math.Nonlinear.quadratureLobatto(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun6(k=k),a3,b3,Tolerance);
...
<li> elliptic integral </li>
  ...
  // components of solveNonlinearEquations1
  ...
  // algorithms and equations of solveNonlinearEquations1
  algorithm
     u_numerical[1]:=Modelica.Math.Nonlinear.solveOneNonlinearEquation(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun1(),-0.5,10,tolerance);
     u_analytical[1]:=1.0;
     u_numerical[2]:=Modelica.Math.Nonlinear.solveOneNonlinearEquation(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun2(w=3),0,5,tolerance);
     u_analytical[2]:=0.6448544035840080891877538;
     u_numerical[3]:=Modelica.Math.Nonlinear.solveOneNonlinearEquation(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun3(p={5,1},m=1),1,8,tolerance);
...
<li> 0 = 5 + log(u) - u </li>
  ...
  // components of solveNonlinearEquations2
   input Real tolerance=100*Modelica.Constants.eps "Relative tolerance of solution u" annotation(Dialog(group="General"));
   ...
   input Real u_min2=0 "Lower limit" annotation(Dialog(group="3*u - sin(w*u) - 1"));
   input Real u_max2=5 "Upper limit" annotation(Dialog(group="3*u - sin(w*u) - 1"));
   input Real w=3 "Angular velocity " annotation(Dialog(group="3*u - sin(w*u) - 1"));
   input Real u_min3=1 "Lower limit" annotation(Dialog(group="p[1] + log(p[2]*u) - m*u"));
   input Real u_max3=8 "Upper limit" annotation(Dialog(group="p[1] + log(p[2]*u) - m*u"));
   input Real p[2]={5,1} "Parameter vector" annotation(Dialog(group="p[1] + log(p[2]*u) - m*u"));
   input Real m=1 "Parameter" annotation(Dialog(group="p[1] + log(p[2]*u) - m*u"));
  protected
   Real u[3];

  // algorithms and equations of solveNonlinearEquations2
  algorithm
     u[1]:=Modelica.Math.Nonlinear.solveOneNonlinearEquation(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun1(),u_min1,u_max1,tolerance);
     u[2]:=Modelica.Math.Nonlinear.solveOneNonlinearEquation(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun2(w=w),u_min2,u_max2,tolerance);
     u[3]:=Modelica.Math.Nonlinear.solveOneNonlinearEquation(Modelica.Math.Nonlinear.Examples.UtilityFunctions.fun3(p=p,m=m),u_min3,u_max3,tolerance);
...
<li> 0 = p[1] + log(p[2]*u) - m*u </li>
  ...
  // components of quadratureLobatto3
   ...
   Real x;
   final parameter Real s=Modelica.Math.Nonlinear.quadratureLobatto(UtilityFunctions.fun7(A=A,w=ws),0,1);

  // algorithms and equations of quadratureLobatto3
  equation
   qd = der(q);
   der(qd)+wq*q = 0;
   x = s*q;
...
to a function in a model.
  ...
  package UtilityFunctions
   ...
   // algorithms and equations of fun1
   algorithm
      y:=u^(2)-1;
   end fun1;
   ...
   // algorithms and equations of fun2
   algorithm
      y:=3*u-sin(w*u)-1;
   end fun2;
   ...
   // algorithms and equations of fun3
   algorithm
      y:=p[1]+log(p[2]*u)-m*u;
   end fun3;
   ...
   // algorithms and equations of fun4
   algorithm
      y:=sin(u);
   end fun4;
   ...
   // algorithms and equations of fun5
   algorithm
      y:=sin(w*u);
   end fun5;
   ...
   // algorithms and equations of fun6
   algorithm
      y:=sqrt(1/(1-k^(2)*(sin(u))^(2)));
   end fun6;
   ...
   // algorithms and equations of fun7
   algorithm
      y:=A*sin(w*u);
   end fun7;
...
arguments to the example functions.
 ...
 package Interfaces
  ...
  // components of partialScalarFunction
...
<a href=\"modelica://Modelica.Math.Nonlinear.Examples\">Math.Nonlinear.Examples</a>.
 ...
 // locally defined classes in quadratureLobatto

 // components of quadratureLobatto
 ...
 protected
  ...
  // algorithms and equations of quadStep
  algorithm
     h:=(b-a)/(2);
     m:=(a+b)/(2);
     alpha:=sqrt(2/(3));
     ...
     if (((is+i1-i2)==is) or (mll<=a)) or (b<=mrr) then
       ...
       I:=quadStep(f,a,mll,fa,fmll,is)+quadStep(f,mll,ml,fmll,fml,is)+quadStep(f,ml,m,fml,fm,is)+quadStep(f,m,mr,fm,fmr,is)+quadStep(f,mr,mrr,fmr,fmrr,is)+quadStep(f,mrr,b,fmrr,fb,is);
     end if;
  end quadStep;
 protected
 ...
 // algorithms and equations of quadratureLobatto
 algorithm
    tol:=tolerance;
    m:=(a+b)/(2);
    h:=(b-a)/(2);
    ...
    is:=h*(.0158271919734802*(y[1]+y[13])+.0942738402188500*(y[2]+y[12])+.155071987336585*(y[3]+y[11])+.188821573960182*(y[4]+y[10])+.199773405226859*(y[5]+y[9])+.224926465333340*(y[6]+y[8])+.242611071901408*y[7]);
    s:=sign(is);
    if s==0 then
      s:=1;
    end if;
    ...
    integral:=quadStep(f,a,b,fa,fb,is);
...
See the examples in <a href=\"modelica://Modelica.Math.Nonlinear.Examples\">Modelica.Math.Nonlinear.Examples</a>.
 ...
 // components of solveOneNonlinearEquation
  input Modelica.Math.Nonlinear.Interfaces.partialScalarFunction f "Function y = f(u); u is computed so that y=0";
