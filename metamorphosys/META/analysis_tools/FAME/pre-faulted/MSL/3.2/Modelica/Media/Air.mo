...
package Air
 ...
 package SimpleAir
  ...
  extends Interfaces.PartialSimpleIdealGasMedium(mediumName="SimpleAir",cp_const=1005.45,MM_const=0.0289651159,R_gas=Constants.R/(0.0289651159),eta_const=1.82e-5,lambda_const=0.026,T_min=Cv.from_degC(0),T_max=Cv.from_degC(100),Temperature(min=Modelica.SIunits.Conversions.from_degC(0),max=Modelica.SIunits.Conversions.from_degC(100)));

  import SI = Modelica.SIunits;
  import Cv = Modelica.SIunits.Conversions;
  import Modelica.Constants;

 // components of SimpleAir
  constant FluidConstants fluidConstants[nS]=FluidConstants(iupacName={"simple air"},casRegistryNumber={"not a real substance"},chemicalFormula={"N2, O2"},structureFormula={"N2, O2"},molarMass=Modelica.Media.IdealGases.Common.SingleGasesData.N2.MM) "constant data for the fluid";
...
Thermal conductivity is computed from temperature using a second order polynomial with a range of validity between 73 and 373 K.
 ...
 annotation(Documentation(info="<HTML>
      <IMG src=\"modelica://Modelica/Resources/Images/Media/IdealGases/SingleGases/Air.png\"></HTML>"),Documentation(info="<html>
Ideal gas medium model for dry air based on the package <a href=\"modelica://Modelica.Media.IdealGases\">IdealGases</a> with additional functions for dynamic viscosity and thermal conductivity in a limited temperature range.
 ...
 package MoistAir
  ...
  extends Interfaces.PartialCondensingGases(mediumName="Moist air",substanceNames={"water","air"},final reducedX=true,final singleState=false,reference_X={0.01,0.99},fluidConstants={IdealGases.Common.FluidData.H2O,IdealGases.Common.FluidData.N2},Temperature(min=200,max=423.15));
  ...
  redeclare replaceable model
   extends BaseProperties(T(stateSelect=(if preferredMediumStates then StateSelect.prefer else StateSelect.default)),p(stateSelect=(if preferredMediumStates then StateSelect.prefer else StateSelect.default)),Xi(stateSelect=(if preferredMediumStates then StateSelect.prefer else StateSelect.default)),redeclare final constant Boolean standardOrderComponents=true)
...
The <a href=\"modelica://Modelica.Media.Air.MoistAir.ThermodynamicState\">thermodynamic state record</a> is computed from density d, temperature T and composition X.
  ...
  // components of Xsaturation
   input ThermodynamicState state "Thermodynamic state record";
   output MassFraction X_sat "Steam mass fraction of sat. boundary";

  // algorithms and equations of Xsaturation
  algorithm
...
Absolute humidity per unit mass of dry air at saturation is computed from pressure and temperature in the thermodynamic state record.
  ...
  // components of xsaturation_pT
   input AbsolutePressure p "Pressure";
   input SI.Temperature T "Temperature";
   output MassFraction x_sat "Absolute humidity per unit mass of dry air";

  // algorithms and equations of xsaturation_pT
  algorithm
...
Absolute humidity per unit mass of dry air at saturation is computed from pressure and temperature.
  ...
  // components of massFraction_pTphi
   input AbsolutePressure p "Pressure";
   input Temperature T "Temperature";
  ...
  // algorithms and equations of massFraction_pTphi
  algorithm
...
Absolute humidity per unit mass of moist air is computed from temperature, pressure and relative humidity.
  ...
  // components of relativeHumidity_pTX
  ...
  // algorithms and equations of relativeHumidity_pTX
  algorithm
     p_steam_sat:=min(saturationPressure(T),0.999*p);
     X_air:=1-X[Water];
...
Relative humidity is computed from pressure, temperature and composition with 1.0 as the upper limit at saturation. Water mass fraction is the first entry in the composition vector.
  ...
  // components of relativeHumidity
   input ThermodynamicState state "Thermodynamic state";
   output Real phi "Relative humidity";

  // algorithms and equations of relativeHumidity
  algorithm
...
Relative humidity is computed from the thermodynamic state record with 1.0 as the upper limit at saturation.
  ...
  // algorithms and equations of gasConstant
  algorithm
...
The ideal gas constant for moist air is computed from the gas phase composition. The first entry in composition vector X is the steam mass fraction of the gas phase.
  ...
  // components of saturationPressureLiquid
   input SI.Temperature Tsat "saturation temperature";
   output SI.AbsolutePressure psat "saturation pressure";

  // algorithms and equations of saturationPressureLiquid
  algorithm
     psat:=611.657*Math.exp(17.2799-4102.99/(Tsat-35.719));

  annotation(Inline=false,smoothOrder=5,derivative=saturationPressureLiquid_der,Documentation(info="<html>
...
273.16 and 373.16 K. Outside these limits a less accurate result is returned.
  ...
  // components of saturationPressureLiquid_der
   input SI.Temperature Tsat "Saturation temperature";
   input Real dTsat(unit="K/s") "Saturation temperature derivative";
   output Real psat_der(unit="Pa/s") "Saturation pressure";

  // algorithms and equations of saturationPressureLiquid_der
  algorithm
...
Derivative function of <a href=\"modelica://Modelica.Media.Air.MoistAir.sublimationPressureIce\">saturationPressureIce</a>
  ...
  // algorithms and equations of saturationPressure
  algorithm
...
<a href=\"modelica://Modelica.Media.Air.MoistAir.sublimationPressureIce\">solid</a> and the <a href=\"modelica://Modelica.Media.Air.MoistAir.saturationPressureLiquid\"> liquid</a> region, respectively, are combined using the first derivative continuous <a href=\"modelica://Modelica.Media.Air.MoistAir.Utilities.spliceFunction\">spliceFunction</a>.
  ...
  // components of saturationPressure_der
   input Temperature Tsat "Saturation temperature";
   input Real dTsat(unit="K/s") "Time derivative of saturation temperature";
   output Real psat_der(unit="Pa/s") "Saturation pressure";

  // algorithms and equations of saturationPressure_der
  algorithm
...
Derivative function of <a href=\"modelica://Modelica.Media.Air.MoistAir.saturationPressure\">saturationPressure</a>
  ...
  // locally defined classes in saturationTemperature
  ...
  // algorithms and equations of saturationTemperature
  algorithm
 ...
 Computes saturation temperature from (partial) pressure via numerical inversion of the function <a href=\"modelica://Modelica.Media.Air.MoistAir.saturationPressure\">saturationPressure</a>. Therefore additional inputs are required (or the defaults are used) for upper and lower temperature bounds.
  ...
  // algorithms and equations of enthalpyOfVaporization
  algorithm
...
Enthalpy of vaporization of water is computed from temperature in the region of 0 to 130 &deg;C.
  ...
  // components of HeatCapacityOfWater
   input Temperature T "Temperature";
   output SpecificHeatCapacity cp_fl "Specific heat capacity of liquid";

  // algorithms and equations of HeatCapacityOfWater
  algorithm
...
The specific heat capacity of water (liquid and solid) is calculated using a
  ...
  // algorithms and equations of enthalpyOfLiquid
  algorithm
...
Specific enthalpy of liquid water is computed from temperature using a polynomial approach. Kept for compatibility reasons, better use <a href=\"modelica://Modelica.Media.Air.MoistAir.enthalpyOfWater\">enthalpyOfWater</a> instead.
  ...
  // algorithms and equations of enthalpyOfGas
  algorithm
     h:=SingleGasNasa.h_Tlow(data=steam,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5)*X[Water]+SingleGasNasa.h_Tlow(data=dryair,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684)*(1.0-X[Water]);
...
Specific enthalpy of moist air is computed from temperature, provided all water is in the gaseous state. The first entry in the composition vector X must be the mass fraction of steam. For a function that also covers the fog region please refer to <a href=\"modelica://Modelica.Media.Air.MoistAir.h_pTX\">h_pTX</a>.
  ...
  // algorithms and equations of enthalpyOfCondensingGas
  algorithm
...
Specific enthalpy of steam is computed from temperature.
  ...
  // algorithms and equations of enthalpyOfNonCondensingGas
  algorithm
...
Temperature is returned from the thermodynamic state record input as a simple assignment.
  ...
  // locally defined classes in T_phX
  ...
  // algorithms and equations of T_phX
  algorithm
...
Specific enthalpy of moist air is computed from the thermodynamic state record. The fog region is included for both, ice and liquid fog.
  ...
  // algorithms and equations of h_pTX
  algorithm
     ...
     X_air:=1-X[Water];
     h:={SingleGasNasa.h_Tlow(data=steam,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5),SingleGasNasa.h_Tlow(data=dryair,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684)}*{X_steam,X_air}+enthalpyOfWater(T)*X_liquid;
...
Specific enthalpy of moist air is computed from pressure, temperature and composition with X[1] as the total water mass fraction. The fog region is included for both, ice and liquid fog.
  ...
  protected
   ...
   Real dx_sat(unit="1/s") "Time derivative of abolute humidity per unit mass of dry air";

  // algorithms and equations of h_pTX_der
  algorithm
     p_steam_sat:=saturationPressure(T);
     ...
     h_der:=X_steam*Modelica.Media.IdealGases.Common.SingleGasNasa.h_Tlow_der(data=steam,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5,dT=dT)+dX_steam*Modelica.Media.IdealGases.Common.SingleGasNasa.h_Tlow(data=steam,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5)+X_air*Modelica.Media.IdealGases.Common.SingleGasNasa.h_Tlow_der(data=dryair,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684,dT=dT)+dX_air*Modelica.Media.IdealGases.Common.SingleGasNasa.h_Tlow(data=dryair,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684)+X_liquid*enthalpyOfWater_der(T=T,dT=dT)+dX_liq*enthalpyOfWater(T);
...
Derivative function for <a href=\"modelica://Modelica.Media.Air.MoistAir.h_pTX\">h_pTX</a>.
  ...
  // components of isentropicEnthalpyApproximation
  ...
  protected
   ...
   MassFraction X[nX] "complete X-vector";

  // algorithms and equations of isentropicEnthalpyApproximation
  algorithm
     X:=(if reducedX then cat(1,state.X,{1-sum(state.X)}) else state.X);
     h:={SingleGasNasa.h_Tlow(data=steam,T=state.T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5),SingleGasNasa.h_Tlow(data=dryair,T=state.T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684)}*X;
...
Specific internal energy is determined from the thermodynamic state record, assuming that the liquid or solid water volume is negligible.
  ...
  // components of specificInternalEnergy_pTX
   input SI.Pressure p "Pressure";
   input SI.Temperature T "Temperature";
   input SI.MassFraction X[:] "Mass fractions of moist air";
   output SI.SpecificInternalEnergy u "Specific internal energy";
  ...
  // algorithms and equations of specificInternalEnergy_pTX
  algorithm
     p_steam_sat:=saturationPressure(T);
     X_sat:=min(p_steam_sat*k_mair/(max(100*Constants.eps,p-p_steam_sat))*(1-X[Water]),1.0);
     X_liquid:=max(X[Water]-X_sat,0.0);
     ...
     u:=X_steam*SingleGasNasa.h_Tlow(data=steam,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5)+X_air*SingleGasNasa.h_Tlow(data=dryair,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684)+enthalpyOfWater(T)*X_liquid-R_gas*T;
...
Specific internal energy is determined from pressure p, temperature T and composition X, assuming that the liquid or solid water volume is negligible.
  ...
  // components of specificInternalEnergy_pTX_der
   input SI.Pressure p "Pressure";
   input SI.Temperature T "Temperature";
   input SI.MassFraction X[:] "Mass fractions of moist air";
   input Real dp(unit="Pa/s") "Pressure derivative";
  ...
  // algorithms and equations of specificInternalEnergy_pTX_der
  algorithm
     p_steam_sat:=saturationPressure(T);
     x_sat:=p_steam_sat*k_mair/(max(100*Modelica.Constants.eps,p-p_steam_sat));
     X_sat:=min(x_sat*(1-X[Water]),1.0);
     ...
     u_der:=X_steam*SingleGasNasa.h_Tlow_der(data=steam,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5,dT=dT)+dX_steam*SingleGasNasa.h_Tlow(data=steam,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=46479.819+2501014.5)+X_air*SingleGasNasa.h_Tlow_der(data=dryair,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684,dT=dT)+dX_air*SingleGasNasa.h_Tlow(data=dryair,T=T,refChoice=ReferenceEnthalpy.UserDefined,h_off=25104.684)+X_liquid*enthalpyOfWater_der(T=T,dT=dT)+dX_liq*enthalpyOfWater(T)-dR_gas*T-R_gas*dT;
...
Derivative function for <a href=\"modelica://Modelica.Media.Air.MoistAir.specificInternalEnergy_pTX\">specificInternalEnergy_pTX</a>.
  ...
  protected
   MoleFraction Y[2]=massToMoleFractions(state.X,{steam.MM,dryair.MM}) "molar fraction";

  // algorithms and equations of specificEntropy
  algorithm
     s:=SingleGasNasa.s0_Tlow(dryair,state.T)*(1-state.X[Water])+SingleGasNasa.s0_Tlow(steam,state.T)*state.X[Water]-(state.X[Water]*Modelica.Constants.R/(MMX[Water])*(if state.X[Water]<Modelica.Constants.eps then state.X[Water] else Modelica.Math.log(Y[Water]*state.p/(reference_p)))+(1-state.X[Water])*Modelica.Constants.R/(MMX[Air])*(if (1-state.X[Water])<Modelica.Constants.eps then 1-state.X[Water] else Modelica.Math.log(Y[Air]*state.p/(reference_p))));
...
Thermal conductivity is computed from temperature using a simple polynomial for dry air, assuming that moisture influence is small. Range of  validity is from 73.15 K to 373.15 K.
  ...
  package Utilities
   ...
   // algorithms and equations of spliceFunction
   algorithm
      scaledX1:=x/(deltax);
   ...
   // components of spliceFunction_der
    input Real pos;
    input Real neg;
    input Real x;
    input Real deltax=1;
   ...
   // algorithms and equations of spliceFunction_der
   algorithm
      scaledX1:=x/(deltax);
      scaledX:=scaledX1*Modelica.Math.asin(1);
  ...
  // locally defined classes in PsychrometricData
      package Medium = Modelica.Media.Air.MoistAir "Used medium package";

  // components of PsychrometricData
   ...
   parameter SIunits.Temperature T_step=10 "Temperature step between two isotherms";
   parameter Integer n_h=16 "Number of lines with constant specific enthalpy";
   parameter SIunits.SpecificEnthalpy h_min=-20e3 "Lowest line of constant enthalpy";
   parameter SIunits.SpecificEnthalpy h_step=1e4 "Enthalpy step between two lines of constant enthalpy";
   ...
   final parameter SIunits.Temperature T_const[n_T]={T_min-T_step+i*T_step for i in 1:n_T} "Constant temperatures";
   final parameter SIunits.SpecificEnthalpy h_const[n_h]={(i-1)*h_step+h_min for i in 1:n_h} "Constant enthalpies";
   final parameter Real phi_const[n_phi]={(i-1)*phi_step+phi_min for i in 1:n_phi} "Constant relative humidities";
   final parameter Real diagSlope=Medium.enthalpyOfVaporization(273.15) "Rotation of diagram that zero degrees isotherm becomes horizontal outside the fog region";
   final parameter SIunits.MassFraction x_start=x_min "Initial absolute humidity in kg water/kg dry air";
   SIunits.MassFraction x(start=x_start) "Absolute humidity in kg water/kg dry air";
   SIunits.SpecificEnthalpy hx_T[n_T] "h_1+x for const T";
   SIunits.SpecificEnthalpy hx_h[n_h](start=h_const) "Const h_1+x";
   SIunits.SpecificEnthalpy hx_phi[n_phi] "h_1+x for const phi";
   SIunits.SpecificEnthalpy y_T[n_T] "Chart enthalpy for const T";
   SIunits.SpecificEnthalpy y_h[n_h] "Chart enthalpy for const h";
   SIunits.SpecificEnthalpy y_phi[n_phi] "Chart enthalpy for const phi";
   Medium.BaseProperties medium_T[n_T] "Medium properties for const T";
   Medium.BaseProperties medium_phi[n_phi] "Medium properties for const phi";
  protected
   SIunits.Pressure ps_phi[n_phi] "Saturation pressure for constant-phi-lines";
   SIunits.Temperature T_phi[n_phi](each start=290);
   Boolean fog[n_T](start=fill(false,n_T)) "Triggers events at intersection of isotherms with phi=1";
   SIunits.Pressure pd[n_T] "Steam partial pressure along isotherms";

  // algorithms and equations of PsychrometricData
  initial equation
   x = x_min;
  equation
   der(x) = (x_max-x_min)/(t);
...
<li><b>y_phi</b>: constant relative humidity</li>
 ...
 // components of MoistAir
  ...
  constant SI.MolarMass MMX[2]={steam.MM,dryair.MM} "Molar masses of components";
