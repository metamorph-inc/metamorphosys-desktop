...
<p>The potential sensor converts the voltage of a node (with respect to the ground node) into a real valued signal. It does not influence the current sum at the node which voltage is measured, therefore, the electrical behavior is not influenced by the sensor.</p>
 ...
 // locally defined classes in VoltageSensor
 ...
 equation
  _famefault_p.port_b.i = 0;
  _famefault_n.port_b.i = 0;
...
<p>The voltage  sensor converts the voltage between the two connectors into a real valued signal. It does not influence the current sum at the nodes in between the voltage is measured, therefore, the electrical behavior is not influenced by the sensor.</p>
 ...
 // locally defined classes in CurrentSensor
    model _famefaults_p = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_n = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_n__p = FAME.Bridges.Electrical;
 ...
 equation
  _famefault_p.port_b.v = _famefault_n.port_b.v;
  _famefault_p.port_b.i = i;
  _famefault_n.port_b.i = -i;
  connect(p,_famefault_p.port_a);
  connect(n,_famefault_n.port_a);
  connect(n,_famefault_n__p.port_a);
  connect(p,_famefault_n__p.port_b);
