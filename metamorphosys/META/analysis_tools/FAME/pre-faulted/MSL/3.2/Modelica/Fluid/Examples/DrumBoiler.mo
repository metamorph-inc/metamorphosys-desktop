...
package DrumBoiler
 ...
 // components of DrumBoiler
  ...
  Modelica.Fluid.Sources.FixedBoundary sink(nPorts=1,p=from_bar(0.5),redeclare package Medium = Modelica.Media.Water.StandardWaterOnePhase,T=500) annotation(Placement(transformation(origin={90,-20},extent={{-10,-10},{10,10}},rotation=180)));
  ...
  Modelica.Fluid.Sensors.Pressure pressure(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(Placement(transformation(extent={{10,14},{30,34}},rotation=0)));
  Modelica.Blocks.Continuous.PI controller(T=120,k=10) annotation(Placement(transformation(extent={{-51,23},{-65,37}},rotation=0)));
  ...
  Modelica.Blocks.Math.Feedback feedback annotation(Placement(transformation(extent={{-26,20},{-46,40}},rotation=0)));
  Modelica.Blocks.Sources.Constant levelSetPoint(k=67) annotation(Placement(transformation(extent={{-43,50},{-30,63}},rotation=0)));
  Modelica.Blocks.Interfaces.RealOutput T_S annotation(Placement(transformation(extent={{100,56},{108,64}},rotation=0)));
  Modelica.Blocks.Interfaces.RealOutput p_S annotation(Placement(transformation(extent={{100,20},{108,28}},rotation=0)));
  Modelica.Blocks.Interfaces.RealOutput qm_S annotation(Placement(transformation(extent={{100,-4},{108,4}},rotation=0)));
  Modelica.Blocks.Interfaces.RealOutput V_l annotation(Placement(transformation(extent={{100,88},{108,96}},rotation=0)));
  Modelica.Blocks.Math.Gain MW2W(k=1e6) annotation(Placement(transformation(extent={{-60,-75.5},{-50,-64.5}},rotation=0)));
  Modelica.Blocks.Math.Gain Pa2bar(k=1e-5) annotation(Placement(transformation(extent={{37,19},{47,29}},rotation=0)));
  Modelica.Thermal.HeatTransfer.Celsius.FromKelvin K2degC annotation(Placement(transformation(extent={{38,55},{48,65}},rotation=0)));
  ...
  Modelica.Blocks.Sources.TimeTable q_F_Tab(table=[0, 0; 3600, 400; 7210, 400]) annotation(Placement(transformation(extent={{-90,-80},{-70,-60}},rotation=0)));
  Modelica.Blocks.Sources.TimeTable Y_Valve_Tab(table=[0, 0; 900, 1; 7210, 1]) annotation(Placement(transformation(extent={{20,-80},{40,-60}},rotation=0)));

 // algorithms and equations of DrumBoiler
 equation
  connect(furnace.port,evaporator.heatPort) annotation(Line(points={{-36,-43},{-36,-30}},color={191,0,0}));
  connect(controller.u,feedback.y) annotation(Line(points={{-49.6,30},{-45,30}},color={0,0,127}));
  connect(feedback.u2,evaporator.V) annotation(Line(points={{-36,22},{-36,-9},{-26,-9}},color={0,0,127}));
  connect(levelSetPoint.y,feedback.u1) annotation(Line(points={{-29.35,56.5},{-22,56.5},{-22,30},{-28,30}},color={0,0,127}));
  connect(massFlowRate.m_flow,qm_S) annotation(Line(points={{30,-9},{30,8.32667e-17},{104,8.32667e-17}},color={0,0,127}));
  connect(evaporator.V,V_l) annotation(Line(points={{-26,-9},{-26,11},{-15,11},{-15,92},{104,92}},color={0,0,127}));
  connect(MW2W.y,furnace.Q_flow) annotation(Line(points={{-49.5,-70},{-36,-70},{-36,-63}},color={0,0,127}));
  connect(pressure.p,Pa2bar.u) annotation(Line(points={{31,24},{36,24}},color={0,0,127}));
  connect(Pa2bar.y,p_S) annotation(Line(points={{47.5,24},{104,24}},color={0,0,127}));
  connect(K2degC.Celsius,T_S) annotation(Line(points={{48.5,60},{104,60}},color={0,0,127}));
  connect(controller.y,limiter.u) annotation(Line(points={{-65.7,30},{-69.6,30}},color={0,0,127}));
  connect(limiter.y,pump.m_flow_in) annotation(Line(points={{-85.7,30},{-90,30},{-90,-12},{-80,-12}},color={0,0,127}));
  connect(temperature.T,K2degC.Kelvin) annotation(Line(points={{4,-1},{4,-1},{10,-1},{10,60},{37,60}},color={0,0,127}));
  connect(pressure.port,massFlowRate.port_a) annotation(Line(points={{20,14},{20,-20}},color={0,127,255}));
  ...
  connect(q_F_Tab.y,MW2W.u) annotation(Line(points={{-69,-70},{-61,-70}},color={0,0,127},smooth=Smooth.None));
  connect(Y_Valve_Tab.y,SteamValve.opening) annotation(Line(points={{41,-70},{60,-70},{60,-28}},color={0,0,127},smooth=Smooth.None));

 annotation(Diagram(coordinateSystem(preserveAspectRatio=true,extent={{-100,-100},{100,100}},grid={1,1}),graphics),Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}},grid={1,1}),graphics={Text(extent={{-151,165},{138,102}},lineColor={0,0,255},textString="%name"),Text(extent={{-79,67},{67,21}},lineColor={0,0,0},textString="drum"),Text(extent={{-90,-14},{88,-64}},lineColor={0,0,0},textString="boiler")}),experiment(StopTime=5400),Documentation(info="<html>
...
<img src=\"modelica://Modelica/Resources/Images/Fluid/Examples/DrumBoiler.png\" border=\"1\">
 ...
 package BaseClasses
  ...
  // locally defined classes in EquilibriumDrumBoiler
  ...
  // components of EquilibriumDrumBoiler
   ...
   parameter Modelica.SIunits.Volume V_l_start=V_t/(2) "Start value of liquid volumeStart value of volume" annotation(Dialog(tab="Initialization"));
   Modelica.Blocks.Interfaces.RealOutput V "liquid volume" annotation(Placement(transformation(origin={100,110},extent={{-10,-10},{10,10}},rotation=90)));
   ...
   Modelica.SIunits.Volume V_l(start=V_l_start,stateSelect=StateSelect.prefer) "volumes of liquid phase";
   ...
   Modelica.SIunits.MassFlowRate qm_S=port_b.m_flow "steam mass flow rate";
   Modelica.SIunits.HeatFlowRate q_F=heatPort.Q_flow "heat flow rate from furnace";
   Modelica.SIunits.MassFlowRate qm_W=port_a.m_flow "feed water mass flow rate";
  protected
   final parameter Boolean port_a_exposesState=true "= true if port_a exposes the state of a fluid volume";
   parameter Boolean showDesignFlowDirection=true "= false to hide the arrow in the model icon";
   final parameter Boolean port_b_exposesState=true "= true if port_b.p exposes the state of a fluid volume";

  // algorithms and equations of EquilibriumDrumBoiler
  initial equation
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}},grid={1,1}),graphics),Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}},grid={1,1}),graphics={Rectangle(extent={{-100,64},{100,-64}},lineColor={0,0,0},fillPattern=FillPattern.Backward,fillColor={135,135,135}),Rectangle(extent={{-100,44},{100,-44}},lineColor={0,0,0},fillPattern=FillPattern.HorizontalCylinder,fillColor={0,127,255}),Ellipse(extent={{18,0},{48,-29}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-1,29},{29,0}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{43,31},{73,2}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-31,1},{-1,-28}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{50,15},{80,-14}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{-72,25},{-42,-4}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{71,-11},{101,-40}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{72,28},{102,-1}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{71,40},{101,11}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Text(extent={{-150,70},{150,110}},lineColor={0,0,255},textString="%name"),Line(points={{0,-64},{0,-100}},color={191,0,0}),Line(points={{100,100},{100,60}},color={0,0,127}),Ellipse(extent={{58,-11},{88,-40}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid),Ellipse(extent={{71,1},{101,-28}},lineColor={0,0,255},pattern=LinePattern.None,fillColor={255,255,255},fillPattern=FillPattern.Solid)}),Documentation(revisions="<html>
<ul>
<li><i>Dec 2008</i>
