...
package Files
 ...
 // locally defined classes in list
 ...
 protected
  ...
  // components of listFile
   input String name;
  protected
   String file[Streams.countLines(name)]=Streams.readFile(name);

  // algorithms and equations of listFile
  algorithm
  ...
  // algorithms and equations of sortDirectory
  algorithm
     ...
     for i in 1:nEntries loop
       if Modelica.Utilities.Internal.FileSystem.stat(directory2+names[i])==Types.FileType.Directory then
  ...
  // algorithms and equations of listDirectory
  algorithm
     if nEntries>0 then
 ...
 // algorithms and equations of list
 algorithm
...
in the \"name\" directory are printed in sorted order.
 ...
 // locally defined classes in copy
 ...
 protected
  ...
  protected
   Integer nNames=Modelica.Utilities.Internal.FileSystem.getNumberOfFiles(oldName);
   String oldNames[nNames];
   String oldName_i;
   String newName_i;

  // algorithms and equations of copyDirectory
  algorithm
     oldNames:=Modelica.Utilities.Internal.FileSystem.readDirectory(oldName,nNames);
 ...
 protected
  ...
  Types.FileType oldFileType=Modelica.Utilities.Internal.FileSystem.stat(oldName2);
  Types.FileType newFileType;

 // algorithms and equations of copy
 algorithm
    ...
    elseif oldFileType==Types.FileType.Directory then
      newFileType:=Modelica.Utilities.Internal.FileSystem.stat(newName2);
      if newFileType==Types.FileType.NoFile then
        createDirectory(newName2);
      elseif (newFileType==Types.FileType.RegularFile) or (newFileType==Types.FileType.SpecialFile) then
        if replace then
      ...
      Modelica.Utilities.Internal.FileSystem.copyFile(oldName2,newName2);
    end if;
...
<blockquote><pre>
 ...
 algorithm
    if (Strings.find(oldName,"/")==0) and (Strings.find(newName,"/")==0) then
      Modelica.Utilities.Internal.FileSystem.rename(oldName,newName);
    else
...
<blockquote><pre>
 ...
 // locally defined classes in remove

 // components of remove
 ...
 protected
  ...
  // components of removeDirectory
   input String name;
  protected
   Integer nNames=Modelica.Utilities.Internal.FileSystem.getNumberOfFiles(name);
  ...
  // algorithms and equations of removeDirectory
  algorithm
     fileNames:=Modelica.Utilities.Internal.FileSystem.readDirectory(name2,nNames);
     for i in 1:nNames loop
 ...
 protected
  ...
  Types.FileType fileType=Modelica.Utilities.Internal.FileSystem.stat(fullName);

 // algorithms and equations of remove
 algorithm
    if (fileType==Types.FileType.RegularFile) or (fileType==Types.FileType.SpecialFile) then
      Modelica.Utilities.Internal.FileSystem.removeFile(fullName);
    elseif fileType==Types.FileType.Directory then
...
This function is silent, i.e., it does not print a message.
 ...
 // components of removeFile
  input String fileName "Name of file that should be removed";
 protected
  Types.FileType fileType=Modelica.Utilities.Internal.FileSystem.stat(fileName);

 // algorithms and equations of removeFile
 algorithm
    if fileType==Types.FileType.RegularFile then
...
This function is silent, i.e., it does not print a message.
 ...
 // locally defined classes in createDirectory

 // components of createDirectory
 ...
 protected
  ...
  // algorithms and equations of existDirectory
  algorithm
     if (fileType==Types.FileType.RegularFile) or (fileType==Types.FileType.SpecialFile) then
 ...
 // algorithms and equations of createDirectory
 algorithm
...
file), an assert is triggered.
 ...
 // algorithms and equations of exist
 algorithm
...
If this is not the case, the function returns false.
 ...
 // algorithms and equations of assertNew
 algorithm
    if fileType==Types.FileType.RegularFile then
...
Returns the full path name of a file or directory \"name\".
 ...
 // algorithms and equations of splitPathName
 algorithm
    while i>=1 loop
...
<pre>
 ...
 // components of temporaryFileName
