...
gases also differentiable at Tlimit.
 ...
 partial package SingleGasNasa
  ...
  extends Interfaces.PartialPureSubstance(ThermoStates=Choices.IndependentVariables.pT,mediumName=data.name,substanceNames={data.name},singleState=false,Temperature(min=200,max=6000,start=500,nominal=500),SpecificEnthalpy(start=(if referenceChoice==ReferenceEnthalpy.ZeroAt0K then data.H0 else (if referenceChoice==ReferenceEnthalpy.UserDefined then h_offset else 0)),nominal=1.0e5),Density(start=10,nominal=10),AbsolutePressure(start=10e5,nominal=10e5));
...
");
  ...
  // components of isentropicEnthalpyApproximation
   ...
   input ReferenceEnthalpy refChoice=referenceChoice "Choice of reference enthalpy";
  ...
  // algorithms and equations of cp_T
  algorithm
  ...
  // components of cp_Tlow
   input IdealGases.Common.DataRecord data "Ideal gas data";
   input SI.Temperature T "Temperature";
   output SI.SpecificHeatCapacity cp "Specific heat capacity at temperature T";

  // algorithms and equations of cp_Tlow
  algorithm
  ...
  // components of cp_Tlow_der
   input IdealGases.Common.DataRecord data "Ideal gas data";
   input SI.Temperature T "Temperature";
   input Real dT "Temperature derivative";
   output Real cp_der "Derivative of specific heat capacity";

  // algorithms and equations of cp_Tlow_der
  algorithm
  ...
  // components of h_T
   ...
   input SI.Temperature T "Temperature";
   input Boolean exclEnthForm=excludeEnthalpyOfFormation "If true, enthalpy of formation Hf is not included in specific enthalpy h";
   input Choices.ReferenceEnthalpy refChoice=referenceChoice "Choice of reference enthalpy";
  ...
  // algorithms and equations of h_T
  algorithm
  ...
  // algorithms and equations of h_T_der
  algorithm
  ...
  // algorithms and equations of h_Tlow
  algorithm
  ...
  // algorithms and equations of h_Tlow_der
  algorithm
  ...
  // algorithms and equations of s0_T
  algorithm
  ...
  // components of s0_Tlow
   input IdealGases.Common.DataRecord data "Ideal gas data";
   input SI.Temperature T "Temperature";
   output SI.SpecificEntropy s "Specific entropy at temperature T";

  // algorithms and equations of s0_Tlow
  algorithm
  ...
  // components of dynamicViscosityLowPressure
  ...
  // algorithms and equations of dynamicViscosityLowPressure
  algorithm
     Tstar:=1.2593*T/(Tc);
...
<p>T. Skoglund, Lund, Sweden, 2004-08-31</p>
  ...
  // components of thermalConductivityEstimate
  ...
  // algorithms and equations of thermalConductivityEstimate
  algorithm
...
thermal conductivity (lambda) at low temperatures.
  ...
  // locally defined classes in T_h
  ...
  // algorithms and equations of T_h
  algorithm
  ...
  // locally defined classes in T_ps
...
and adapted to the Modelica.Media package.
 ...
 partial package MixtureGasNasa
  ...
  extends Modelica.Media.Interfaces.PartialMixtureMedium(ThermoStates=Choices.IndependentVariables.pTX,substanceNames=data[:].name,reducedX=false,singleState=false,reference_X=fill(1/(nX),nX),SpecificEnthalpy(start=(if referenceChoice==ReferenceEnthalpy.ZeroAt0K then 3e5 else (if referenceChoice==ReferenceEnthalpy.UserDefined then h_offset else 0)),nominal=1.0e5),Density(start=10,nominal=10),AbsolutePressure(start=10e5,nominal=10e5),Temperature(min=200,max=6000,start=500,nominal=500));
...
required from medium model \""+mediumName+"\".");
  ...
  protected
   Real Y[nX](unit="mol/mol")=massToMoleFractions(state.X,data.MM) "Molar fractions";
  ...
  // algorithms and equations of h_TX
  algorithm
  ...
  // algorithms and equations of h_TX_der
  algorithm
  ...
  // components of MixEntropy
   input SI.MoleFraction x[:] "mole fraction of mixture";
   output Real smix "mixing entropy contribution, divided by gas constant";

  // algorithms and equations of MixEntropy
  algorithm
     smix:=sum((if x[i]>Modelica.Constants.eps then (-x[i])*Modelica.Math.log(x[i]) else x[i]) for i in 1:size(x,1));
  end MixEntropy;
  ...
  // components of s_TX
   input Temperature T "temperature";
   input MassFraction X[nX] "mass fraction";
   output SpecificEntropy s "specific entropy";

  // algorithms and equations of s_TX
  algorithm
  ...
  // components of gasMixtureViscosity
  ...
  // algorithms and equations of gasMixtureViscosity
  algorithm
...
"));
  ...
  protected
   DynamicViscosity etaX[nX] "component dynamic viscosities";
  ...
  // components of mixtureViscosityChung
   input Temperature T "Temperature";
  ...
  protected
   constant Real Vc[size(y,1)]=Vcrit*1000000 "Critical volumes (cm3/mol)";
   constant Real M[size(y,1)]=MolecularWeights*1000 "Molecular weights (g/mol)";
   ...
   Real kappam "Correlation for highly polar substances such as alcohols and acids";
  ...
  // algorithms and equations of mixtureViscosityChung
  algorithm
     for i in 1:n loop
       for j in 1:n loop
...
"));
  ...
  // components of lowPressureThermalConductivity
  ...
  protected
   MolarMass gamma[size(y,1)];
   Real Tr[size(y,1)] "Reduced temperature";
   Real A[size(y,1),size(y,1)] "Mason and Saxena Modification";
   constant Real epsilon=1.0 "Numerical constant near unity";

  // algorithms and equations of lowPressureThermalConductivity
  algorithm
     ...
     for i in 1:size(y,1) loop
       for j in 1:size(y,1) loop
         A[i,j]:=epsilon*(1+(gamma[j]*(Math.exp(0.0464*Tr[i])-Math.exp((-0.2412)*Tr[i]))/(gamma[i]*(Math.exp(0.0464*Tr[j])-Math.exp((-0.2412)*Tr[j]))))^(1/(2))*(M[i]/(M[j]))^(1/(4)))^(2)/((8*(1+M[i]/(M[j])))^(1/(2)));
       end for;
     end for;
...
"));
  ...
  protected
   ThermalConductivity lambdaX[nX] "component thermal conductivities";
   DynamicViscosity eta[nX] "component thermal dynamic viscosities";
   SpecificHeatCapacity cp[nX] "component heat capacity";
  ...
  // locally defined classes in T_hX
  ...
  protected
   MassFraction Xfull[nX]=(if size(X,1)==nX then X else cat(1,X,{1-sum(X)}));

  // algorithms and equations of T_hX
  algorithm
  ...
  // locally defined classes in T_psX
  ...
  protected
   ...
   package Internal
    ...
    protected
     MassFraction Xfull[nX]=(if size(X,1)==nX then X else cat(1,X,{1-sum(X)}));
     Real Y[nX](unit="mol/mol")=massToMoleFractions((if size(X,1)==nX then X else cat(1,X,{1-sum(X)})),data.MM) "Molar fractions";
  ...
  // algorithms and equations of T_psX
  algorithm
     T:=Internal.solve(s,200,6000,p,Xfull,data[1]);
  end T_psX;

 // components of MixtureGasNasa
  ...
  constant SpecificEnthalpy h_offset=0.0 "User defined offset for reference enthalpy, if referenceChoice = UserDefined";
  constant MolarMass MMX[nX]=data[:].MM "molar masses of components";
