...
package Utilities
 ...
 // components of theoreticalNormalGravityWGS84
  input Modelica.SIunits.Conversions.NonSIunits.Angle_deg phi "Geodetic latitute" annotation(Dialog);
 protected
  constant Modelica.SIunits.Position a=6378137.0 "Semi-major axis of the earth ellipsoid";
  constant Modelica.SIunits.Position b=6356752.3142 "Semi-minor axis of the earth ellipsoid";
  constant Modelica.SIunits.AngularAcceleration g_e=9.7803253359 "Theoretical gravity acceleration at the equator";
  constant Modelica.SIunits.AngularAcceleration g_p=9.8321849378 "Theoretical gravity acceleration at the poles";
  constant Real k=b/(a)*g_p/(g_e)-1;
  constant Real e2=8.1819190842622e-2^(2) "Square of the first ellipsoidal eccentricity";
  constant Real f=1/(298.257223563) "Ellipsoidal flattening";
  constant Modelica.SIunits.AngularVelocity w=7292115e-11 "Angular velocity of earth";
  constant Real GM(unit="m3/s2")=3986004.418e8 "Earths Gravitational Constant";
  constant Real m(unit="1")=w^(2)*a^(2)*b/(GM);
  Real sinphi2(unit="1");
  Modelica.SIunits.AngularAcceleration gn "Normal gravity at the earth ellipsoid";
  Modelica.SIunits.AngularAcceleration gh "Normal gravity at height h over the earth ellipsoid";
  Modelica.SIunits.Position h "Height over the WGS84 earth ellipsoid";
  Real ha(unit="1") "h/a";

 // algorithms and equations of theoreticalNormalGravityWGS84
 algorithm
    h:=r[2];
...
ellipsoid surface.
 ...
 // algorithms and equations of sineSurface
 algorithm
    for i in 1:nu loop
    ...
    if multiColoredSurface then
      C:={{(Z[i,j]+1)*200,255,0} for i in 1:nu, j in 1:nv};
    end if;
 end sineSurface;
end Utilities;
