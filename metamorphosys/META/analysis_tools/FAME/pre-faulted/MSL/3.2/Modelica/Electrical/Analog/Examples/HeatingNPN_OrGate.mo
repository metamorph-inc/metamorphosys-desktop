...
model HeatingNPN_OrGate
 "Heating NPN Or Gate"
 extends Modelica.Icons.Example;

// components of HeatingNPN_OrGate
 ...
 Semiconductors.HeatingNPN T1(Bf=100,Br=1,Is=1.e-14,Vak=0,Tauf=tauVal,Taur=tauVal,Ccs=CapVal,Cje=CapVal,Cjc=CapVal,Phie=1,Me=0.5,Phic=1,Mc=0.5,Gbc=1.e-12,Gbe=1.e-12,EMax=40,vt_t(start=0.01,fixed=false),useHeatPort=true) annotation(Placement(transformation(extent={{-20,48},{0,68}},rotation=0)));
 Semiconductors.HeatingNPN T2(Bf=100,Br=1,Is=1.e-14,Vak=0,Tauf=tauVal,Taur=tauVal,Ccs=CapVal,Cje=CapVal,Cjc=CapVal,Phie=1,Me=0.5,Phic=1,Mc=0.5,Gbc=1.e-12,Gbe=1.e-12,EMax=40,vt_t(start=0.01,fixed=false),useHeatPort=true) annotation(Placement(transformation(extent={{20,-22},{40,-2}},rotation=0)));

// algorithms and equations of HeatingNPN_OrGate
equation
 ...
 connect(C3.p,R2.n) annotation(Line(points={{-16,-30},{-16,-21},{-16,-12}},color={0,0,255}));
 connect(C1.p,R1.n) annotation(Line(points={{-70,48},{-70,58}},color={0,0,255}));
 connect(Gnd5.p,C3.n) annotation(Line(points={{-16,-56},{-16,-50},{-16,-50}},color={0,0,255}));
 connect(T1.B,R1.n) annotation(Line(points={{-20,58},{-70,58}},color={0,0,255}));
 connect(T1.E,Gnd3.p) annotation(Line(points={{0,53},{0,48.75},{1.05471e-015,48.75},{1.05471e-015,44.5},{0,44.5},{0,36}},color={0,0,255}));
