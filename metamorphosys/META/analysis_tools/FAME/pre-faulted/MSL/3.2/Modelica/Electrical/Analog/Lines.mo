...
package Lines
 ...
 // locally defined classes in OLine
    model _famefaults_p3 = FAME.Dampers.ElectricalWithConnectEquations;
    model _famefaults_p2 = FAME.Dampers.ElectricalWithConnectEquations;
    model _famefaults_p1 = FAME.Dampers.ElectricalWithConnectEquations;
    model _famefaults_p2__p3 = FAME.Bridges.Electrical;
    model _famefaults_p1__p3 = FAME.Bridges.Electrical;
 ...
 // components of OLine
  ...
  Modelica.Electrical.Analog.Interfaces.Pin p3 annotation(Placement(transformation(extent={{-10,-110},{10,-90}},rotation=0)));
  FAME.Dampers.ElectricalWithConnectEquations _famefault_p3(amount=0.0);
  Modelica.Electrical.Analog.Interfaces.Pin p2 annotation(Placement(transformation(extent={{90,-10},{110,10}},rotation=0)));
  FAME.Dampers.ElectricalWithConnectEquations _famefault_p2(amount=0.0);
  parameter Real c(final min=Modelica.Constants.small,unit="F/m",start=1) "Capacitance per meter";
  Modelica.Electrical.Analog.Interfaces.Pin p1 annotation(Placement(transformation(extent={{-110,-10},{-90,10}},rotation=0)));
  FAME.Dampers.ElectricalWithConnectEquations _famefault_p1(amount=0.0);
  ...
  parameter Modelica.SIunits.Length length(final min=Modelica.Constants.small,start=1) "Length of line";
  Modelica.SIunits.Voltage v23;
  Modelica.SIunits.Current i2;
  Modelica.SIunits.Current i1;
  FAME.Bridges.Electrical _famefault_p2__p3(amount=0.0);
  FAME.Bridges.Electrical _famefault_p1__p3(amount=0.0);
  FAME.Bridges.Electrical _famefault_p1__p2(amount=0.0);
 protected
  Modelica.Electrical.Analog.Basic.Conductor G[N](G=fill(g*length/(N),N));
  Modelica.Electrical.Analog.Basic.Capacitor C[N](C=fill(c*length/(N),N));
  Modelica.Electrical.Analog.Basic.Inductor L[N+1](L=fill(l*length/(N+1),N+1));
  Modelica.Electrical.Analog.Basic.Resistor R[N+1](R=fill(r*length/(N+1),N+1));

 // algorithms and equations of OLine
 equation
  v13 = _famefault_p1.port_b.v-_famefault_p3.port_b.v;
  v23 = _famefault_p2.port_b.v-_famefault_p3.port_b.v;
  i1 = _famefault_p1.port_b.i;
  i2 = _famefault_p2.port_b.i;
  connect(_famefault_p1.port_b,R[1].p);
  for i in 1:N loop
   connect(R[i].n,L[i].p);
   connect(L[i].n,C[i].p);
   connect(L[i].n,G[i].p);
   connect(C[i].n,_famefault_p3.port_b);
   connect(G[i].n,_famefault_p3.port_b);
   connect(L[i].n,R[i+1].p);
  end for;
  connect(R[N+1].n,L[N+1].p);
  connect(L[N+1].n,_famefault_p2.port_b);
  connect(p3,_famefault_p3.port_a);
  connect(p2,_famefault_p2.port_a);
  connect(p1,_famefault_p1.port_a);
  connect(p3,_famefault_p2__p3.port_a);
  connect(p2,_famefault_p2__p3.port_b);
  connect(p3,_famefault_p1__p3.port_a);
  connect(p1,_famefault_p1__p3.port_b);
...
<li><i> 1998   </i>
 ...
 // locally defined classes in M_OLine
  ...
  // algorithms and equations of segment
  equation
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{40,-40},{-40,40}},lineColor={0,0,255})}),Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics),Documentation(info="<html>
<p>The segment model is part of the multiple line model. It describes one line segment as outlined in the M_Oline description. Using the loop possibilities of Modelica it is formulated by connecting components the number of which depends on the number of lines.</p>
  ...
  // algorithms and equations of segment_last
  equation
   for j in 1:lines-1 loop
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{20,-40},{-20,40}},lineColor={0,0,255})}),Documentation(info="<html>
<p>The segment_last model is part of the multiple line model. It describes the special  line segment which is used to get the line symmetrical as outlined in the M_Oline description. Using the loop possibilities of Modelica it is formulated by connecting components the number of which depends on the number of lines.</p>
 ...
 // components of M_OLine
  parameter Real g[dim_vector_lgc](final min=Modelica.Constants.small,unit="S/m")={8.05e-6,3.42e-5,2-91e-5,1.76e-6,9.16e-6,7.12e-6,2.43e-5,5.93e-6,4.19e-5,6.64e-6} "Conductance per meter";
  parameter Real c[dim_vector_lgc](final min=Modelica.Constants.small,unit="F/m")={2.38e-11,1.01e-10,8.56e-11,5.09e-12,2.71e-11,2.09e-11,7.16e-11,1.83e-11,1.23e-10,2.07e-11} "Capacitance per meter";
  ...
  FAME.Dampers.ElectricalWithConnectEquations _famefault_n[lines](each amount=0.0);
  parameter Integer N(final min=1)=5 "Number of lumped segments";
  parameter Real l[dim_vector_lgc](final min=Modelica.Constants.small,unit="H/m")={5.98e-7,4.44e-7,4.39e-7,3.99e-7,5.81e-7,4.09e-7,4.23e-7,5.96e-7,4.71e-7,6.06e-7} "Inductance per meter";
  ...
  parameter Real r[lines](final min=Modelica.Constants.small,unit="Ohm/m")={4.76e5,1.72e5,1.72e5,1.72e5} "Resistance per meter";
  ...
  FAME.Dampers.ElectricalWithConnectEquations _famefault_p[lines](each amount=0.0);
  ...
  FAME.Bridges.Electrical _famefault_n__p[lines,lines](each amount=0.0);
 protected
  parameter Integer dim_vector_lgc=div(lines*(lines+1),2);

 // algorithms and equations of M_OLine
 equation
  connect(_famefault_p.port_b,s_first.p);
  connect(s_first.n,s[1].p);
  for i in 1:N-2 loop
   connect(s[i].n,s[i+1].p);
  end for;
  connect(s[N-1].n,s_last.p);
  connect(s_last.n,_famefault_n.port_b);
  for _fame_lhs_0 in 1:lines loop
   connect(n[_fame_lhs_0],_famefault_n[_fame_lhs_0].port_a);
  end for;
  for _fame_lhs_0 in 1:lines loop
   connect(p[_fame_lhs_0],_famefault_p[_fame_lhs_0].port_a);
  end for;
  for _fame_lhs_0 in 1:lines, _fame_rhs_0 in 1:lines loop
   connect(n[_fame_lhs_0],_famefault_n__p[_fame_rhs_0,_fame_lhs_0].port_a);
  end for;
  for _fame_lhs_0 in 1:lines, _fame_rhs_0 in 1:lines loop
   connect(p[_fame_lhs_0],_famefault_n__p[_fame_lhs_0,_fame_rhs_0].port_b);
  end for;

 annotation(Icon(coordinateSystem(preserveAspectRatio=false,extent={{-100,-100},{100,100}}),graphics={Rectangle(extent={{60,80},{-60,-80}},lineColor={0,0,255},fillPattern=FillPattern.Solid,fillColor={255,255,255}),Line(points={{40,60},{40,40}}),Line(points={{40,50},{-40,50}}),Line(points={{-40,60},{-40,40}}),Line(points={{40,-40},{40,-60}}),Line(points={{40,-50},{-40,-50}}),Line(points={{-40,-40},{-40,-60}}),Line(points={{40,30},{40,10}}),Line(points={{40,20},{-40,20}}),Line(points={{-40,30},{-40,10}}),Line(points={{0,6},{0,-34}},color={0,0,255},pattern=LinePattern.Dot),Text(extent={{-146,135},{154,95}},textString="%name",lineColor={0,0,255})}),Documentation(info="<html>
...
<table border=\"1\" cellspacing=\"0\" cellpadding=\"2\">
 ...
 // locally defined classes in ULine
    model _famefaults_p3 = FAME.Dampers.ElectricalWithConnectEquations;
    model _famefaults_p2 = FAME.Dampers.ElectricalWithConnectEquations;
    model _famefaults_p1 = FAME.Dampers.ElectricalWithConnectEquations;
    model _famefaults_p1__p3 = FAME.Bridges.Electrical;
 ...
 // algorithms and equations of ULine
 equation
  v13 = _famefault_p1.port_b.v-_famefault_p3.port_b.v;
  v23 = _famefault_p2.port_b.v-_famefault_p3.port_b.v;
  i1 = _famefault_p1.port_b.i;
  ...
  connect(R[N+1].n,_famefault_p2.port_b);
  connect(p3,_famefault_p3.port_a);
  connect(p2,_famefault_p2.port_a);
  connect(p1,_famefault_p1.port_a);
  connect(p1,_famefault_p1__p3.port_a);
  connect(p3,_famefault_p1__p3.port_b);
...
<li><i> 1998   </i>
 ...
 // locally defined classes in TLine1
 ...
 // algorithms and equations of TLine1
 equation
...
<li><i> 1998   </i>
 ...
 // locally defined classes in TLine2
    model _famefaults_n1 = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_p2 = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_p1 = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_n2 = FAME.Dampers.ElectricalWithoutConnectEquations;
 ...
 // algorithms and equations of TLine2
 equation
  assert(Z0>0,"Z0 has to be positive");
...
<li><i> 1998   </i>
 ...
 // locally defined classes in TLine3
    model _famefaults_n1 = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_p2 = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_p1 = FAME.Dampers.ElectricalWithoutConnectEquations;
    model _famefaults_n2 = FAME.Dampers.ElectricalWithoutConnectEquations;
 ...
 // algorithms and equations of TLine3
 equation
  assert(Z0>0,"Z0 has to be positive");
  assert(F>0,"F  has to be positive");
  i1 = (v1-es)/(Z0);
