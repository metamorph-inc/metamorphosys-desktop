...
def convert_json_to_CyPhy_results_format (inputjson):
    ...
    requirements = [vname for vname in data['variables'] if data['variables'][vname]['type'].endswith('.Requirement')]
    # find the terminals
    terminals = [x for x in data['situations'] if x['type'] == 'INSTANT' and not x['children']]
    # pprint.pprint([t for t in terminals if t['complete']])
    results = []
...
def do_envisionment_from_modelica (modelica_model_name, modelica_files, timeout=None, depth=None, CyPhy_output=False):
    """
    Given one or more files containing a model, and the name of that model,
    use OpenModelica's "omc" tool to generate an optimized XML description of that model.
    ...
    json_file = tempfile.mktemp(".json")
    command = [QRM, "--modelica", json_file, modelica_model_name]
    print "command"
    print command
